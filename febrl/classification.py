  ...
  """
    ...
    """
        ...
        if self.train_w_vec_dict != None:
            logging.info(
                "  Number of weight vectors provided:              %d"
                % (len(self.train_w_vec_dict))
            )
        if self.train_match_set != None:
            logging.info(
                "  Number of match training examples provided:     %d"
                % (len(self.train_match_set))
            )
        if self.train_non_match_set != None:
            logging.info(
                "  Number of non-match training examples provided: %d"
  ...
  """
    ...
    """
        ...
        if self.lower_threshold > self.upper_threshold:
            logging.exception(
                "Lower threshold is larger than upper threshold: "
        ...
        #
        if (
            (self.train_w_vec_dict != None)
            and (self.train_match_set != None)
            and (self.train_non_match_set != None)
        ):
            logging.info(
                'Train Fellegi and Sunter classifier: "%s"' % (self.description)
            )
    ...
    """
        ...
        logging.info("")
        logging.info('Train Fellegi and Sunter classifier: "%s"' % (self.description))
        logging.info("  Nothing needs to be done.")
    ...
    """
        ...
        if (len(match_set) + len(non_match_set)) != len(w_vec_dict):
            logging.exception(
                "Weight vector dictionary of different length than"
        ...
        logging.info("")
        logging.info(
            "Testing Fellegi and Sunter classifier using %d weight " % (len(w_vec_dict))
            + "vectors"
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
            % (len(match_set), len(non_match_set))
        )
        ...
        assert (
            num_true_m + num_false_nm + num_false_m + num_true_nm + num_poss_m
        ) == len(w_vec_dict)

        logging.info(
            "  Results: TP = %d, FN = %d, FP = %d, TN = %d; "
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Cross validation for Fellegi and Sunter is the same as" + "testing."
        )
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Classify %d weight vectors using Fellegi and Sunter " % (len(w_vec_dict))
            + "classifier"
        )
        ...
        assert (len(match_set) + len(non_match_set) + len(poss_match_set)) == len(
            w_vec_dict
        )

        logging.info(
            "Classified %d weight vectors: %d as matches, %d as "
            % (len(w_vec_dict), len(match_set), len(non_match_set))
            + "non-matches, and %d as possible matches" % (len(poss_match_set))
        )
...
class OptimalThreshold(Classifier):
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("bin_w"):
                ...
                if value not in ["pos-neg", "pos", "neg"]:
                    logging.exception(
                        'Value of "min_method" is not one of "pos-neg"'
                        + ', "pos", or "neg": %s' % (value)
                    )
                    raise Exception
        ...
        self.log(
            [("Bin width", self.bin_width), ("Minimise method", self.min_method)]
        )  # Log a message
        ...
        if (
            ...
            self.train(
                self.train_w_vec_dict, self.train_match_set, (self.train_non_match_set)
            )
    ...
    """
        ...
        logging.info(
            "Train optimal threshold classifier using %d weight " % (len(w_vec_dict))
            + "vectors"
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
        ...
        for (rec_id_tuple, w_vec) in w_vec_dict.items():
            ...
            for i in range(v_dim):
                ...
                if rec_id_tuple in match_set:
                    ...
                    logging.exception(
                        "Record identifier tuple %s not in match sets!"
                        % (str(rec_id_tuple))
                    )
                    raise Exception
        ...
        for i in range(v_dim):
            ...
            logging.info(
                "  Minimum and maximum binnded weights in dimension"
                + " %d: %.3f / %.3f" % (i, all_weights_list[0], all_weights_list[-1])
            )
            logging.info(
                "      True match weights range:     %.3f to %.3f"
                % (min_match_weight, max_match_weight)
            )
            logging.info(
                "      True non-match weights range: %.3f to %.3f"
            ...
            logging.info(
                "    Optimal threshold in dimension %d is %.3f" % (i, opt_threshold)
            )
            logging.info(
                '      Number of "%s" misclassifications: %d'
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Testing optimal threshold classifier using %d weight " % (len(w_vec_dict))
            + "vectors"
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
        ...
        assert (num_true_m + num_false_nm + num_false_m + num_true_nm) == len(
            w_vec_dict
        )
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Conduct %d-fold cross validation on optimal threshold " % (n)
        ...
        for fold in range(n):
            ...
            assert len(m_set_test_list[fold]) + len(nm_set_test_list[fold]) == len(
                this_fold_test_ids
            )
        ...
        for fold in range(n):  # First extract test record identifier pairs
            ...
            assert len(this_fold_test_m_set) + len(this_fold_train_m_set) == len(
                match_set
            )
            assert len(this_fold_test_nm_set) + len(this_fold_train_nm_set) == len(
                non_match_set
            )
            assert len(this_fold_test_w_vec_dict) + len(
                this_fold_train_w_vec_dict
            ) == len(w_vec_dict)
        ...
        logging.info(
            "Optimal thresholds: %s" % (auxiliary.str_vector(self.opt_threshold_list))
        )

        # Test on complete weight vector dictionary
        #
        [num_true_m, num_false_nm, num_false_m, num_true_nm] = self.test(
            w_vec_dict, match_set, non_match_set
        )
        return [num_true_m, num_false_nm, num_false_m, num_true_nm]
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Classify %d weight vectors using optimal threshold " % (len(w_vec_dict))
            + "classifier"
        )
        ...
        logging.info(
            "Classified %d weight vectors: %d as matches and %d as "
            % (len(w_vec_dict), len(match_set), len(non_match_set))
            + "non-matches"
        )
...
class KMeans(Classifier):
  ...
  """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("max_it"):
                ...
                if value not in ["random", "min/max"]:
                    logging.exception(
                        'Value of "centroid_init" is not one of '
    ...
    """
        ...
        logging.info(
            "Train K-means classifier using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info("  Dimensionality:   %d" % (v_dim))
        ...
        if self.sample == 100.0:
            ...
            rec_id_tuple_sample = random.sample(
                list(w_vec_dict.keys()), num_w_vec_sample
            )
        ...
        logging.info(
            "  Number of weight vectors to be used for clustering: %d"
            % (len(use_w_vec_dict))
        )
        ...
        logging.info(
            'Initial cluster centroids using method "%s":' % (self.centroid_init)
        )
        logging.info(
            "  Initial match centroid:     %s" % (auxiliary.str_vector(m_centroid))
        )
        logging.info(
            "  Initial non-match centroid: %s" % (auxiliary.str_vector(nm_centroid))
        )
        ...
        while (num_changed > 0) and (iter_cnt < self.max_iter_count):
            ...
            if (num_m + num_nm) != num_all:
                logging.exception(
                    "Not all %d weight vectors assigned: M=%d, U=%d"
            ...
            if (num_m == 0) or (num_nm == 0):
                logging.warn(
                    "One cluster is empty: matches=%d, non-matches=%d" % (num_m, num_nm)
                )
            ...
            logging.info(
                "Iteration %d: %d vectors changed cluster assignment"
        ...
        logging.info(
            'Final cluster centroids using method "%s":' % (self.centroid_init)
        )
        logging.info("  Match centroid:     %s" % (auxiliary.str_vector(m_centroid)))
        logging.info("  Non-match centroid: %s" % (auxiliary.str_vector(nm_centroid)))
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Testing K-means classifier using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
            % (len(match_set), len(non_match_set))
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Conduct %d-fold cross validation on K-means classifier " % (n)
        ...
        logging.info(
            "  Match centroid:     %s" % (auxiliary.str_vector(self.m_centroid))
        )
        logging.info(
            "  Non-match centroid: %s" % (auxiliary.str_vector(self.nm_centroid))
        )
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Classify %d weight vectors using K-means classifier" % (len(w_vec_dict))
        )
...
class FarthestFirst(Classifier):
  ...
  """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("dist_m"):
                ...
                if value not in ["traditional", "min/max", "mode/max"]:
                    logging.exception(
                        'Value of "centroid_init" is not one of '
                        + '"traditional, "min/max" or "mode/max": %s' % (value)
                    )
                    raise Exception
                self.centroid_init = value
    ...
    """
        ...
        logging.info(
            "Train farthest first classifier using %d weight vectors"
            % (len(w_vec_dict))
        )
        logging.info("  Dimensionality:   %d" % (v_dim))
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Testing farthest first classifier using %d weight " % (len(w_vec_dict))
            + "vectors"
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Conduct %d-fold cross validation on farthest first " % (n)
            + "classifier using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Classify %d weight vectors using farthest first " % (len(w_vec_dict))
            + "classifier"
        )
...
class SuppVecMachine(Classifier):
  ...
  """
    ...
    """
        ...
        if imp_svm == False:
            logging.exception(
                'Module "svm.py" not installed, cannot use '
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("kernel"):
                ...
                if value not in ["LINEAR", "POLY", "RBF", "SIGMOID"]:
                    logging.exception(
                        "Illegal value for kernel type: %s " % (value)
    ...
    """
        ...
        logging.info("Train SVM classifier using %d weight vectors" % (len(w_vec_dict)))
        logging.info(
            "  Match and non-match sets with %d and %d entries"
            % (len(match_set), len(non_match_set))
        )
        ...
        logging.info(
            "  Number of weight vectors to be used for SVM "
        ...
        #
        try:
            svm_param = svm.svm_parameter(
                svm_type=svm.C_SVC, C=self.C, kernel_type=svm_kernel
            )
            ...
            svm_param = svm.svm_parameter(
                "-s %d -c %f -t %d" % (svm.C_SVC, self.C, svm_kernel)
            )
            self.svm_model = svm.libsvm.svm_train(svm_prob, svm_param)
            self.svm_version = "new"

        logging.info("Trained SVM with %d training examples" % (len(use_w_vec_dict)))
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Conduct %d-fold cross validation on SVM classifier " % (n)
            + "using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
        ...
        for fold in range(n):  # First extract test record identifier pairs
            ...
            [
                ...
                this_fold_test_w_vec_dict, this_fold_test_m_set, this_fold_test_nm_set
            )
  ...
  """
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("s1_match"):
                ...
                if len(value) not in [3, 4]:
                    logging.exception(
                        "Match method tuple is not of length 3 or 4: %s" % (str(value))
                    )
                    raise Exception
                ...
                if len(value) not in [3, 4]:
                    logging.exception(
                        "Non-match method tuple is not of length 3 or "
                        + "4: %s" % (str(value))
                    )
                    raise Exception
                ...
                if value != None:
                    ...
                    if len(value) != 3:
                        logging.exception(
                            "Random selection tuple is not of length 3:"
                            + " %s" % (str(value))
                        )
                        raise Exception
                    self.rand_sel = value
                ...
                if len(value) not in [3, 5]:
                    logging.exception(
                        'Value of "s2_classifier" is not a tuple with '
                        + "three or five elements: %s" % (str(value))
                    )
                    raise Exception
                if value[0] not in ["kmeans", "nn", "svm"]:
                    logging.exception(
                        'Value of "s2_classifier[0]" is not one of '
        ...
        if self.s1_m_method[1] not in ["threshold", "nearest"]:
            logging.exception(
                'Value of "s1_match_method[1]" is neither '
                + '"threshold" nor "nearest"'
            )
            raise Exception
        if self.s1_nm_method[1] not in ["threshold", "nearest"]:
            logging.exception(
                'Value of "s1_non_match_method[1]" is neither '
                + '"threshold" nor "nearest"'
            )
            raise Exception
        ...
        if self.s1_m_method[1] == "nearest":
            auxiliary.check_is_integer(
                's1_match_method[2] for "nearest"', self.s1_m_method[2]
            )
            if len(self.s1_m_method) != 4:
                logging.exception(
                    'Value of "s1_match_method" tuple with "nearest"'
        ...
        if self.s1_nm_method[1] == "nearest":
            auxiliary.check_is_integer(
                's1_non_match_method[2] for "nearest"', self.s1_nm_method[2]
            )
            if len(self.s1_nm_method) != 4:
                logging.exception(
                    'Value of "s1_non_match_method" tuple with '
        ...
        if self.rand_sel != None:
            ...
            if self.rand_sel[0] not in ["uniform", "linear", "exponential"]:
                logging.exception(
                    'Value of "random_selection[0]" is not one of '
            ...
            if float(self.rand_sel[1]) + float(self.rand_sel[2]) > 80:
                logging.exception(
                    "Sum of random selection percentages must be "
        ...
        if self.s2_classifier[0] == "kmeans":
            if len(self.s2_classifier) != 3:
                logging.exception(
                    'Step 2 classifer "kmeans" reguires two '
                    + "parameters (distance measure and maximum "
                    + "iteration count): %s" % (str(self.s2_classifier))
                )
                raise Exception
            auxiliary.check_is_function_or_method("dist_measure", self.s2_classifier[1])
            ...
            if len(self.s2_classifier) != 3:
                logging.exception(
                    'Step 2 classifer "nn" reguires two parameters '
            ...
            if (self.s2_classifier[2] % 2) == 0:  # An even number
                logging.exception(
                    'Number of nearest neighbours "k" for step 2 '
            ...
            if len(self.s2_classifier) != 5:
                logging.exception(
                    'Step 2 classifer "svm" reguires four parameters'
            ...
            if self.s2_classifier[1] not in ["LINEAR", "POLY", "RBF", "SIGMOID"]:
                logging.exception(
                    "Illegal value for kernel type: %s " % (self.s2_classifier[1])
                    + "(possible are: LINEAR, POLY, RBF, SIGMOID)"
                )
                raise Exception
            ...
            if (self.s2_classifier[3] > 0) and (self.s2_classifier[4] == 0):
                logging.exception(
                    "Two-step SVM increment larger than 0 but "
                    + "training percentage set to 0 - not possible."
                )
                raise Exception
            if (self.s2_classifier[3] == 0) and (self.s2_classifier[4] > 0):
                logging.warning(
                    "Two-step SVM increment set to 0 but training "
        ...
        if (self.s2_classifier == "svm") and (imp_svm == False):
            logging.exception(
                'Module "svm.py" not installed, cannot use "svm" '
        ...
        self.log(
            [
                ("Step 1 match method", str(self.s1_m_method)),
                ("Step 1 non-match method", str(self.s1_nm_method)),
                ("Random selection", str(self.rand_sel)),
                ("Step 2 classifier", str(self.s2_classifier)),
            ]
        )  # Log a message
    ...
    """
        ...
        logging.info(
            "Train two-step classifier using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
            % (len(match_set), len(non_match_set))
            ...
            while (len(m_train_set) < m_nearest) and (len(m_heap) > 0):
                ...
                if (m_unique == True) and (prev_tuple != -1):
                    if w_vec_dict[prev_tuple] != w_vec_dict[this_tuple]:
                        m_train_set.add(
                            this_tuple
                        )  # A different weight vector, add tuple
        ...
        logging.info(
            "Set %d weight vectors as matches and %d as non-matches"
            % (len(m_train_set), len(nm_train_set))
        )
        logging.info(
            "  %d weight vectors not included into set of examples"
        ...
        if len(remove_tuple_set) > 0:
            ...
            logging.warn(
                "Removed %d weight vectors from both match and "
                % (len(remove_tuple_set))
                + "non-match training sets"
            )
        ...
        if self.rand_sel != None:
            ...
            if (num_w_vec_left - (num_m_examples + num_nm_examples)) < 0:
                ...
                logging.warn(
                    "Cannot select %d match and %d non-match examples, "
                ...
                logging.warn(
                    "  Ajusted number of random match examples to %d and"
                    % (num_m_examples)
                    + " number of non-match examples to"
                    + " %d" % (num_nm_examples)
                )
            ...
            if (num_m_examples > 0) or (num_nm_examples > 0):
                ...
                logging.info(
                    "Randomly select %d match and %d non-match examples "
                    % (num_m_examples, num_nm_examples)
                    + "using random "
                ...
                logging.info(
                    "  Minimum and maximum distance from 0.0:           "
                    + "%.3f / %.3f" % (w_vec_dist_list[0], w_vec_dist_list[-1])
                )
                logging.info(
                    "    Minimum and maximum distance in match set:     "
                    + "%.3f / %.3f" % (w_vec_m_min_dist, w_vec_m_max_dist)
                )
                logging.info(
                    "    Minimum and maximum distance in non-match set: "
                ...
                logging.info(
                    "  %d weight vectors have so far not been selected"
                    % (num_w_vec_not_sel)
                )
                ...
                while (len(random_m_set) < num_m_examples) or (
                    ...
                    if len(random_m_set) < num_m_examples:  # Select a match example
                        ...
                        if rand_method[:3] == "uni":
                            ...
                            r_ind = (
                                num_w_vec_dist
                                - int(mymath.random_expo(num_w_vec_dist))
                                - 1
                            )
                        ...
                        if w_vec_list != []:  # If so select one at random
                            ...
                            random_m_set.add(
                                rec_id
                            )  # Add record identifiers to match set
                    ...
                    if len(random_nm_set) < num_nm_examples:  # Select a non-match ex.
                        ...
                        if rand_method[:3] == "uni":
                            ...
                            r_ind = (
                                num_w_vec_dist
                                - int(mymath.random_linear(num_w_vec_dist))
                                - 1
                            )
                        else:  # Exponential
                        ...
                        if w_vec_list != []:
                            ...
                            random_nm_set.add(
                                rec_id
                            )  # Add record identifiers to non-matches
        ...
        if (len(m_train_set) == 0) or (len(nm_train_set) == 0):
            logging.warn(
                "At least one of the two training sets is empty: "
        ...
        #
        logging.info(
            "Start step two classification using method: %s "
            % (str(self.s2_classifier))
            + "with %d match and %d non-match" % (len(m_train_set), len(nm_train_set))
            + " training examples"
        )
        ...
        if self.s2_classifier[0] == "svm":  # - - - - - - - - - - - - - - - - - -
            ...
            try:
                svm_param = svm.svm_parameter(
                    svm_type=svm.C_SVC, C=C, kernel_type=svm_kernel
                )
                self.svm_model = svm.svm_model(svm_prob, svm_param)
                self.svm_version = "old"
                ...
                svm_param = svm.svm_parameter(
                    "-s %d -c %f -t %d" % (svm_type, C, svm_kernel)
                )
                self.svm_model = svm.libsvm.svm_train(svm_prob, svm_param)
                self.svm_version = "new"
            ...
            if increment > 0:
                ...
                assert len(w_vec_dict) == len(un_used_w_vec_dict) + len(train_data), (
                    len(w_vec_dict),
                    len(un_used_w_vec_dict) + len(train_data),
                )
                ...
                while len(train_data) < total_train_num_w_vec:
                    ...
                    for (rec_id_tuple, w_vec) in un_used_w_vec_dict.items():
                        ...
                        if self.svm_version == "old":
                            ...
                            c2 = svm.libsvm.svm_predict_values(
                                self.svm_model, x0, dec_values
                            )
                            assert c1 == c2
                    ...
                    if (new_m_class_set_list == []) or (new_nm_class_set_list == []):
                        logging.warn(
                            "Two-step SVM classifier: One of the training "
                    ...
                    #
                    add_num_m = max(
                        1, int(len(new_m_class_set_list) * increment / 100.0)
                    )
                    add_num_nm = max(
                        1, int(len(new_nm_class_set_list) * increment / 100.0)
                    )
                    ...
                    for i in range(add_num_m):  # Add new match training records
                        ...
                        del un_used_w_vec_dict[
                            rec_id_tuple
                        ]  # It is used in training now
                    ...
                    assert len(w_vec_dict) == len(un_used_w_vec_dict) + len(
                        train_data
        ...
        #
            ...
            if max_iter_count > 0:
                ...
                while (num_changed > 0) and (iter_cnt < max_iter_count):
                    ...
                    if (num_m + num_nm) != num_all:
                        logging.exception(
                            "Not all %d weight vectors were assigned: " % (num_all)
                            + "M=%d, U=%d" % (num_m, num_nm)
                        )
                        raise Exception
                    ...
                    logging.info(
                        "Iteration %d: %d vectors changed cluster "
                        % (iter_cnt, num_changed)
                        + "assignment"
                    )
            ...
            logging.info(
                "Initials match centroid:     %s" % (auxiliary.str_vector(m_centroid))
            )
        ...
        #
            ...
            for (rec_id_tuple, w_vec) in w_vec_dict.items():
                ...
                if (rec_id_tuple not in m_train_set) and (
                    ...
                    #
                    if (
                        (this_w_vec not in nn_to_classify_w_vec_dict)
                        and (this_w_vec not in nn_m_train_w_vec_dict)
                        and (this_w_vec not in nn_nm_train_w_vec_dict)
                    ):
            ...
            assert len(nearest_w_vec_heap) == len(nn_to_classify_w_vec_dict), (
                len(nearest_w_vec_heap),
                len(nn_to_classify_w_vec_dict),
            )
                ...
                if nn_to_classify_w_vec_dict != {}:
                    ...
                    for this_w_vec in nearest_w_vec_set:
                        ...
                        if new_dist < largest_dist:
                            this_nearest_list.append(
                                (new_dist, nearest_w_vec_type, nearest_w_vec)
                            )
                        ...
                        if new_train_w_vec_nearest_list == []:
                            ...
                            new_train_w_vec_nearest_list = new_train_w_vec_nearest_list[
                                :k1
                            ]
                        ...
                        #
                        dist_sum = 0.0
                        for (dist_val, match_type, train_w_vec) in this_nearest_list[
                            :k
                        ]:
                            dist_sum += dist_val
                        ...
                        #
                        nn_to_classify_w_vec_dict[this_w_vec] = (
                            this_nearest_list,
                            dist_sum,
                        )
            ...
            logging.exception(
                "Illegal step classifier method: %s" % (str(self.s2_classifier))
            )
            raise Exception
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Testing two-step classifier using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
            % (len(match_set), len(non_match_set))
        ...
        if self.s2_classifier[0] == "svm":  # SVM classifier - - - - - - - - - -
            ...
            if (self.m_centroid == None) or (self.nm_centroid == None):
                logging.warn(
                    "K-means centroids have not been calculated, testing "
                    + " not possible"
                )
                return [0, 0, 0, 0]
            ...
            if (nn_m_train_w_vec_set == None) or (nn_nm_train_w_vec_set == None):
                logging.warn(
                    "Nearest neighbour classifier has not been trained, "
            ...
            logging.exception(
                "Illegal step classifier method: %s" % (self.s2_classifier)
            )
            raise Exception
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Cross validation for two-step classifier is the same as" + "testing."
        )
    ...
    """
        ...
        if self.s2_classifier[0] == "svm":  # SVM classifier - - - - - - - - - -
            ...
            if (self.m_centroid == None) or (self.nm_centroid == None):
                logging.warn(
                    "K-means centroids have not been calculated, "
...
class TAILOR(Classifier):
  ...
  """
    ...
    """
        ...
        logging.info(
            "Train TAILOR classifier using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info("  Dimensionality:   %d" % (v_dim))
        ...
        logging.info(
            "  Initial possible match centroid: %s"
        ...
        while (num_changed > 0) and (iter_cnt < self.max_iter_count):
            ...
            if (num_m + num_nm + num_pm) != num_all:
                logging.exception(
                    "Not all %d weight vectors assigned: "
                    + "M=%d, NM=%d, PM=%d" % (num_all, num_m, num_nm, num_pm)
                )
                raise Exception

            if (num_m == 0) or (num_nm == 0):
                logging.warn(
                    "One cluster is empty: matches=%d, non-matches=%d, "
                    % (num_m, num_nm)
                    + " possible matches=%d" % (num_pm)
                )
                break  # Stop K-means iterations
        ...
        logging.info(
            "  Match centroid:          %s" % (auxiliary.str_vector(m_centroid))
        )
        logging.info(
            "  Non-match centroid:      %s" % (auxiliary.str_vector(nm_centroid))
        )
        logging.info(
            "  Possible match centroid: %s" % (auxiliary.str_vector(pm_centroid))
        )
        logging.info("  Cluster sizes: M=%d, NM=%d, PM=%d" % (num_m, num_nm, num_pm))
    ...
    """
        ...
        logging.info("")
        logging.info(
            "Conduct %d-fold cross validation on TAILOR classifier " % (n)
            + "using %d weight vectors" % (len(w_vec_dict))
        )
        logging.info(
            "  Match and non-match sets with %d and %d entries"
...
def extract_collapse_weight_vectors(manipulate_list, weight_vec_dict, vec_weights=None):
  ...
  """
    ...
    if vec_weights != None:
        ...
        if len(vec_weights) != v_dim:
            logging.exception(
                'Argument "vec_weights" given is of different '
    ...
    for i in range(len(manipulate_list)):
        ...
        for e in manipulate_list[i]:
            if e >= v_dim:
                logging.exception(
                    "Element in tuple %d of the manipulate list " % (i + 1)
                    + "is out of range: %s" % (str(manipulate_list))
                )
                raise Exception
...
def DecisionTree(weight_vec_dict, match_set, non_match_set):
  ...
  """
    ...
    logging.info("")
    logging.info(
        "Classify %d weight vectors using the ID3 decision tree "
        % (len(weight_vec_dict))
        + "classifier"
    )
    logging.info(
    ...
    """
        ...
        for rec_id_tuple in weight_vec_dict.keys():
            if rec_id_tuple in match_set:
                m_count += 1.0
            elif rec_id_tuple in non_match_set:
                nm_count += 1.0
        ...
        if m_count > 0.0:
            data_entropy += (
                -m_count / num_weight_vec * math.log(m_count / num_weight_vec, 2)
            )
        if nm_count > 0.0:
            data_entropy += (
                -nm_count / num_weight_vec * math.log(nm_count / num_weight_vec, 2)
            )
    ...
    """
        ...
        for (val, val_count) in val_count_dict.items():
            ...
            subset_entropy += val_prob * entropy(
                weight_vec_subset_dict, match_set, non_match_set
            )
    ...
    """
        ...
        if isinstance(tree, dict):
            logging.info("    %scolumn %s:" % (str, list(tree.keys())[0]))
    ...
    #
    def create_decision_tree(weight_vec_dict, column_list, match_set, non_match_set):
        """Return a new decision tree based on the examples given.
    """

        # Get the most common value and its count
        #
        (most_common_class, most_common_count) = majority_value(
            weight_vec_dict, match_set, non_match_set
        )
        ...
        for val in best_column_val_list:
            ...
            subtree = create_decision_tree(
                this_weight_vec_dict, new_column_list, match_set, non_match_set
            )
    ...
    column_list = list(range(num_weights))

    # Create the decision tree using all weight vectors - - - - - - - - - - - - -
    #
    id3_tree = create_decision_tree(
        weight_vec_dict, column_list, match_set, non_match_set
    )
    ...
    logging.info(
        "Classified %d weight vectors as matches, %d as non-matches"
        % (len(match_set), len(non_match_set))
    )
