...
class RecordComparator:
  ...
  """
    ...
    """
        ...
        for (field_comp, field_name1, field_name2) in self.field_comparator_list:
            ...
            if field_name1 not in dataset1_field_names:
                logging.exception(
                    'Field "%s" is not in data set 1 field name list: ' % (field_name1)
                    + "%s" % (str(dataset1.field_list))
                )
                raise Exception
            ...
            if field_name2 not in dataset2_field_names:
                logging.exception(
                    'Field "%s" is not in data set 2 field name list: ' % (field_name2)
                    + "%s" % (str(dataset2.field_list))
                )
                raise Exception
        ...
        for i in range(len(self.field_comparison_list)):
            ...
            logging.info(
                "    Field name 1: %s (at column %d)" % (field_name1, field_index1)
            )
            logging.info(
                "    Field name 2: %s (at column %d)" % (field_name2, field_index2)
            )
    ...
    """
        ...
        logging.info(
            'Caching statistics for record comparator "%s"' % (self.description)
        )
        ...
        for (field_comp, field_name1, field_name2) in self.field_comparator_list:
            ...
            if field_comp.do_caching == False:
                ...
                logging.info(
                    "    Maximum and average cache entry count: %d / %.2f"
  ...
  """
    ...
    """
        ...
        if self.val_freq_table != None:
            ...
            for (k, v) in list(self.val_freq_table.items()):
            ...
            if self.freq_max_weight == None:  # Has not be given as argument
                ...
                logging.warning(
                    "Setting maximum frequency agreement weight to "
    ...
    """
        ...
        if self.agree_weight < self.disagree_weight:
            logging.exception(
                "Agreement weight set to a value smaller than the "
                + "disagreement weight"
            )
            raise Exception
        ...
        if self.agree_weight < self.missing_weight:
            logging.exception(
                "Agreement weight set to a value smaller than the " + "missing weight"
            )
            raise Exception

        if self.disagree_weight > self.missing_weight:
            logging.exception(
                "Disagreement weight set to a value larger than " + "the missing weight"
            )
            raise Exception
        ...
        if self.val_freq_table != None:
            ...
            if self.freq_max_weight < self.disagree_weight:
                logging.exception(
                    "Maximum frequency agreement weight set to a "
    ...
    """
        ...
        if access_count in self.cache_warn_counts:  # Check if warning needed
            ...
            if num_count_pairs == self.max_cache_size:  # All pairs have this count
                logging.warning("All cache entries have a count of %d" % (access_count))
    ...
    """
        ...
        #
        if (self.max_cache_size != None) and (len(self.cache) == self.max_cache_size):

            self.cache_num_not_cached += 1  # One more pair not cached

            if self.cache_num_not_cached in [100, 1000, 10000, 100000]:
                logging.warning(
                    "Cache is full, %d comparisons cannot be cached"
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("missing_w"):
                ...
                if self.val_freq_table == None:
                    logging.warning(
                        "No frequency table given, so maximum frequency "
    ...
    """
        ...
        logging.info(
            "    Warnings will be given once all cache entries have "
        ...
        if self.val_freq_table != None:
            logging.info(
                "  Number of entires in frequency table: %d"
                % (len(self.val_freq_table))
            )
            logging.info(
                "  Frequency table sum:                  %f" % (self.val_freq_sum)
            )
            logging.info(
                "  Maximum frequency agreement weight:   %f" % (self.freq_max_weight)
            )
...
class FieldComparatorTruncateString(FieldComparator):
  ...
  """
    ...
    """
        ...
        auxiliary.check_is_not_negative("num_char_compared", self.num_char_compared)

        self.log(
            [("Maximum number of characters compared", self.num_char_compared)]
        )  # Log a message
...
class FieldComparatorNumericPerc(FieldComparator):
  ...
  """
    ...
    """
        ...
        #
        perc_diff = (
            100.0 * abs(float_val1 - float_val2) / max(abs(float_val1), abs(float_val2))
        )
...
class FieldComparatorEncodeString(FieldComparator):
  ...
  """
    ...
    """
            ...
            if keyword.startswith("encode"):
                if value not in ["", None, "None", "none"]:
                    ...
                    if value not in [
                        ...
                        logging.exception(
                            'Value of argument "encode_method" is not '
        ...
        self.log(
            [
                ("Encoding method", str(self.encode_method)),
                ("Reverse flag", self.reverse),
                ("Maximum code length", self.max_code_length),
            ]
        )  # Log a message
...
class FieldComparatorDistance(FieldComparator):
  ...
  """
    ...
    """
        ...
        self.log(
            [
                ("Geocode table length", len(self.geocode_table)),
    ...
    """
        ...
        if (loc1 == None) or (loc2 == None):
            ...
            if loc1 == loc2:
                ...
                x = alpha * math.cos(lati1) * math.cos(lati2) + math.sin(
                    lati1
                ) * math.sin(lati2)
...
class FieldComparatorDate(FieldComparator):
  ...
  """
    ...
    """
        ...
        self.log(
            [
                ("Maximum day 1 before day 2 tolerated", self.max_day1_before_day2),
                ("Maximum day 2 before day 1 tolerated", self.max_day2_before_day1),
    ...
    """
        ...
        if (len(val1) not in [6, 8]) or (len(val2) not in [6, 8]):
            logging.warn(
                "At least one of the field values is not of correct "
        ...
        #
        if (month1 < "01") or (month1 > "12") or (month2 < "01") or (month2 > "12"):
            logging.warn(
                "At least one of the month values is out of range: "
                + "%s / %s" % (val1, val2)
            )
            return self.disagree_weight
        ...
        if days_ok == False:
            logging.warn(
                "At least one of the day values is out of range: "
                + "%s / %s" % (val1, val2)
            )
            return self.disagree_weight
    ...
    """
        ...
        if isinstance(self.day_start, str):
            if (len(self.day_start) == 5) and (":" in self.day_start):
                ...
                logging.exception(
                    'Day start is not a string of length 4 or 5: "%s"'(
                        str(self.day_start)
                    )
                )
                raise Exception
            ...
            if not daystart_str.isdigit():
                logging.exception(
                    "Day start is not a string made of digits: %s" % (str(daystart_str))
                )
                raise Exception
            ...
            if (hrs < 0) or (hrs > 23) or (min < 0) or (min > 59):
                logging.exception(
                    'Day start time value out of range: "%s"' % (daystart_str)
                )
                raise Exception
            ...
            logging.exception(
                "Day start is not a string of length 4 or 5 nor an"
        ...
        self.log(
            [
                (
                    "Maximum time 1 before time 2 tolerated (in minutes)",
    ...
    """
        ...
        if isinstance(val1, str):
            if (len(val1) == 5) and (":" in val1):
                ...
                logging.warn(
                    'Value 1 is not a string of length 4 or 5: "%s"' % (str(val1))
                )
                return self.disagree_weight
        else:
        ...
        if isinstance(val2, str):
            if (len(val2) == 5) and (":" in val2):
                ...
                logging.warn(
                    'Value 2 is not a string of length 4 or 5: "%s"' % (str(val2))
                )
                return self.disagree_weight
        else:
        ...
        if (not time1str.isdigit()) or (not time2str.isdigit()):
            logging.warn(
                "At least one of the field values is not a string made "
        ...
        elif (self.max_time1_before_time2 == 0) and (self.max_time2_before_time1 == 0):
  ...
  """
    ...
    """
        ...
        if self.fix_date == "today":
            ...
            if not isinstance(self.fix_date, tuple):
                logging.exception(
                    'Fix date is not "today" nor a tuple: %s' % (str(self.fix_date))
                )
                raise Exception
            if len(self.fix_date) != 3:
                logging.exception(
                    "Fix date is not a triplet: %s" % (str(self.fix_date))
                )
                raise Exception

            self.fix_date_val = datetime.date(
                self.fix_date[2], self.fix_date[1], self.fix_date[0]
            )
        ...
        self.log(
            [
                ("Fix date", self.fix_date),
                ("Fix date value", self.fix_date_val),
                ("Date format", self.date_format),
    ...
    """
        ...
        if (age1 < 0) or (age1 > 54750):  # Number of days in 150 years
            logging.warning(
                "Illegal value for age 1: %d days" % (age1)
            ...
            logging.warning(
                "Illegal value for age 2: %d days" % (age2)
                + ", set weight to missing value"
            )
            w = self.disagree_weight
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("thres"):
                ...
                if value == 1.0:
                    logging.exception(
                        'Value of argument "threshold" must be smaller'
                        + "then 1.0: %s" % (value)
                    )
                    raise Exception
    ...
    """
        ...
        for i in range(len1):  # Analyse the first string
            ...
            if index > -1:  # Found common character
                ...
                ass1 = ass1 + val1[i]
                workstr2 = (
                    workstr2[:index] + self.JARO_MARKER_CHAR + workstr2[index + 1 :]
                )
        ...
        for i in range(len2):  # Analyse the second string
            ...
            if index > -1:  # Found common character
                ...
                ass2 = ass2 + val2[i]
                workstr1 = (
                    workstr1[:index] + self.JARO_MARKER_CHAR + workstr1[index + 1 :]
                )
  ...
  """
    ...
    """
            ...
            if keyword.startswith("check_s"):
                ...
                if value is None:
                    ...
                    if value not in ["sort", "perm"]:
                        logging.exception(
                            'Value of argument "multi_word" is not one'
        ...
        self.log(
            [
                ...
                ("Check long strings flag", self.check_long),
            ]
        )  # Log a message
    ...
    """
            ...
            for c1 in workstr1:
                for j in range(len(workstr2)):
                    if (c1, workstr2[j]) in self.sim_char_pairs:
                        sim_weight += 3
                        workstr2 = (
                            workstr2[:j] + self.JARO_MARKER_CHAR + workstr2[j + 1 :]
                        )
        ...
        if self.check_long == True:
            ...
            if (
                (min(len1, len2) > 4)
                and (common1 > same_init + 1)
                and (common1 >= min(len1, len2) + same_init)
            ):
    ...
    """
        ...
        if (self.multi_word != None) and ((" " in val1) or (" " in val2)):
            ...
            if self.multi_word == "sort":  # Sort words first
                ...
                logging.debug(
                    "Permutation Winkler best permutation: %s" % (str(max_perm))
                )
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword == "q":
                ...
                if value not in ["average", "shortest", "longest"]:
                    logging.exception(
                        'Value of argument "common_divisor" is not one'
                        + 'of: "average", "shortest", or "longest": %s' % (value)
                    )
                    raise Exception
    ...
    """
        ...
        if (self.padded == False) and (min(num_qgram1, num_qgram2) == 0):
            ...
            if w < self.threshold:  # Similariy is smaller than threshold
                ...
                if self.padded == True:
                    qgram_str1 = (
                        (q - 1) * self.QGRAM_START_CHAR
                        + val1
                        + (q - 1) * self.QGRAM_END_CHAR
                    )
                    qgram_str2 = (
                        (q - 1) * self.QGRAM_START_CHAR
                        + val2
                ...
                #
                qgram_list1 = [
                    qgram_str1[i : i + q] for i in range(len(qgram_str1) - (q - 1))
                ]
                qgram_list2 = [
                    qgram_str2[i : i + q] for i in range(len(qgram_str2) - (q - 1))
                ]
        ...
        if (self.padded == False) and (min(num_qgram1, num_qgram2) == 0):
            ...
            if w < self.threshold:  # Similariy is smaller than threshold
                ...
                for pos_q_gram in short_qgram_list:
                    ...
                    pos_range = list(
                        range(max(pos - self.max_dist, 0), pos + self.max_dist + 1)
                    )
                    ...
                    for test_pos in pos_range:
                        ...
                        if test_pos_q_gram in long_qgram_list:
                            common += 1
                            long_qgram_list.remove(
                                test_pos_q_gram
                            )  # Remove counted q-gram
                            break
            ...
            for i in range(1, m + 1):
                ...
                for j in range(1, n + 1):
                    ...
                    if (i == 1) or (j == 1):  # First characters, no transp possible
                        ...
                        #
                        current[j] = min(
                            previous[j] + 1, current[j - 1] + 1, substitute
                        )

                    else:
                        if (str1[j - 2] == str2[i - 1]) and (
                            str1[j - 1] == str2[i - 2]
                        ):
...
class FieldComparatorSWDist(FieldComparatorApproxString):
  ...
  """
    ...
    """
        ...
        self.log(
            [("Threshold", self.threshold), ("Common divisor", self.common_divisor)]
        )  # Log a message
    ...
    """
        ...
        for (s1, s2) in [(val1, val2), (val2, val1)]:
            ...
            h_diff += (
                s1_len
                * s2_len
                / (self.p + (1 - self.p) * (s1_len + s2_len - s1_len * s2_len))
            )
        ...
        assert (w_lcs_wink >= 0.0) and (
            w_lcs_wink <= 1.0
        ), "Winkler modified LCS similarity weight outside [0,1]: %f" % (w_lcs_wink)
...
class FieldComparatorCompress(FieldComparatorApproxString):
  ...
  """
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("comp"):
                ...
                if value not in ["zlib", "bz2"]:
                    logging.exception(
                        'Value of argument "compressor" is not one'
        ...
        self.log(
            [("Threshold", self.threshold), ("Compression method", self.compressor)]
        )  # Log a message
    ...
    """
        ...
        if self.compressor == "zlib":
            ...
            c12 = 0.5 * (
                len(zlib.compress(val1 + val2)) + len(zlib.compress(val2 + val1))
            )
            ...
            c12 = 0.5 * (
                len(bz2.compress(val1 + val2)) + len(bz2.compress(val2 + val1))
            )
        ...
        if c12 == 0.0:
            ...
            if w < 0.0:
                logging.warning(
                    "%s Compression based comparison smaller than 0.0 "
                    % (self.compressor)
                    + 'with strings "%s" and "%s": %.3f' % (val1, val2, w)
                )
                w = 0.0
  ...
  """
    ...
    """
            ...
            if keyword.startswith("comp_f"):
                if isinstance(value, str):
                    if value == "equal":
                        ...
                        logging.exception(
                            'Value of "comp_funct" must be a function or'
                            + ' the string "equal"'
                        )
                        raise Exception
                else:
        ...
        if (self.comp_funct != "equal") and (self.min_threshold == None):
            logging.exception(
                "Comparison function is given but no minimal " + "threshold"
            )
            raise Exception
        ...
        self.log(
            [
                ...
                ("Minimum threshold", self.min_threshold),
            ]
        )  # Log a message
    ...
    """
        ...
        if self.do_caching:  # Check if values pair is in the cache
            cache_weight = self.__get_from_cache__(val1, val2)
            if cache_weight != None:
                return cache_weight
        ...
        #
            ...
            if common1 != common2:
                logging.warning(
                    "Two-level-Jaro: Different common values for "
