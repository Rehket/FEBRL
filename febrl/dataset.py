...
"""
    ...
    """
        ...
        num_fields = len(list(test_rec_dict.values())[0])
        ...
        for (rec_id, rec_list) in self.readall():
            ...
            if random.random() * 100 < sample:  # Randomly select a record
                ...
                for col_val in rec_list[:num_fields]:
                    ...
                    if col_val == "":
                        ...
                        if (
                            word_analysis == True
                        ):  # Split into words and analyse separate
                        ...
                        if " " in col_val:  # Count number of whitespaces in value
                            max_num_spaces_list[c] = max(
                                max_num_spaces_list[c], (len(col_val.split(" ")) - 1)
                            )
                    c += 1
            ...
            if (
                (log_funct != None)
                and (log_num_recs != None)
                and ((num_records % log_num_recs) == 0)
            ):
                used_time = time.time() - start_time
                processed_perc = 100.0 * float(num_records) / self.num_records
                log_funct(
                    "Read %.2f%% of %d records in %.2f sec "
        ...
        if (log_funct != None) and (log_num_recs != None):
            used_time = time.time() - start_time
            log_funct(
                "Finished reading a %.1f%% sample of %d records in %.2f"
                % (sample, num_records, used_time)
                + " sec"
            )
        ...
        if warn_message_dict != {}:
            warn_msg_tuples = list(warn_message_dict.items())
        ...
        final_stats.append("")
        final_stats.append("Detailed statistics for data set: %s" % (self.file_name))
        final_stats.append("=" * (34 + len(self.file_name)))
        final_stats.append("")
        final_stats.append("Analysis conducted on " + time.asctime())
        final_stats.append("")
        final_stats.append(
            "Analysed %d of %d records (%.2f%% sample)"
        ...
        if word_analysis == True:
            final_stats.append(
                "Frequency analysis based on separate words " + "in fields"
            )
        else:
        ...
        for c in range(num_fields):
            ...
            value_list = list(values_dict_list[c].keys())
            freq_list = list(values_dict_list[c].values())
            ...
            if freq_list_len > 0:
                ...
                value_list.sort()
                final_stats.append(
                    "    Smallest and largest values (as strings): "
                    + '"%s" / "%s"' % (str(value_list[0]), str(value_list[-1]))
                )
                ...
                quantiles_str = auxiliary.str_vector(
                    mymath.quantiles(freq_list, QUANT_LIST), num_digits=2
                )
                ...
                final_stats.append(
                    "  Minimum and maximum value lengths: %d / %d"
                    % (min_length_list[c], max_length_list[c])
                )
                final_stats.append(
                    "  Is-digit: %s, is-alpha: %s, is-alnum: %s"
                    % (str(isdigit_list[c]), str(isalpha_list[c]), str(isalnum_list[c]))
                )
                final_stats.append(
                    "  Maximum number of spaces in values: %d"
                    % (max_num_spaces_list[c])
                )
                final_stats.append(
                    "  Number of records with missing value: %d" % (num_missing_list[c])
                )
        ...
        final_stats.append("Field names             Quantiles  %s" % (quant_list_str))
        final_stats.append("-" * 86)
        ...
        final_stats.append("==================================")
        final_stats.append(
            "(number of record pair comparisons is calculated " + "for a deduplication)"
        )
        final_stats.append("")
        ...
        for c in range(num_fields):
            ...
            if miss_perc <= MISS_PERC_THRES:  # Field suitable for blocking
                ...
                final_stats.append(
                    "%22s  %d unique values, resulting in %d "
                ...
                if miss_perc > 0.0:
                    final_stats.append(
                        " " * 24
                        + "Note field contains %.2f%% (%d) " % (miss_perc, num_miss_rec)
    ...
    """
        ...
        for (field_name, val) in self.field_list:
            if field_name == self.rec_ident:
                logging.info(
                    "    %s: %s (record identifier)" % (str(field_name), str(val))
                )
            else:
  ...
  """
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("file"):
                ...
                if len(value) != 1:
                    logging.exception(
                        'Value of "delimiter" argument must be a one-'
        ...
        if (self.header_line == False) and (self.field_list == None):
            logging.exception(
                'Argument "field_list" must be given if field '
        ...
        if self.access_mode == "read":
            ...
            if (self.file_name.endswith(".gz")) or (self.file_name.endswith(".GZ")):
                ...
                try:
                    ...
                    logging.exception(
                        'Cannot open gzipped CSV file "%s" for reading'
                        % (self.file_name)
                    )
                    raise IOError
                ...
                try:  # Try to open the file in read mode
                    ...
                    logging.exception(
                        'Cannot open CSV file "%s" for reading' % (self.file_name)
                    )
                    raise IOError
            ...
            if self.header_line == True:
                header_line = next(self.csv_parser)
            ...
            try:
                ...
                logging.exception(
                    'Cannot open CSV file "%s" for writing' % (self.file_name)
                )
                raise IOError
            ...
            if self.write_header == True:
                ...
                for (field_name, col) in self.field_list:
                    ...
                    if self.write_quote_char != "":
                        field_name = (
                            self.write_quote_char + field_name + self.write_quote_char
                        )
            ...
            try:
                ...
                logging.exception(
                    'Cannot open CSV file "%s" for appending' % (self.file_name)
                )
                raise IOError
            ...
            logging.exception(
                'Illegal data set access mode: "%s" (not allowed '
                % (str(self.access_mode))
                + "with CSV data set implementation)."
            )
            raise Exception
        ...
        for (field_name, field_col) in self.field_list:
            ...
            auxiliary.check_is_string(
                "Field name for column %d is not a string" % (field_col), field_name
            )
            auxiliary.check_is_integer(
                'Value of field column "%s" is not an ' % (field_name)
                + " integer number",
                field_col,
            )
            auxiliary.check_is_not_negative(
                'Value of field column "%s" is a ' % (field_name) + "negative number",
                field_col,
            )
            if this_col_num != field_col:
                logging.exception(
                    "Column numbers are not consecutive: %s" % (str(self.field_list))
                )
                raise Exception
            else:
                this_col_num += 1
    ...
    """
        ...
        if len_recs_arg == 0:  # No arguments, just read one record - - - - - - -
            ...
            if not isinstance(num_recs, int) or (num_recs < 1):
                logging.exception(
                    "Number of records given is not a positive integer"
            ...
            for i in range(num_recs):
                ...
                if (
                    list(this_rec_dict.keys())[0] in rec_dict
                ):  # Check for unique rec ident.
                    logging.exception(
                        "Already existing record identifier returned: %s"
                        % (str(this_rec_dict))
                    )
                    raise Exception
            ...
            if not isinstance(start_num, int) or (start_num < 0):
                logging.exception(
                    "Start record number given is not a positive "
            ...
            if start_num >= self.num_records:
                logging.exception(
                    "Start record number is larger than the number "
            ...
            logging.exception(
                "Illegal call to read(): 3 or more arguments: %s" % (str(recs))
            )
            raise Exception
    ...
    """
        ...
        if self.access_mode not in ["write", "append"]:
            logging.exception(
                'Data set not initialised for "write" or "append" ' + "access"
            )
            raise Exception

        # Sort record identifiers
        #
        rec_ident_keys = list(rec_dict.keys())
        rec_ident_keys.sort()
        ...
        for rec_ident in rec_ident_keys:
            ...
            if self.write_quote_char != "":
                rec = [self.write_quote_char + s + self.write_quote_char for s in rec]
    ...
    """
        ...
        for field_data in self.field_list:  # Check all entries in field list
            ...
            if (self.header_line == True) and (self.access_mode == "read"):
                ...
                if len(field_data) != 2:
                    logging.exception(
                        "field_list[%d] does not have 2 elements" % (field_cnt)
                    )
                    raise Exception
                auxiliary.check_is_string(
                    "field_list[%d][0]" % (field_cnt), field_data[0]
                )
                auxiliary.check_is_integer(
                    "field_list[%d][1]" % (field_cnt), field_data[1]
                )
                auxiliary.check_is_positive(
                    "field_list[%d][1]" % (field_cnt), field_data[1]
                )
        ...
        if self.access_mode == "read":
            ...
            try:
                ...
                logging.exception(
                    'Cannot open COL file "%s" for writing' % (self.file_name)
                )
                raise IOError
            ...
            logging.exception(
                'Illegal data set access mode: "%s" (not allowed '
                % (str(self.access_mode))
                + "with COL data set implementation)."
            )
            raise Exception
  ...
  """
    ...
    """
        ...
        if self.access_mode != "readwrite":
            logging.exception(
                'Memory data set must be initialised in "readwrite" '
        ...
        for (field_name, not_used) in self.field_list:
            ...
            auxiliary.check_is_string(
                "field name in column %d" % (field_col), field_name
            )
    ...
    """
        ...
        #
        elif isinstance(recs, list) or isinstance(recs, set) or isinstance(recs, set):
            ...
            for rec_ident in recs:
                ...
                if not isinstance(rec_ident, str):
                    logging.exception(
                        'Record identifier is not a string: "%s"' % (str(rec_ident))
                    )
                    raise Exception
            ...
            logging.exception(
                'Illegal argument given to read(): "%s" of type %s'
                % (str(recs), type(recs))
            )
            raise Exception
    ...
    """
        ...
        for rec_key in list(self.dict.keys()):
    ...
    """
        ...
        for rec_ident in rec_dict:
            if rec_ident in self.dict:
                logging.warn(
                    'Record with identifer "%s" is already in the memory ' % (rec_ident)
                    + "data set - overwrite old version."
                )
            else:
...
class DataSetShelve(DataSet):
  ...
  """
    ...
    """
        ...
        if self.access_mode != "readwrite":
            logging.exception(
                'Shelve data set must be initialised in "readwrite" '
                + ' access mode, not: "%s"' % (self.access_mode)
            )
            raise Exception
        ...
        self.log(
            [
                ("Shelve file name", self.file_name),
