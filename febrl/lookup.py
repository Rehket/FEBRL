            ...
            for line in file_data:
                ...
                if (len(l) > 0) and (l[0] != "#"):  # Not empty line and not comment
                    ...
                    if l[:5] == "tag=<":  # It's a line with a new tag
                        ...
                        if tag == "":
                            logging.exception(
                                'Missing tag specification in file "%s"' % (fn)
                            )
                            raise Exception
                        ...
                        if len(line_list) > 2:
                            logging.exception(
                                'Illegal format in file "%s" in line: %s' % (fn, l)
                            )
                            raise Exception
                        ...
                        if len(line_list) == 2:  # Line contains a key - - - - - - - - -
                            ...
                            if self.__contains__(dict_key):  # Already in lookup table
                                ...
                                if dict_val != test_val:
                                    logging.warn(
                                        'Key "%s" already in dictionary with '
                                        % (str(dict_val))
                                        + "different value (old value "
                                        + 'will be over written with "%s")'
                                        % (str(test_val))
                                    )
                            ...
                            vals = line_list[
                                1
                            ].lower()  # Get values in this line in a string
                        ...
                        for (
                            val
                        ) in val_list:  # Loop over all values  - - - - - - - - - - -

                            val_strip = val.strip()

                            if val_strip != "":  # Only append non-empty values
                                key_list = val_strip.split(
                                    " "
                                )  # Make a list of key words
                                if len(key_list) > self.max_key_length:
                                    self.max_key_length = len(
                                        key_list
                                    )  # Update maximal key len
...
class FrequencyLookupTable(LookupTable):
  ...
  """
    ...
    def __init__(self, **kwargs):
        ...
        # A log message - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        #
        logging.info('Initialised frequency look-up table "%s"' % (self.description))
        logging.info("  With default: %s" % (str(self.default)))
    ...
    """
        ...
        for fn in self.file_names:
            ...
            for line in file_data:
                ...
                if (len(l) > 0) and (l[0] != "#"):  # Not empty line and not comment
                    ...
                    if len(ll) != 2:
                        logging.exception(
                            "Illegal file format (not 2 columns) in file"
                            + ': "%s" in line: %s"' % (fn, l)
                        )
                        raise Exception
                    ...
                    try:
                        ...
                        logging.exception(
                            'Illegal value for frequency count: "%s"' % (str(val))
                            + ' in line: "%s"' % (l)
                        )
                        raise Exception
        ...
        # A log message - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        #
        logging.info('Loaded frequency look-up table "%s"' % (self.description))
        logging.info("  From files:        %s" % (str(self.file_names)))
        logging.info("  Number of entries: %i" % (self.length))
...
class GeocodeLookupTable(LookupTable):
  ...
  """
    ...
    def __init__(self, **kwargs):
        ...
        # A log message - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        #
        logging.info('Initialised geocode look-up table "%s"' % (self.description))
        logging.info("  With default: %s" % (str(self.default)))
        ...
        for fn in self.file_names:
            ...
            for line in file_data:
                ...
                if (len(l) > 0) and (l[0] != "#"):  # Not empty line and not comment
                    ...
                    if len(ll) != 3:
                        logging.exception(
                            "Illegal file format (not 3 columns) in file"
                            + ': "%s" in line: %s' % (fn, l)
                        )
                        raise Exception
                    ...
                    try:
                        ...
                        logging.exception(
                            'Longitude: "%s" is not a number in line: "%s"'
                            % (str(int), l)
                        )
                        raise Exception
                    try:
                        ...
                        logging.exception(
                            'Lattitude: "%s" is not a number in line: "%s"'
                            % (str(lati), l)
                        )
                        raise Exception
                    ...
                    #
                    if (int < -180.0) or (int > 180.0):
                        logging.exception("Illegal value for longitude: " + str(int))
                        raise Exception
                    if (lati < -90.0) or (lati > 90.0):
                        logging.exception("Illegal value for latitude: " + str(lati))
                        raise Exception

                    val = [int, lati]  # Value for dictionary

                    if (self.__contains__(key)) and (self.__getitem__(key) != val):
                        logging.exception(
                            'Key "%s" already in look-up table with ' % (str(key))
                            + "different value"
                        )
                        raise Exception
            ...
            if (len(l) > 0) and (l[0] != "#"):  # Not an empty line and not comment
                ...
                if len(ll) == 2:  # Line contains a replacement - - - - - - - - - - -
                    ...
                    if not (
                        ...
                        logging.exception(
                            "Replacement string is not properly quoted: "
                ...
                for v in vv:  # Loop over all values  - - - - - - - - - - - - - - - -
                    ...
                    if org != "":  # Only process non-empty values
                        if not (
                            ...
                            logging.exception(
                                "Original string is not properly quoted: "
