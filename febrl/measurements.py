...
def pairs_completeness(
    weight_vec_dict, dataset1, dataset2, get_id_funct, match_check_funct
):
  ...
  """
    ...
    logging.info(
        "  Data set 1: %s (containing %d records)"
    ...
    if do_dedup == True:
        ...
        logging.info(
            "  Data set 2: %s (containing %d records)"
            % (dataset2.description, dataset2.num_records)
        )
        logging.info("  Data sets differ:       Linkage")
    logging.info(
        "  Number of record pairs in weight vector dictionary: %d"
        % (len(weight_vec_dict))
    )
    ...
    if do_dedup == True:
        ...
        logging.info(
            "  Number of unique entity identifiers in data set 1: %d"
        ...
        for (ent_id, ent_count) in entity_ident_dict2.items():
        ...
        logging.info(
            "  Number of unique entity identifiers in data set 2: %d"
        ...
        if len(entity_ident_dict1) < len(entity_ident_dict2):
            for (ent_id1, rec_list1) in entity_ident_dict1.items():
            ...
            for (ent_id2, rec_list2) in entity_ident_dict2.items():
        ...
        if len(entity_ident_dict3) < len(entity_ident_dict4):
            for (ent_id, ent_count) in entity_ident_dict3.items():
                if ent_id in entity_ident_dict4:
                    tm += ent_count * entity_ident_dict4[ent_id]
        else:
            for (ent_id, ent_count) in entity_ident_dict4.items():
    ...
    logging.info(
        "  Number of true and false matches in weight vector "
...
def reduction_ratio(weight_vector_dict, dataset1, dataset2):
  ...
  """
    ...
    if do_deduplication == True:
        logging.info("  Data sets are the same: Deduplication")
        logging.info(
            "  Total number of possible comparisons: %d"
...
def quality_measures(weight_vec_dict, match_set, non_match_set, match_check_funct):
  ...
  """
    ...
    if (len(match_set) + len(non_match_set)) != len(weight_vec_dict):
        logging.exception(
            "Match and non-match set are not of same length as "
    ...
    logging.info("")
    logging.info(
        "Classification results: TP=%d, FP=%d / TN=%d, FN=%d" % (tp, fp, tn, fn)
    )
    ...
    logging.info(
        "  Accuracy: %.6f  Precision:%.4f  Recall: %.4f  " % (acc, prec, reca)
        + "F-measure: %.4f" % (fmeas)
    )
...
def get_examples_quality(match_set, true_match_set, non_match_set, true_non_match_set):
