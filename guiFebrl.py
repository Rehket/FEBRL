...
class MainFebrlWindow:
    def __init__(self):
        ...
        #
        self.main_notebook_page_active_dict = {
            "Data": True,
            "Explore": False,
            "Standardise": False,
            "Index": False,
        ...
        #
        dict = {
            "main_window_quit": self.quitMain,
    ...
    def initFebrlProjectVariables(self, project_type="Deduplicate"):
        ...
        self.modified = {
            ...
            "output": False,
        }  # Set to True when values change
        ...
        #
        self.stringencode_dict = {
            "None": "",
            "Soundex": "soundex",
            "Mod-Soundex": "mod_soundex",
            "Phonex": "phonex",
            "NYSIIS": "nysiis",
        ...
        self.file_filters = {
            "CSV": ["CSV files", ("*.csv", "*.CSV", "*.csv.gz", "*.CSV.GZ")],
            "COL": ["COL files", ("*.col", "*.COL", "*.col.gz", "*.COL.GZ")],
            "TAB": ["TAB files", ("*.tab", "*.TAB", "*.tab.gz", "*.TAB.GZ")],
            "TXT": [
                "Text files",
                (
                    "*.csv",
        ...
        #
        self.field_comp_dict = {
            "Age": "FieldComparatorAge",
        ...
        #
        self.output_dict = {
            "w_vec_file": (False, "(None)"),
            "histo_file": (False, "(None)", "1.0"),
            "m_status_file": (False, "(None)"),
            "m_datasets": (False, ("(None)", "match_id"), ("(None)", "match_id")),
            "progress_perc": "10",
    ...
    def mainNotebookSwitchPage(self, widget, dummy, curr_page):
        ...
        if prev_page == prev_page2:
            ...
            if curr_page_active == True:
                ...
                print(
                    "Switched from page %s to page %s"
    ...
    def displayCurrentNotebookPage(self):
        ...
        #
        for (page_name, active) in list(self.main_notebook_page_active_dict.items()):
    ...
    def newMain(self, widget):
        ...
        newProjTree = gtk.glade.XML(self.glade_file_name, self.new_project_dialog_name)
    ...
    def saveMain(self, widget):
        print(
            "Clicked Save or activated Save or Save As menu, or Execute on"
            + "Output/Run page"
        )
        ...
        if (self.project_name == None) or (widget_name == "save_as"):
            ...
            saveFileTree = gtk.glade.XML(
                self.glade_file_name, self.save_file_dialog_name
            )
            ...
            print(
                "Selected file name in save dialog: %s" % (self.save_dialog_file_name)
            )
        ...
        if self.project_name != None:
            ...
            if os.access(self.project_name, os.F_OK) == True:
                ...
                overwrite = self.messageDialog(
                    'File "%s" exists. ' % (self.project_name)
            ...
            try:
                ...
                self.messageDialog(
                    "Cannot write project file: %s"(self.project_name), "error"
                )
                return
            ...
            for line in febrl_project_code:
                ...
                if "XXXXXXXXXX" in line:
                    line = line.replace(
                        "XXXXXXXXXX", self.project_name.split(os.sep)[-1]
                    )
    ...
    def activateAbout(self, widget):
        ...
        self.aboutTree = gtk.glade.XML(self.glade_file_name, self.about_dialog_name)
    ...
    def activateLicense(self, widget):
        ...
        self.licenseTree = gtk.glade.XML(self.glade_file_name, self.license_dialog_name)
    ...
    def messageDialog(self, message_str, message_type):
        ...
        gtk_msg_buttons = {
            ...
            "ques": gtk.BUTTONS_YES_NO,
        }[message_type[:4]]

        msgDialog = gtk.MessageDialog(
            None, gtk.DIALOG_MODAL, gtk_msg_type, gtk_msg_buttons, message_str
        )
    ...
    def dataView(self):  # A switch to the Data page
        ...
        for (data_set_index, data_set_suffix) in data_set_index_suffix_list:
            ...
            #
            file_chooser_widget = self.mainTree.get_widget(
                data_set_type.lower() + "_file_chooser_button" + data_set_suffix
            )
            ...
            if data_set_type in ["CSV", "COL", "TAB"]:
                ...
                miss_val_widget = self.mainTree.get_widget(
                    "miss_val_entry" + data_set_suffix
                )
                miss_val_widget.set_text(",".join(miss_val_list))

                header_button_name = (
                    data_set_type.lower() + "_header_check_button" + data_set_suffix
                )
                ...
                #
                strip_button_name = (
                    data_set_type.lower() + "_strip_check_button" + data_set_suffix
                )
            ...
            if data_set_type == "CSV":  # Set the CSV delimiter
                ...
                delimiter_widget = self.mainTree.get_widget(
                    "csv_delimiter_entry" + data_set_suffix
                )
    ...
    def dataSetTypeButtonToggle(self, widget):
        ...
        if widget.get_active() == True:  # Check if this is the activated widget
            ...
            if data_set_type_name not in list(self.data_set_type_page_dict.keys()):
                raise Exception("Illegal data set type widget name: %s" % (widget_name))
            ...
            for data_set_type_name in ["csv", "col", "tab"]:
                file_chooser_widget = self.mainTree.get_widget(
                    data_set_type_name + "_file_chooser_button" + widget_name[-2:]
                )
    ...
    def dataSetTypeHeaderLineToggle(self, widget):
        ...
        #
        self.data_set_info_list[data_set_ind][
            "field_names"
    ...
    def dataSetDelimiterActivate(self, widget):
        ...
        if (len(delimiter_val) > 1) or (delimiter_val == ""):
            ...
            #
            self.data_set_info_list[data_set_index][
                "delimiter"
    ...
    def getDataSetFromFile(self, data_set_index):
        ...
        self.writeStatusBar(
            "Read first %d lines from data set: %s."
    ...
    def generateRecIDField(self, data_set_index):
        ...
        if (rec_id_field_name == self.data_set_default_values["rec_id_field"]) or (
            rec_id_field_name
            == self.data_set_default_values["rec_id_field"] + data_set_suffix + "__"
        ):
    ...
    def generateDataView(self, data_set_index):
        ...
        #
        tree_view_widget = self.mainTree.get_widget(
            "data_set_treeview" + data_set_suffix
        )
        ...
        if data_set_type in ["CSV", "TAB"]:  # - - - - - - - - - - - - - - - - - -
            ...
            for val in col_width_vals.split(","):
                ...
                if (not val.isdigit()) or (int(val < 1)):
                    self.messageDialog(
                        "Column widths must be positive integers.", "error"
                    )
                    return
        ...
        for i in range(1, len(file_data)):  ## PC 26/09 self.num_data_rows+1):
            if len(file_data[i]) != num_fields:
                self.messageDialog(
                    "Different number of fields in data in line %d." % (i), "erro"
                )
        ...
        if header_line == True:
            ...
            for i in range(
                0, len(file_data) - 1
            ):  # First line is already a data record
                data_store.append(file_data[i] + [False, 400])  # No editing, not bold
    ...
    def headerLineEdit(self, cell, path, new_value, user_data):
        ...
        print(
            "Header line column %d edited for data set %d, new value: %s"
    ...
    def dataExecute(self):
        ...
        if (self.project_type == "Link") and (
            ...
            self.messageDialog(
                "Two data sets must be initalised for a linkage!", "warning"
            )
            return  # Don't do anything
        ...
        for (data_set_index, data_set_suffix) in data_set_index_suffix_list:
            ...
            febrl_code.append(
                "# Define input data set %s:" % data_set_suffix[-1].upper()
            )
            febrl_code.append("#")

            febrl_code.append(
                "data_set_%s = dataset.DataSet" % (data_set_suffix[-1])
                + data_set_type_name
            ...
            if data_set_type == "TAB":  # Add a comment for TAB data set
                ...
                last_line = (
                    last_line[:-2]
                    + " (note TAB data set is implemented as "
            ...
            febrl_code.append(
                indention_space + "strip_fields=%s," % (str(strip_fields))
            )

            miss_val_list = data_set_info["miss_val"]
            febrl_code.append(indention_space + "miss_val=%s," % (str(miss_val_list)))
            ...
            if data_set_type in ["CSV", "COL", "TAB"]:
                ...
                febrl_code.append(
                    indention_space + "header_line=%s," % (str(header_flag))
                )
            ...
            if data_set_type in ["CSV", "TAB"]:
                ...
                febrl_code.append(indention_space + 'delimiter="%s",' % (delimiter_val))
        ...
        self.writeStatusBar(
            "Generated Febrl Python code for data set "
    ...
    def exploreExecute(self):
        ...
        if use_sample_widget.get_active() == True:
            ...
            try:
                ...
                self.messageDialog(
                    "Sampling rate must be a positive percentage " + "value.", "error"
                )
                return
        ...
        for analysis_flag in analyse_data_set:  # Loop over both data sets
            ...
            if analysis_flag == True:  # Analyse this data set
                ...
                if data_set_type in ["CSV", "TAB"]:
                    ...
                    self.writeStatusBar(
                        'Initialise CSV data set "%s" for reading.' % (file_name_val)
                    )
                    field_val_list = []
                    for field_num in range(num_fields):
                        field_val_list.append((field_names[field_num], field_num))

                    data_set = dataset.DataSetCSV(
                    ...
                    file_name_val = data_set_info["file_name"]
                    self.writeStatusBar(
                        'Initialise COL data set "%s" for reading.' % (file_name_val)
                    )
                    ...
                    for field_num in range(num_fields):
                        if header_line_flag == True:
                            ...
                            field_val_list.append(
                                (field_names[field_num], int(col_width_list[field_num]))
                            )

                    data_set = dataset.DataSetCOL(
                        file_name=file_name_val,
                        rec_ident=rec_id_str,
                        access_mode="read",
                        header_line=header_line_flag,
                ...
                #
                analysis_str_list = data_set.analyse(
                    sample_rate_val, word_analysis, self.writeStatusBar, 1000
                )
                ...
                #
                try:
                    clean_explore_text = str(explore_text)
                except:  # Have to manually remove the illegal characters
                    clean_explore_text = (
                        "**** Note some non-ASCII characters were "
    ...
    def standardView(self):
        ...
        for comp_std in self.comp_std_list:
            ...
            comp_std_type_label = gtk.Label(
                "<b>%s standardiser:</b>" % (comp_std_type_str)
            )
    ...
    def create_comp_std_button(self, del_comp_std=True):
        ...
        add_date_button.connect(
            "clicked", self.clickAddStandardiserButton, comp_std_button_box
        )
        ...
        if del_comp_std == True:
            ...
            del_button.connect(
                "clicked", self.clickDelCompStdButton, comp_std_button_box
            )
    ...
    def clickAddStandardiserButton(self, widget, add_comp_std_box):
        ...
        comp_std_type_box = gtk.HBox()
        comp_std_type_label = gtk.Label("<b>%s standardiser:</b>" % (comp_std_type_str))
        comp_std_type_label.set_use_markup(True)
        comp_std_type_box.pack_start(comp_std_type_label, False, False, 0)
        comp_std_type_label.show()
        std_box.pack_start(comp_std_type_box, False, False, 0)
    ...
    def new_comp_std(self, comp_std_type, comp_std_dict={}):
        ...
        if comp_std_type == "Date":  # Date specific parameters
            ...
            parse_formats_val = comp_std_dict.get(
                "parse_formats", default_parse_formats
            )
            ...
            hmm_file_name = comp_std_dict.get(
                "%s_hmm" % (comp_std_type_str.lower()), "(None)"
            )
    ...
    def runClickedStandardFileButton(self, widget):
        ...
        # Create file dialog
        #
        saveFileTree = gtk.glade.XML(self.glade_file_name, self.save_file_dialog_name)
        saveFileDialog = saveFileTree.get_widget(self.save_file_dialog_name)
        ...
        if save_dialog_file_name != None:
            ...
            #
            if save_dialog_file_name.split(os.sep)[-1].lower() in ["(none)", "none"]:
    ...
    def create_in_field_combo_box(self, del_in_field=True):
        ...
        add_field_button.connect(
            "clicked", self.clickAddStdInFieldButton, in_field_button_box
        )
        ...
        if del_in_field == True:
            ...
            del_field_button.connect(
                "clicked", self.clickDelStdInFieldButton, in_field_button_box
            )
    ...
    def standardExecute(self):
        ...
        if comp_std_list == []:  # No component standardiser defined
            self.messageDialog(
                "At least one component standardiser is required.", "error"
            )
            return
        ...
        for i in range(len(comp_std_list)):
            ...
            if this_comp_std_type in ["Name", "Addr"]:
                ...
                if (check_word_spill_val == True) and (field_sep_val == ""):
                    self.messageDialog(
                        "Word spill checking only possible with\nan "
                        + "non-empty field separator.",
                        "error",
                    )
                    return
            ...
            if this_comp_std_type == "Date":  # Date specific parameters
                ...
                for format_str in parse_formats_val.split(","):  # Check parse formats
                    if len(format_str.strip()) != 8:
                        self.messageDialog(
                            "Date parse format string has wrong length"
                    ...
                    while tmp_str != "":  # Check if all directives are valid
                        if tmp_str[:2] not in ["%b", "%B", "%d", "%m", "%y", "%Y"]:
                            self.messageDialog(
                                "Illegal directive in format string: "
                                + '"%s".' % (format_str),
                                "error",
                            )
                            return
                ...
                if (
                    (pivot_year_val.isdigit() != True)
                    or (int(pivot_year_val) < 0)
                    or (int(pivot_year_val) > 99)
                ):
                    self.messageDialog(
                        "Pivot year must be an integer between 0 and 99.", "error"
                    )
                    return
                ...
                if (hmm_file_val == "(None)") and (hmm_train_file_val == "(None)"):
                    self.messageDialog(
                        "Both HMM file and HMM training file set to "
            ...
            if num_out_field_none == num_out_field:  # No output field defined
                self.messageDialog(
                    "At least one output field must be not None.", "error"
                )
                return
        ...
        for comp_std_dict in comp_std_list:
            ...
            if corr_list_file_name != "(None)":  # Need to initialise a corr. list
                corr_list_name = "%s_corr_list_%d" % (
                    comp_std_type.lower(),
                    corr_list_cnt,
                )
                febrl_code.append(
                    "%s = lookup.CorrectionList(" % (corr_list_name)
                    + 'descr = "%s correction list")' % (comp_std_type_str)
                )
                febrl_code.append(
                    '%s.load("%s")' % (corr_list_name, corr_list_file_name)
                )
                febrl_code.append("")
            ...
            if tag_table_file_list != []:  # Need to initialise a tag table
                tag_table_name = "%s_tag_table_%d" % (
                    comp_std_type.lower(),
                    tag_table_cnt,
                )
                febrl_code.append(
                    "%s = lookup.TagLookupTable(" % (tag_table_name)
                    + 'descr = "%s tag lookup table")' % (comp_std_type_str)
                )
                indent_space = " " * 22
                febrl_code.append(
                    '%s.load(["%s",' % (tag_table_name, tag_table_file_list[0])
                )
            ...
            if comp_std_type == "Date":  # Date standardiser
                indent_space = " " * 51
                febrl_code.append(
                    "date_comp_std_%d = standardisation." % (comp_std_cnt)
                    + "DateStandardiser(input_fields = %s," % (in_field_str)
                )

            elif comp_std_type == "Phon":  # Phone number standardiser
                indent_space = " " * 56
                febrl_code.append(
                    "phone_comp_std_%d = standardisation." % (comp_std_cnt)
                    + "PhoneNumStandardiser(input_fields = %s," % (in_field_str)
                )

            elif comp_std_type == "Addr":  # Address standardiser
                indent_space = " " * 57
                febrl_code.append(
                    "address_comp_std_%d = standardisation." % (comp_std_cnt)
                    + "AddressStandardiser(input_fields = %s," % (in_field_str)
                )

            else:  # Name standardiser
                indent_space = " " * 51
                febrl_code.append(
                    "name_comp_std_%d = standardisation." % (comp_std_cnt)
                    + "NameStandardiser(input_fields = %s," % (in_field_str)
                )

            # Some common attributes next
            #
            febrl_code.append(indent_space + "output_fields = %s," % (out_field_str))
            if comp_std_type != "Date":
                febrl_code.append(
                    indent_space
                    + 'field_separator = "%s",' % (comp_std_dict["field_separator"])
                )
            if comp_std_type in ["Name", "Addr"]:
                febrl_code.append(
                    indent_space
            ...
            if comp_std_type == "Date":
                febrl_code.append(
                    indent_space + "pivot_year = %s," % (comp_std_dict["pivot_year"])
                )
                ...
                for parse_format in comp_std_dict["parse_formats"].split(","):
                    if first_parse_format == True:
                        febrl_code.append(
                            indent_space + 'parse_formats = ["%s",' % (parse_format)
                        )
                        first_parse_format = False
                    else:
                        febrl_code.append(
                            indent_space + " " * 17 + '"%s",' % (parse_format.strip())
                        )
                ...
                if corr_list_file_name != "(None)":
                    febrl_code.append(
                        indent_space + "corr_list = %s," % (corr_list_name)
                    )
                if tag_table_file_list != []:
                    febrl_code.append(
                        indent_space + "tag_table = %s," % (tag_table_name)
                    )
            ...
            if comp_std_type == "Phon":  # Phone number standardiser
                ...
                febrl_code.append(
                    indent_space + "female_titles = %s," % (female_title_str)
                )
                febrl_code.append(indent_space + "male_titles = %s," % (male_title_str))

                febrl_code.append(
                    indent_space
                    + 'first_name_comp = "%s",' % (comp_std_dict["first_name_comp"])
                )
            ...
            if comp_std_type in ["Addr", "Name"]:
                ...
                if hmm_train_file_name != "(None)":
                    febrl_code.append(
                        indent_space + 'hmm_train_file = "%s",' % (hmm_train_file_name)
                    )
                hmm_seq_prob_file_name = comp_std_dict.get("hmm_seq_prob_file", "(None")
                if hmm_seq_prob_file_name != "(None)":
                    febrl_code.append(
                        indent_space
                        + 'hmm_seq_prob_file = "%s",' % (hmm_seq_prob_file_name)
                    )
        ...
        self.addToLog("")  # Add generated code into log page text
        self.addToLog("=" * 79)
        self.addToLog('Generated Febrl code for "standardise" on %s' % (time.asctime()))
        self.addToLog("")
        for line in febrl_code:
            self.addToLog(line)
        self.addToLog("")
        ...
        self.writeStatusBar(
            "Generated Febrl Python code for standardisation "
    ...
    def indexView(self):
        ...
        skip_missing_widget = self.mainTree.get_widget("index_skip_miss_check_button")
        ...
        if index_method_name in ["FullIndex", "BlockingIndex"]:
            ...
            sim_funct_name_list = list(self.stringcmp_dict.keys())
        ...
        if index_method_name in ["BlockingIndex", "SortingIndex", "QGramIndex"]:
            ...
            special_index_box_widget.pack_start(
                special_index_check_button, False, False, 0
            )
        ...
        button_box = self.create_index_buttons(add_index_def, del_index_def, del_index)
    ...
    #
    def create_index_buttons(self, add_index_def, del_index_def=True, del_index=True):
        index_button_box = gtk.HBox()

        if add_index_def == True:
            add_index_def_button = gtk.Button("Add new index definition")
            add_index_def_button.connect(
                "clicked", self.clickAddIndexDefButton, index_button_box
            )
        ...
        if del_index_def == True:
            ...
            del_index_def_button.connect(
                "clicked", self.clickDelIndexDefButton, index_button_box
            )
        ...
        add_index_button.connect("clicked", self.clickAddIndexButton, index_button_box)
        index_button_box.pack_start(add_index_button, False, False, 4)

        if del_index == True:
            del_index_button = gtk.Button("Delete last index")
            del_index_button.connect(
                "clicked", self.clickDelIndexButton, index_button_box
            )
    ...
    def new_index_def(self, index_def_tuple=None):
        ...
        encode_name_list = list(self.stringencode_dict.keys())  # Encode methods names
    ...
    def indexChangeMethod(self, widget):
        ...
        for k in list(self.index_method.keys()):  # Remove all non general parameters
    ...
    def indexExecute(self):
        ...
        if (index_method_name == "CanopyIndex") and (skip_missing_val == False):
            self.messageDialog(
                "Skip missing must be set (ticked) for canopy index", "warn"
            )
            return
        ...
        if index_method_name == "SortingIndex":  # - - - - - - - - - - - - - -
            ...
            if not self.str_is_normalised(threshold):
                self.messageDialog(
                    "Threshold value must be larger or equal to 0\n"
            ...
            if p.count(",") != 1:  # One comma required to separate two parameters
                self.messageDialog(
                    'Two parameter values required, use comma ","\n'
                    + "to separate them.",
                    "warn",
                )
                return
            ...
            if canopy_method[-1] == "nearest":
                if (not self.str_is_pos_int(p1)) or (not self.str_is_pos_int(p2)):
                    self.messageDialog(
                        "Parameter values for nearest neighbour "
                        + "method must be positive integers.",
                        "warn",
                    )
                    return
                if int(p1) > int(p2):
                    self.messageDialog(
                        "First nearest neighbour value must be\n"
                ...
                if (not self.str_is_normalised_not_zero(p1)) or (
                    ...
                    self.messageDialog(
                        "Parameter values for global thresholds "
                ...
                if float(p1) < float(p2):
                    self.messageDialog(
                        "First global threshold value must be\n"
                        + "larger than second value.",
                        "warn",
                    )
                    return
            ...
            if not self.str_is_percentage_not_zero(del_perc):
                self.messageDialog(
                    "Delete percentage value must be larger than 0\n"
                    + "and smaller or equal to 100.\nSet to 100 if no"
                    + " q-grams shall be deleted.",
                    "warn",
                )
                return
            ...
            if (not self.str_is_pos_int(grid_res)) or (
                ...
                self.messageDialog(
                    "Grid resoultion must be a power of 10 value\n"
            ...
            if int(sub_dim) > int(dim):
                self.messageDialog(
                    "Sub-dimension must be smaller or equal to " + "dimension.", "warn"
                )
                return
            ...
            if not self.str_is_pos_int(min_len):
                self.messageDialog("Minimum length must be a positive integer.", "warn")
                return
            max_block = parameter_box1_children[3].get_text().strip()
            if not self.str_is_pos_int(max_block):
                self.messageDialog(
                    "Maximum block size must be a positive integer.", "warn"
                )
                return
        ...
        if (scrolled_window.get_child() == None) and (index_method_name != "FullIndex"):
        ...
        if index_method_name != "FullIndex":
            ...
            encode_name_list = list(
                self.stringencode_dict.keys()
            )  # Encode methods names
            encode_name_list.sort()
            ...
            while i < num_index_boxes:  # Loop over boxes with index and index def.
                ...
                if max_len == "":  # No maximum length limit provided
                    ...
                    if not self.str_is_pos_int(max_len):
                        self.messageDialog(
                            "Maximum length must be empty or a positive" + " integer.",
                            "warn",
                        )
                        return
                ...
                if encode_funct_name == "Substring":
                    if (
                        (len(endcode_funct_param.split(",")) != 2)
                        or (not endcode_funct_param.split(",")[0].isdigit())
                        or (not endcode_funct_param.split(",")[1].isdigit())
                    ):
                        self.messageDialog(
                            "Parameters for substring encoding function"
                            + " must be two integers (start, end).",
                            "warn",
                        )
                        return
            ...
            for index_list in self.index_def:
                ...
                for index_def_tuple in index_list:  # Each index is one or more tuple
                    ...
                    if index_def_num == num_index_tuples - 1:
                        this_index_def_str += (
                            "]]"
                        )  # Last index definition in this index
                    else:
                        this_index_def_str += "],"
        ...
        if index_method_name in ["BlockingIndex", "SortingIndex", "QGramIndex"]:
            ...
            elif (self.project_type != "Link") and (self.index_method["doDedup"]):
        ...
        #
        febrl_code.append(
            "index = indexing.%s(dataset1 = data_set_a," % (index_method_name)
        )
        if self.project_type == "Link":
        ...
        febrl_code.append(indention_space + "skip_missing = %s," % (str(skip_missing)))
        ...
        if index_method_name == "FullIndex":  # No index definition needed
            ...
            if index_num == 1:  # Only one index
                ...
                for i in range(2, index_num):
                    febrl_code.append(
                        indention_space + " " * 13 + "index_def_%d," % (i)
                    )
                febrl_code.append(
                    indention_space + " " * 13 + "index_def_%d]," % (index_num)
                )
        ...
        #
            ...
            febrl_code.append(
                indention_space + "block_method = %s)" % (block_method_str)
            )
            ...
            febrl_code.append(
                indention_space
                + 'canopy_method = ("%s","%s",' % (canopy_method[0], canopy_method[1])
                + "%s)," % (canopy_method[2])
            )
            q = index_dict["q"]
            padded = index_dict["padded"]
            ...
            padded = index_dict["padded"]
            febrl_code.append(
                indention_space + 'suffix_method = "%s",' % (suffix_method)
            )
            febrl_code.append(
                indention_space
                + "block_method = (%s, %s)," % (block_method[0], block_method[1])
            )
            febrl_code.append(indention_space + "padded = %s)" % (str(padded)))
        ...
        self.addToLog("")  # Add generated code into log page text
        self.addToLog("=" * 79)
        self.addToLog('Generated Febrl code for "indexing" on %s' % (time.asctime()))
        self.addToLog("")
        for line in febrl_code:
            self.addToLog(line)
        self.addToLog("")
        ...
        self.writeStatusBar(
            "Generated Febrl Python code for indexing (see Log"
    ...
    def create_comp_funct_button(self, del_comp_funct=True):
        ...
        add_button.connect(
            "clicked", self.clickAddCompFunctButton, comp_funct_button_box
        )
        ...
        if del_comp_funct == True:
            ...
            del_button.connect(
                "clicked", self.clickDelCompFunctButton, comp_funct_button_box
            )
    ...
    def compChangeFunction(self, widget):
        ...
        #
        field_comp_names = list(self.field_comp_dict.keys())
    ...
    def new_field_comp(self, field_comp_dict={}):
        ...
        #
            ...
            if comp_name == "FieldComparatorWinkler":  # - - - - - - - - - - - - -
                ...
                init_check_button = gtk.CheckButton(
            ...
            #
                ...
                "FieldComparatorSGram",
                "FieldComparatorSWDist",
                "FieldComparatorSyllAlDist",
                "FieldComparatorLCS",
                ...
                if comp_name == "FieldComparatorSGram":  # - - - - - - - - - - - - -
                    ...
                    if "gram_class_list" in field_comp_dict:
                        gram_class_text_entry.set_text(
                    ...
                    if comp_name == "FieldComparatorOntoLCS":
                        p_label = gtk.Label(
                            "     Constant for Hamacher product " + "difference:"
                        )
    ...
    def compareExecute(self):
        ...
        if len(comp_box) == 1:  # 'Add' button only
            self.messageDialog(
                "At least one field comparison function is required.", "error"
            )
            return
        ...
        for fc in range(num_comp_funct):
            ...
            if field_comp_funct_index == -1:
                self.messageDialog(
                    "All field comparison functions have to be " + "selected.", "error"
                )
                return
            ...
            if field_comp_dict["cache_field"] == True:
                ...
                if max_cache_size != "None":
                    if not self.str_is_pos_int(max_cache_size):
                        self.messageDialog(
                            'Maximum cache size must be "None" or a '
                            + "positive integer.",
                            "warn",
                        )
                        return
            ...
            if (
                (not self.str_is_float(miss_weight))
                or (not self.str_is_float(agree_weight))
                or (not self.str_is_float(disagree_weight))
            ):
            ...
            if float(agree_weight) <= float(disagree_weight):
                self.messageDialog(
                    "Agreement weights must be larger\nthan" + " disagreement weights.",
                    "warn",
                )
                return
            ...
            if float(agree_weight) < float(miss_weight):
                self.messageDialog(
                    "Agreement weights must be equal to\nor larger "
                    + "than missing weights.",
                    "warn",
                )
                return
            ...
            if float(miss_weight) < float(disagree_weight):
                self.messageDialog(
                    "Missing weights must be equal to\nor larger "
                    + "than disagreement weights.",
                    "warn",
                )
                return
            ...
            if comp_name not in [
                ...
                if comp_name == "FieldComparatorTruncateString":  # - - - - - - - - -
                    ...
                    if not self.str_is_pos_int(num_char_compared):
                        self.messageDialog(
                            "Number of characters to compare must\nbe a"
                            + " positive integer.",
                            "warn",
                        )
                        return
                    ...
                    if not self.str_is_percentage(max_perc_diff):
                        self.messageDialog(
                            "Maximum percentage difference must\nbe a "
                            + "percentage value.",
                            "warn",
                        )
                        return
                    ...
                    if not self.str_is_pos_float(max_abs_diff):
                        self.messageDialog(
                            "Maximum absolute difference must\nbe zero "
                    ...
                    if encode_name_ind == -1:
                        self.messageDialog(
                            "Encoding method names have to be selected.", "error"
                        )
                        return
                ...
                # elif (comp_name == 'FieldComparatorDistance'):  # TODO ###############
                    ...
                    if not self.str_is_not_neg_int(max_before_day):
                        self.messageDialog(
                            "Maximum day A before day B must\nbe zero "
                            + "or a positive number.",
                            "warn",
                        )
                        return
                    if not self.str_is_not_neg_int(max_after_day):
                        self.messageDialog(
                            "Maximum day A after day B must\nbe zero "
                            + "or a positive number.",
                            "warn",
                        )
                        return
                    ...
                    if date_format not in [
                        ...
                        self.messageDialog(
                            "Illegal date format value. Possible are:\n"
                    ...
                    if not self.str_is_not_neg_int(max_before_time):
                        self.messageDialog(
                            "Maximum time A before time B must\nbe zero"
                            + " or a positive number.",
                            "warn",
                        )
                        return
                    if not self.str_is_not_neg_int(max_after_time):
                        self.messageDialog(
                            "Maximum time A after time B must\nbe zero "
                            + "or a positive number.",
                            "warn",
                        )
                        return
                    ...
                    if len(day_start) not in [4, 5]:
                        self.messageDialog(
                            "Day start must be of the form HH:MM or HHMM.", "warn"
                        )
                        return
                    ...
                    if (hrs < 0) or (hrs > 23) or (mins < 0) or (mins > 59):
                        self.messageDialog(
                            "Day start values out of range: Hours must "
                            + "be [0..23]\nand minutes [0..59].",
                            "warn",
                        )
                        return
                    ...
                    if fix_date != "today":  # Is not 'today' it must be a tuple
                        if (
                            (fix_date[0] != "(")
                            or (fix_date[0] != "(")
                            or (len(fix_date.split(",")) != 3)
                        ):
                            self.messageDialog(
                                'Fix date must either be set to "today" '
                        ...
                        if (
                            ...
                            self.messageDialog(
                                "Fix date (DD,MM,YYYY) out of range: Day "
                                + "must be [1..31],\nmonth [1..12] and year"
                                + " [1900..2030].",
                                "warn",
                            )
                ...
                #
                    ...
                    if (not self.str_is_normalised(threshold)) or (
                        ...
                        self.messageDialog(
                            "Threshold value must be larger or equal\n"
                            + "to 0 and smaller to 1.",
                            "warn",
                        )
                        return
                    ...
                    if comp_name == "FieldComparatorWinkler":  # - - - - - - - - - - -
                        field_comp_dict["check_sim"] = comp_funct_box_children[
                            2
                        ].get_active()
                        field_comp_dict["check_init"] = comp_funct_box_children[
                            3
                        ].get_active()
                        field_comp_dict["check_long"] = comp_funct_box_children[
                            4
                        ].get_active()

                    elif comp_name in [
                        "FieldComparatorQGram",
                        "FieldComparatorPosQGram",
                    ]:  # - - - - - - - - -
                        q = comp_funct_box_children[3].get_text().strip()
                        if not self.str_is_pos_int(q):
                            self.messageDialog(
                                'Value of "q" must be a positive integer.', "warn"
                            )
                            return
                        field_comp_dict["q"] = q
                        ...
                        if common_div_ind == -1:
                            self.messageDialog(
                                "Common divisor names have to be selected.", "error"
                            )
                            return
                        common_div_list = ["shortest", "average", "longest"]
                        field_comp_dict["common_divisor"] = common_div_list[
                            common_div_ind
                        ]

                        field_comp_dict["padded"] = comp_funct_box_children[
                            6
                        ].get_active()
                        ...
                        if comp_name == "FieldComparatorPosQGram":  # One more parameter
                            ...
                            if not self.str_is_pos_int(max_dist):
                                self.messageDialog(
                                    "Maximum distance must be a positive " + "integer.",
                                    "warn",
                                )
                                return
                    ...
                    #
                        ...
                        if comp_name == "FieldComparatorSGram":  # - - - - - - - - - - -
                            ...
                            if (
                                ...
                                self.messageDialog(
                                    "Gram-class value(s) must be a non-" + "empty list",
                                    "warn",
                                )
                                return
                            ...
                            comp_name == "FieldComparatorSyllAlDist"
                        ):  # - - - - - - - -
                            field_comp_dict["do_phonix"] = comp_funct_box_children[
                                4
                            ].get_active()
                            ...
                            if not self.str_is_pos_int(min_len):
                                self.messageDialog(
                                    "Minimum common length must be a "
                                    + "positive integer.",
                                    "warn",
                                )
                                return
                            ...
                            if (
                                comp_name == "FieldComparatorOntoLCS"
                            ):  # - - - - - - - - -
                            ...
                            comp_name == "FieldComparatorTokenSet"
                        ):  # - - - - - - - - -
                            stop_word_list = (
                                comp_funct_box_children[5].get_text().strip()
                            )
                        ...
                        if compressor_index == -1:
                            self.messageDialog(
                                "Compressor(s) have to be selected.", "warn"
                            )
                            return
                        compressor_list = ["zlib", "bz2"]
                        field_comp_dict["compressor"] = compressor_list[
                            compressor_index
                        ]
        ...
        for fc in range(num_comp_funct):
            ...
            descr_str = (
                field_comp_dict["name"] + "-" + field_a_name + "-" + field_b_name
            )

            febrl_code.append(
                "fc_funct_%d = comparison.%s(agree_weight = %s,"
            ...
            febrl_code.append(
                indention_space
                + "disagree_weight = %s," % (field_comp_dict["disagree_weight"])
            )
            febrl_code.append(
                indention_space
                + "missing_weight = %s," % (field_comp_dict["missing_weight"])
            )
            if field_comp_dict["cache_field"] == True:
                ...
                if field_comp_dict["max_cache_size"] != "None":
                    febrl_code.append(
                        indention_space
                        + "max_cache_size = %s," % (field_comp_dict["max_cache_size"])
                    )
            ...
            if comp_name == "FieldComparatorTruncateString":
                febrl_code.append(
                    indention_space
                    + "num_char_compared = %s," % (field_comp_dict["num_char_compared"])
                )

            elif comp_name == "FieldComparatorKeyDiff":
                febrl_code.append(
                    indention_space
                    + "max_key_diff = %s," % (field_comp_dict["max_key_diff"])
                )

            elif comp_name == "FieldComparatorNumericPerc":
                febrl_code.append(
                    indention_space
                    + "max_perc_diff = %s," % (field_comp_dict["max_perc_diff"])
                )

            elif comp_name == "FieldComparatorNumericAbs":
                febrl_code.append(
                    indention_space
                    + "max_abs_diff = %s," % (field_comp_dict["max_abs_diff"])
                )
                ...
                febrl_code.append(
                    indention_space
                    + 'encode_method = "%s",' % (self.stringencode_dict[encode_method])
                )
                febrl_code.append(
                    indention_space
                    + "reverse = %s," % (str(field_comp_dict["reverse"]))
                )
                febrl_code.append(
                    indention_space
                    + "max_code_length = %s," % (field_comp_dict["max_code_length"])
                )
            ...
            # elif (comp_name == 'FieldComparatorDistance'):  # TODO ###############
                ...
                febrl_code.append(
                    indention_space
                    + "max_day1_before_day2 = %s,"
                    % (field_comp_dict["max_day1_before_day2"])
                )
                febrl_code.append(
                    indention_space
                    + "max_day2_before_day1 = %s,"
                    % (field_comp_dict["max_day2_before_day1"])
                )
                febrl_code.append(
                    indention_space
                    + 'date_format = "%s",' % (field_comp_dict["date_format"])
                )

            elif comp_name == "FieldComparatorTime":
                febrl_code.append(
                    indention_space
                    + "max_time1_before_time2 = %s,"
                    % (field_comp_dict["max_time1_before_time2"])
                )
                febrl_code.append(
                    indention_space
                    + "max_time2_before_time1 = %s,"
                    % (field_comp_dict["max_time2_before_time1"])
                )
                febrl_code.append(
                    indention_space
                    + 'day_start = "%s",' % (field_comp_dict["day_start"])
                )

            elif comp_name == "FieldComparatorAge":
                febrl_code.append(
                    indention_space + 'fix_date = "%s",' % (field_comp_dict["fix_date"])
                )
                febrl_code.append(
                    indention_space
                    + "max_perc_diff = %s," % (field_comp_dict["max_perc_diff"])
            ...
            #
                ...
                febrl_code.append(
                    indention_space + "threshold = %s," % (field_comp_dict["threshold"])
                )

                if comp_name == "FieldComparatorWinkler":
                    febrl_code.append(
                        indention_space
                        + "check_sim = %s," % (str(field_comp_dict["check_sim"]))
                    )
                    febrl_code.append(
                        indention_space
                        + "check_init = %s," % (str(field_comp_dict["check_init"]))
                    )
                    febrl_code.append(
                        indention_space
                        + "check_long = %s," % (str(field_comp_dict["check_long"]))
                    )

                elif comp_name in ["FieldComparatorQGram", "FieldComparatorPosQGram"]:
                    febrl_code.append(
                        indention_space + "q = %s," % (field_comp_dict["q"])
                    )
                    febrl_code.append(
                        indention_space
                        + 'common_divisor = "%s",' % (field_comp_dict["common_divisor"])
                    )
                    febrl_code.append(
                        indention_space
                        + "padded = %s," % (str(field_comp_dict["padded"]))
                    )
                    if comp_name == "FieldComparatorPosQGram":
                        febrl_code.append(
                            indention_space
                            + "max_dist = %s," % (field_comp_dict["max_dist"])
                        )
                    ...
                    if comp_name == "FieldComparatorSGram":
                        ...
                        febrl_code.append(
                            indention_space
                            + "do_phonix = %s," % (str(field_comp_dict["do_phonix"]))
                        )
                        ...
                        febrl_code.append(
                            indention_space + "p = %s," % (field_comp_dict["p"])
                        )
                        ...
                        febrl_code.append(
                            indention_space
                            + "stop_word_list = [%s]," % (stop_word_list_str[:-1])
                        )  # Remove last comma

                elif comp_name == "FieldComparatorCompress":
                    febrl_code.append(
                        indention_space
                        + 'compressor = "%s",' % (field_comp_dict["compressor"])
                    )
            ...
            field_comp_list_str += (
                '(fc_funct_%d, "%s", "%s"),\n' % (fc + 1, field_a_name, field_b_name)
                + " " * 19
            )
        ...
        if self.project_type == "Link":
            febrl_code.append(
                "rec_comp = comparison.RecordComparator("
        ...
        self.addToLog("")  # Add generated code into log page text
        self.addToLog("=" * 79)
        self.addToLog('Generated Febrl code for "comparison" on %s' % (time.asctime()))
        self.addToLog("")
        for line in febrl_code:
            self.addToLog(line)
        self.addToLog("")
        ...
        self.writeStatusBar(
            "Generated Febrl Python code for comparisons (see "
    ...
    def classifyView(self):  # A switch to the Classify page
        ...
        classifier_method_widget = self.mainTree.get_widget(
            "classifier_method_combo_box"
        )
        ...
        if classifier_method_name == "FellegiSunter":  # - - - - - - - - - - - -
            ...
            #
            m_method_combo_box.connect("changed", self.classifyTwoStepChangeMatchMethod)
            ...
            # Connect to method that will handle changes
            #
            nm_method_combo_box.connect(
                "changed", self.classifyTwoStepChangeNonMatchMethod
            )
            ...
            if rand_sel_combo_box.get_active() != 0:
                ...
                rand_nm_perc_text_entry.set_text(classifier_dict["random_selection"][2])
                classifier_param_box3.pack_start(rand_m_perc_label, False, False, 0)
                classifier_param_box3.pack_start(
                    rand_m_perc_text_entry, False, False, 0
                )
                classifier_param_box3.pack_start(rand_nm_perc_label, False, False, 0)
                classifier_param_box3.pack_start(
                    rand_nm_perc_text_entry, False, False, 0
                )
            ...
            # Connect to method that will handle changes
            #
            classifier_combo_box.connect(
                "changed", self.classifyTwoStepChangeClassifier
            )
            ...
            if classifier_combo_box.get_active() == 0:  # SVM classifier
                ...
                classifier_param_box4.pack_start(
                    dist_measure_combo_box, False, False, 0
                )
                dist_measure_label.show()
                dist_measure_combo_box.show()
        ...
        if classifier_method_name in ["OptimalThreshold", "SuppVecMachine"]:
            ...
            for field_comp_dict in self.field_comp_list:
                ...
                if field_comp_dict["name"] == "Str-Exact":  # Exact string comparison
                    ...
                    if (field_name1 == field_name2) and (self.project_type != "Link"):
                        match_status_name = (
                            'Exact match on comparison of field "%s"' % (field_name1)
                            + " with itself"
                        )
                    else:  # Two different field names or linkage
                        match_status_name = (
                            "Exact match on comparison of fields "
            ...
            if len(self.exact_comp_list) == 0:
                self.messageDialog(
                    "No exact comparisons given, so match status\n"
        ...
        for hbox in [
            classifier_param_box2,
            classifier_param_box3,
    ...
    def classifyExecute(self):
        ...
        if classifier_method_name == "FellegiSunter":  # - - - - - - - - - - - -
            ...
            if float(lower_threshold) > float(upper_threshold):
                self.messageDialog(
                    "Lower threshold must be smaller or\n"
                    + "equal to upper threshold.",
                    "warn",
                )
                return
            ...
            if (not self.str_is_float(bin_width_val)) or (float(bin_width_val) == 0):
            ...
            classifier_dict["min_method"] = ["pos-neg", "pos", "neg"][min_method_index]
            ...
            if sample_val != "":
                if not self.str_is_percentage_not_zero(sample_val):
                    self.messageDialog(
                        "Sample value can be empty (not applied)\n"
            ...
            if fuzzy_thres_val != "":
                if not self.str_is_normalised(fuzzy_thres_val):
                    self.messageDialog(
                        "Fuzzy threshold can be empty (not applied)\n"
                        + "or its value must be larger or equal\n"
                        + "to 0 and smaller or equal to 1.",
                        "warn",
                    )
                    return
            ...
            if classifier_method_name == "KMeans":
                ...
                if not self.str_is_not_neg_int(max_iter_count_val):
                    self.messageDialog(
                        "Maximum iteration count must be\n"
            ...
            if classifier_method_name == "KMeans":
                classifier_dict["centroid_init"] = ["min/max", "random"][
                    centroid_init_index
                ]
            else:  # Farthest first
            ...
            classifier_dict["kernel_type"] = ["LINEAR", "POLY", "RBF", "SIGMOID"][
                kernel_type_index
            ]
            ...
            if not self.str_is_float(nm_method_val):
                self.messageDialog(
                    "Non-match comparison value must be a number.", "warn"
                )
                return

            if float(m_method_val) < float(nm_method_val):
                self.messageDialog(
                    "Match comparison value must be larger than\n"
            ...
            if m_method_type == "nearest":
                ...
                if not self.str_is_pos_int(m_nearest_val):
                    self.messageDialog(
                        "Match selection nearest must be a\n"
                ...
                if not self.str_is_normalised_not_zero(m_threshold_val):
                    self.messageDialog(
                        "Match selection threshold value must be\n"
            ...
            if nm_method_type == "nearest":
                ...
                if not self.str_is_pos_int(nm_nearest_val):
                    self.messageDialog(
                        "Non-match selection nearest must be a\n"
                        + "positive integer value.",
                        "warn",
                    )
                    return
                ...
                if not self.str_is_normalised_not_zero(nm_threshold_val):
                    self.messageDialog(
                        "Non-match selection threshold value must be"
                        + "\nlarger than 0 and smaller or equal to 1.",
                        "warn",
                    )
                    return
            ...
            if rand_sel_type == None:
                ...
                if (not self.str_is_percentage(m_perc_val)) or (
                    ...
                    self.messageDialog(
                        "Random selection percentage values have\n"
                        + "to be percentages (0..100).",
                        "warn",
                    )
                    return
                if (float(m_perc_val) + float(nm_perc_val)) > 80:
                    self.messageDialog(
                        "Sum of random selection percentage values\n"
            ...
            if s2_classifier_type == "svm":
                ...
                kernel_type_val = ["LINEAR", "POLY", "RBF", "SIGMOID"][
                    kernel_type_index
                ]
                ...
                dist_measure_val = ["Manhatten", "Euclidean", "L-Infinity", "Canberra"][
                    dist_measure_index
                ]
        ...
        if classifier_method_name == "FellegiSunter":  # - - - - - - - - - - - -
            ...
            febrl_code.append(
                ...
                + "lower_threshold = %s," % (classifier_dict["lower_threshold"])
            )
            febrl_code.append(
                indention_space
                + "upper_threshold = %s)" % (classifier_dict["upper_threshold"])
            )
            ...
            febrl_code.append(
                "classifier = classification.OptimalThreshold("
                + "bin_width = %s," % (classifier_dict["bin_width"])
            )
            febrl_code.append(
                indention_space + 'min_method = "%s")' % (classifier_dict["min_method"])
            )
            ...
            febrl_code.append(
                "classifier = classification.KMeans("
                + "dist_measure = %s," % (febrl_dist_str)
            )
            febrl_code.append(
                indention_space
                + "max_iter_count = %s," % (classifier_dict["max_iter_count"])
            )
            if ("sample" in classifier_dict) and (classifier_dict["sample"] != ""):
                febrl_code.append(
                    indention_space + "sample = %s," % (classifier_dict["sample"])
                )
            if ("fuzzy_reg_thres" in classifier_dict) and (
                ...
                febrl_code.append(
                    indention_space
                    + "fuzz_reg_thres = %s," % (classifier_dict["fuzzy_reg_thres"])
                )
            febrl_code.append(
                indention_space
                + 'centroid_init = "%s")' % (classifier_dict["centroid_init"])
            )
            ...
            febrl_code.append(
                "classifier = classification.FarthestFirst("
                + "dist_measure = %s," % (febrl_dist_str)
            )
            if ("sample" in classifier_dict) and (classifier_dict["sample"] != ""):
                febrl_code.append(
            ...
            febrl_code.append(
                "classifier = classification.SuppVecMachine("
                + 'kernel_type = "%s",' % (classifier_dict["kernel_type"])
            )
            if ("sample" in classifier_dict) and (classifier_dict["sample"] != ""):
                febrl_code.append(
                    indention_space + "sample = %s," % (classifier_dict["sample"])
                )
            febrl_code.append(indention_space + "C = %s)" % (classifier_dict["C"]))
            ...
            febrl_code.append(
                "classifier = classification.TwoStep("
                + "s1_match_method = %s," % (s1_method_str)
            )
            febrl_code.append(
                indention_space + "s1_non_match_method = %s," % (s1_non_method_str)
            )

            if rand_sel_str != None:
                febrl_code.append(
                    indention_space + "random_selection = %s," % (rand_sel_str)
                )

            febrl_code.append(
                indention_space + "s2_classifier = %s)" % (s2_classifier_str)
            )
        ...
        self.addToLog("=" * 79)
        self.addToLog(
            'Generated Febrl code for "classification" on %s' % (time.asctime())
        )
        self.addToLog("")
        for line in febrl_code:
            self.addToLog(line)
        ...
        self.writeStatusBar(
            "Generated Febrl Python code for classification"
            + " (see Log page for generated code)."
        )
    ...
    def runView(self):
        ...
        if self.project_type == "Standardise":  # - - - - - - - - - - - - - - - -
            ...
            #
            progress_perc_text_entry = self.mainTree.get_widget(
                "run_std_progress_entry"
            )
            ...
            for field_name in field_name_list:
                ...
                pass_field_table.attach(
                    pass_field_check_button, col_ind, col_ind + 1, row_num, row_num + 1
                )
            ...
            if output_dict["m_datasets"][1][0] == "(None)":
                ...
                if dot_index > 0:
                    match_dataset_a_name = (
                        org_file_name[:dot_index] + "-match" + org_file_name[dot_index:]
                    )
                else:
                    match_dataset_a_name = org_file_name
                output_dict["m_datasets"] = (
                    output_dict["m_datasets"][0],
                    (match_dataset_a_name, output_dict["m_datasets"][1][1]),
            ...
            if self.project_type == "Link":
                if output_dict["m_datasets"][2][0] == "(None)":
                    ...
                    output_dict["m_datasets"] = (
                        ...
                        (match_dataset_b_name, output_dict["m_datasets"][2][1]),
                    )

            # Set progress percentage value - - - - - - - - - - - - - - - - - - - -
            #
            progress_perc_text_entry = self.mainTree.get_widget(
                "run_link_progress_entry"
            )
            if output_dict["progress_perc"] != None:
                progress_perc_text_entry.set_text(output_dict["progress_perc"])
            else:
            ...
            #
            len_filter_text_entry = self.mainTree.get_widget("run_length_filter_entry")
            ...
            #
            coff_th_text_entry = self.mainTree.get_widget("run_cutoff_threshold_entry")
            ...
            histo_file_button = self.mainTree.get_widget("save_histogram_file_button")
            ...
            m_stat_file_button = self.mainTree.get_widget("save_m_status_file_button")
            ...
            #
            for wid_name in [
                "run_data_set_b_label",
                "save_data_set_b_file_button",
                "data_set_b_field_label",
                "run_data_set_b_field_entry",
            ]:
                if self.project_type == "Link":
            ...
            m_dataset_tick_box = self.mainTree.get_widget("m_data_set_file_checkbutton")
            ...
            #
            for wid_name in [
                "run_data_set_a_label",
                "save_data_set_a_file_button",
                "data_set_a_field_label",
                "run_data_set_a_field_entry",
            ]:
            ...
            if m_dataset_details[0] == True:  # Sensitive
                ...
                if self.project_type == "Link":  # Also show second data set
                    ...
                    dsb_f_button = self.mainTree.get_widget(
                        "save_data_set_b_file_button"
                    )
                    dsb_f_button.set_label(dsb_details[0].split(os.sep)[-1])
                    dsb_field_ent = self.mainTree.get_widget(
                        "run_data_set_b_field_entry"
                    )
    ...
    def runSaveFileButtonToggle(self, widget):
        ...
        if "w_vec" in widget_name:
            self.output_dict["w_vec_file"] = (
                not self.output_dict["w_vec_file"][0],
                self.output_dict["w_vec_file"][1],
            )
        elif "histo" in widget_name:
            self.output_dict["histo_file"] = (
                not self.output_dict["histo_file"][0],
    ...
    def runClickedFileNameButton(self, widget):
        ...
        if self.save_dialog_file_name != None:
            ...
            # Special case - if None was typed in, set to (None)
            #
            if self.save_dialog_file_name.split(os.sep)[-1].lower() in [
                "(none)",
                "none",
            ]:
                self.save_dialog_file_name = "(None)"
            ...
            if ("w_vec") in widget_name:
                ...
                self.output_dict["w_vec_file"] = (
                    self.output_dict["w_vec_file"][0],
                    self.save_dialog_file_name,
                )
            elif ("histo") in widget_name:
                self.output_dict["histo_file"] = (
                    self.output_dict["histo_file"][0],
                    self.save_dialog_file_name,
                    self.output_dict["histo_file"][2],
                )
            elif ("status") in widget_name:
                self.output_dict["m_status_file"] = (
                    self.output_dict["m_status_file"][0],
                    self.save_dialog_file_name,
                )
    ...
    def runExecute(self):
        ...
        if self.project_type == "Standardise":  # - - - - - - - - - - - - - - - -
            ...
            #
            progr_perc_val = self.mainTree.get_widget(
                "run_std_progress_entry"
            ).get_text()
            ...
            if progr_perc_val.lower() in ["", "none"]:  # Do not report progress
                ...
                self.messageDialog(
                    'Progress report percentage must be "None"\n'
            ...
            febrl_code.append("")
            febrl_code.append(
                "# Define standardised output data set and record " + "standardiser"
            )
            febrl_code.append("#")
            ...
            #
            febrl_code.append(
                "data_set_a_std = dataset.DataSetCSV(description="
                ...
                for out_field in cs_out_field_list:
                    ...
                    if out_field != "None":
                        ...
                        if field_num == 0:  # First field
                            febrl_code.append(
                                indent_space
                                + 'field_list = [("%s",%d),' % (out_field, field_num)
                            )
                        else:
                            febrl_code.append(
                                indent_space
                                + " " * 14
                                + '("%s",%d),' % (out_field, field_num)
                            )
                        field_num += 1
            ...
            for pass_field in pass_field_list:
                ...
                febrl_code.append(
                    indent_space + " " * 14 + '("%s",%d),' % (pass_field, field_num)
                )
                field_num += 1
            ...
            febrl_code.append("#")
            febrl_code.append(
                "rec_std = standardisation.RecordStandardiser("
            ...
            febrl_code.append(
                indent_space + "comp_stand_list = [%s]," % ",".join(comp_std_name_list)
            )
            if output_dict["progress_perc"] != None:
                febrl_code.append(
                    indent_space
                    + "progress_report = %s," % output_dict["progress_perc"]
                )
            ...
            #
            len_filter_val = self.mainTree.get_widget(
                "run_length_filter_entry"
            ).get_text()
            ...
            if len_filter_val.lower() in ["", "none", "(none)"]:  # Dont use len-filt
                ...
                self.messageDialog(
                    'Length filtering percentage must be "None"\n'
                    + "or a percentage value larger than 0.",
                    "warn",
                )
                return

            if (
                output_dict.get("length_filter_perc", new_len_filter_val)
                != new_len_filter_val
            ):
                self.re_run["w_vec_generate"] = True
            ...
            #
            coff_th_val = self.mainTree.get_widget(
                "run_cutoff_threshold_entry"
            ).get_text()
            ...
            if output_dict.get("cut_off_threshold", new_coff_th_val) != new_coff_th_val:
                self.re_run["w_vec_generate"] = True
            ...
            if output_dict["m_datasets"][0] == True:
                if self.project_type == "Link":
                    if (output_dict["m_datasets"][1][0] == "(None)") or (
                        ...
                        self.messageDialog(
                            "Two match data set file names\n"
                            + "need to be given for a linkage.",
                            "warn",
                        )
                        return
                    if (
                        ...
                        self.messageDialog(
                            "Match data set file names for\n"
                            + "linkage have to be different.",
                            "warn",
                        )
                        return
                    ...
                    if (in_file_name_a == output_dict["m_datasets"][1][0]) or (
                        ...
                        self.messageDialog(
                            "Match data set file names have to be "
                            + "different from input data set file names.",
                            "warn",
                        )
                        return

                    fna = self.mainTree.get_widget(
                        "run_data_set_a_field_entry"
                    ).get_text()
                    fna = fna.strip()
                    fnb = self.mainTree.get_widget(
                        "run_data_set_b_field_entry"
                    ).get_text()
                    fnb = fnb.strip()
                    if (fna == "") or (fnb == ""):
                        self.messageDialog(
                            "Two match identifier field names\n"
                            + "need to be given for a linkage.",
                            "warn",
                        )
                        return
                    ...
                    if (fna in field_name_a_list) or (fnb in field_name_b_list):
                        self.messageDialog(
                            "Match identifier field names have to be "
                    ...
                    if in_file_name_a == output_dict["m_datasets"][1][0]:
                        self.messageDialog(
                            "Match data set file name has to be different"
                            + " from input data set file name.",
                            "warn",
                        )
                        return
                    ...
                    if fna in field_name_a_list:
                        self.messageDialog(
                            "Match identifier field name has to be "
                            + "different from all other data set field "
                            + "names.",
                            "warn",
                        )
                ...
                self.output_dict["m_datasets"] = (
                    True,
                    (output_dict["m_datasets"][1][0], fna),
                    (output_dict["m_datasets"][2][0], fnb),
                )
            ...
            if output_dict["histo_file"][0] == False:  # Don't save into file
                febrl_code.append(
                    ...
                    + "class_w_vec_dict, %s)" % (output_dict["histo_file"][2])
                )
            else:
                febrl_code.append(
                    "histo_str_list = output.GenerateHistogram("
                    + 'class_w_vec_dict, %s, "%s")'
                    % (output_dict["histo_file"][2], output_dict["histo_file"][1])
                )
            febrl_code.append("")
            ...
            if output_dict["m_status_file"][0] == True:
                febrl_code.append(
                    "output.SaveMatchStatusFile(class_w_vec_dict, "
            ...
            if output_dict["m_datasets"][0] == True:
                if self.project_type == "Link":
                    febrl_code.append(
                        "output.SaveMatchDataSet(m_set, data_"
                    ...
                    febrl_code.append(
                        "output.SaveMatchDataSet(m_set, data_set_a, "
                        + '"%s", "%s")'
                        % (
                            output_dict["m_datasets"][1][1],
                            output_dict["m_datasets"][1][0],
        ...
        self.addToLog("")  # Add generated code into log page text
        self.addToLog("=" * 79)
        self.addToLog('Generated Febrl code for "output" on %s' % (time.asctime()))
        self.addToLog("")
        for line in febrl_code:
            self.addToLog(line)
        self.addToLog("")
        ...
        self.writeStatusBar(
            "Generated Febrl Python code for output files "
            + "(see Log page for generated code)."
        )
        ...
        #
        do_save = self.messageDialog(
            "Do you want to save final project code " + "before running it?",
            "c_question",
        )
        if do_save == True:
            self.saveMain(gtk.HSeparator())  # Need to provide a widget

        do_run = self.messageDialog(
            "Do you want to run the project code now?", "c_question"
        )
        if do_run == True:
            if self.project_type == "Standardise":
                self.runStdCode()
    ...
    def generateAllCode(self):
        ...
        febrl_code.append("#")
        febrl_code.append('# Generated using "guiFebrl.py" on %s' % (time.asctime()))
        febrl_code.append("# " + "=" * 77)
        febrl_code.append("")

        febrl_code.append(
            "# Import necessary modules (Python standard modules"
        ...
        if self.febrl_code["data"] == None:
            febrl_code.append(
                "# WARNING: No code for data set initialisation " + " provided"
            )
            febrl_code.append("")
        else:
        ...
        if self.project_type == "Standardise":
            ...
            if self.febrl_code["comparison"] == None:
                febrl_code.append(
                    "# WARNING: No code for field and record " + "comparisons provided"
                )
                febrl_code.append("")
            else:
            ...
            if self.febrl_code["indexing"] != None:
                ...
                if self.output_dict["w_vec_file"][0] == True:  # Nothing is returned
                    ...
                    #
                    febrl_code.append(
                        "[field_names_list, w_vec_dict] = output."
            ...
            if self.febrl_code["classification"] == None:
                ...
                #
                if self.classifier_method["name"] in [
                    "OptimalThreshold",
                    "SuppVecMachine",
                ]:

                    febrl_code.append(
                        "# Get true match and non-match status for "
                        + "supervised classifier"
                    )
                    febrl_code.append("")

                    match_status_field = self.classifier_method.get(
                        "match_status_ind", -1
                    )
                    if match_status_field < 0:
                        ...
                        match_status_field_index = self.exact_comp_list[
                            match_status_field
                        ][1]
                        ...
                        febrl_code.append(
                            "  return (w_vec[%d] == 1.0)" % (match_status_field_index)
                        )
                        febrl_code.append("")
                        ...
                        febrl_code.append("#")
                        febrl_code.append(
                            "class_w_vec_dict = classification."
                    ...
                    febrl_code.append("#")
                    febrl_code.append(
                        "class_w_vec_dict = w_vec_dict  # Use orignal "
                ...
                febrl_code.append("#")
                febrl_code.append(
                    "[m_set, nm_set, pm_set] = classifier.classify("
                    + "class_w_vec_dict)"
                )
                febrl_code.append("")
                ...
                if self.classifier_method["name"] in [
                    ...
                    febrl_code.append("#")
                    febrl_code.append(
                        "acc, prec, reca, fmeas = measurements."
                    ...
                    if self.project_type == "Link":
                        febrl_code.append(
                            "rr = measurements.reduction_ratio("
                    ...
                    febrl_code.append(
                        "pq = measurements.pairs_quality(w_vec_dict, "
                        + "match_status_funct)"
                    )
    ...
    def runStdCode(self):
        ...
        #
        if (
            (self.febrl_code["data"] == None)
            or (self.febrl_code["standardise"] == None)
            or (self.febrl_code["output"] == None)
        ):
            self.messageDialog(
                "Cannot run standardisation project\n"
        ...
        if self.data_set_type_list[0] in ["CSV", "TAB"]:
            ...
            for field_num in range(len(in_data_set_info["field_names"])):
                field_val_list.append(
                    (in_data_set_info["field_names"][field_num], field_num)
                )

            ids = dataset.DataSetCSV(
                access_mode="read",
        ...
        self.writeStatusBar(
            "Initalised input data set with %d records." % (ids.num_records)
        )
        ...
        for cs_dict in comp_std_list:
            ...
            if comp_std_type == "Date":  # - - - - - - - - - - - - - - - - - - - -
                ...
                this_cs = standardisation.DateStandardiser(
                    input_fiel=in_field_list,
                ...
                this_cs = standardisation.PhoneNumStandardiser(
                    input_fi=in_field_list,
                    output_fiel=out_field_list,
                ...
                this_cs = standardisation.AddressStandardiser(
                    input_fi=in_field_list,
                    output_fiel=out_field_list,
                    corr_list=this_corr_list,
                    tag_table=this_tag_table,
                ...
                this_cs = standardisation.NameStandardiser(
                    input_fi=in_field_list,
                    output_fiel=out_field_list,
                    corr_list=this_corr_list,
                    tag_table=this_tag_table,
        ...
        self.writeStatusBar(
            "Standardised data set with %d records." % (ids.num_records)
        )
    ...
    def runLinkCode(self):
        ...
        if (
            (self.febrl_code["data"] == None)
            or (self.febrl_code["indexing"] == None)
            or (self.febrl_code["comparison"] == None)
            or (self.febrl_code["classification"] == None)
            or (self.febrl_code["output"] == None)
        ):
            self.messageDialog(
                "Cannot run linkage or deduplication\n"
        ...
        if self.re_run["data_init"] == True:  # Was modified, so have to re-do
            ...
            if self.data_set_type_list[0] in ["CSV", "TAB"]:
                ...
                for field_num in range(len(data_set_info["field_names"])):
                    field_val_list.append(
                        (data_set_info["field_names"][field_num], field_num)
                    )

                ds1 = dataset.DataSetCSV(
                    access_mode="read",
                    file_name=data_set_info["file_name"],
                ...
                # Get width of the columns from GUI
                #
                col_width_vals = self.mainTree.get_widget(
                    "col_width_entry_a"
                ).get_text()
                col_width_vals = col_width_vals.strip()
                col_width_list = col_width_vals.split(",")
            ...
            self.writeStatusBar(
                "Initalised first data set with %d records." % (ds1.num_records)
            )
            ...
            if self.project_type != "Link":
                ...
                self.writeStatusBar(
                    "Initalised second data set with %d records." % (ds2.num_records)
                )
        ...
        if self.re_run["w_vec_generate"] == True:  # Re-generate weight vectors
            ...
            for comp_funct_dict in self.field_comp_list:
                ...
                descr_str = (
                    comp_funct_dict["name"] + "-" + field_a_name + "-" + field_b_name
                )

                if comp_funct_dict["name"] == "Str-Exact":
                    cf = comparison.FieldComparatorExactString(
                        do_caching=cache_field,
                    ...
                    cf = comparison.FieldComparatorContainsString(
                        do_cachi=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                        agree_weight=agree_weight,
                    ...
                    cf = comparison.FieldComparatorTruncateString(
                        do_cachi=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorKeyDiff(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorNumericPerc(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorNumericAbs(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorEncodeString(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                ...
                # elif (comp_name == 'FieldComparatorDistance'):  # TODO ###############
                    ...
                    cf = comparison.FieldComparatorDate(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                        ...
                        max_day1_before_day2=int(
                            comp_funct_dict["max_day1_before_day2"]
                        ),
                        max_day2_before_day1=int(
                            comp_funct_dict["max_day2_before_day1"]
                        ),
                    )

                elif comp_funct_name == "Time":
                    cf = comparison.FieldComparatorTime(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                        ...
                        max_time1_before_time2=int(
                            comp_funct_dict["max_time1_before_time2"]
                        ),
                        max_time2_before_time1=int(
                            comp_funct_dict["max_time2_before_time1"]
                        ),
                    )

                elif comp_funct_name == "Age":
                    cf = comparison.FieldComparatorAge(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorJaro(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorWinkler(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorQGram(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorPosQGram(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorSGram(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorEditDist(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorDaLeDist(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorBagDist(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorSWDist(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorSyllAlDist(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorSeqMatch(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorEditex(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorLCS(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorOntoLCS(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorTokenSet(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    cf = comparison.FieldComparatorCompress(
                        do_caching=cache_field,
                        description=descr_str,
                        max_cache_size=max_cache_size,
                        missing_weight=miss_weight,
                    ...
                    raise Exception(
                        "Illegal field comparison method: %s" % (comp_funct_name)
                    )
            ...
            self.writeStatusBar(
                "Initalised field comparisons and record " + "comparator."
            )
                ...
                for index_def_tuple in index_list:
                    ...
                    if encode_funct in [None, "None"]:
                        ...
                        if encode_funct == "Soundex":
                            ...
                            raise Exception(
                                "Illegal phonetic encoding function: %s"
                                % (encode_funct)
                            )
            ...
            if index_method_name == "FullIndex":
                ...
                if (self.project_type == "Link") and (
                    ...
                    index = indexing.BigMatchIndex(
                        ...
                        weight_vec_file=w_vec_file,
                        index_def=index_def_list,
                        block_method=("qgram", q, padded, threshold),
                    )
                ...
                p1, p2 = self.index_method["canopy_method"][2].split(
                    ","
                )  # Get parameters
                ...
                canopy_method = (
                    self.index_method["canopy_method"][0],
                    self.index_method["canopy_method"][1],
                    p1,
                    p2,
                )

                index = indexing.CanopyIndex(
                    dataset1=ds1,
                    dataset2=ds2,
                    log_funct=self.update_progress_bar,
                    rec_comparator=rec_comp,
                ...
                p1, p2 = self.index_method["canopy_method"][1].split(
                    ","
                )  # Get parameters
                ...
                sim_funct = {
                    ...
                    "Q-Gram": stringcmp.qgram,
                    "Pos-Q-Gram": stringcmp.posqgram,
                    "S-Gram": stringcmp.sgram,
                    "Edit-Dist": stringcmp.editdist,
                    ...
                    "Onto-LCS": stringcmp.ontolcs,
                    "Editex": stringcmp.editex,
                    "Perm-Winkler": stringcmp.permwinkler,
                    "Sort-Winkler": stringcmp.sortwinkler,
                }[sim_funct_name]

                index = indexing.StringMapIndex(
                    dataset1=ds1,
                    dataset2=ds2,
                    log_funct=self.update_progress_bar,
                    rec_comparator=rec_comp,
                ...
                padded = self.index_method["padded"]
                block_method = (
                    int(self.index_method["block_method"][0]),
                    int(self.index_method["block_method"][1]),
                )

                index = indexing.SuffixArrayIndex(
                    dataset1=ds1,
                    dataset2=ds2,
                    log_funct=self.update_progress_bar,
                    rec_comparator=rec_comp,
        ...
        if classifier_method_name == "FellegiSunter":
            ...
            classifier = classification.FellegiSunter(
                lower_threshold=lower_thres, upper_threshold=upper_thres
            )
            ...
            classifier = classification.OptimalThreshold(
                min_method=min_method, bin_width=bin_width
            )
            ...
            dist_meas = {
                "Manhatten": mymath.distL1,
                "Euclidean": mymath.distL2,
                "L-Infinity": mymath.distLInf,
                "Canberra": mymath.distCanberra,
            }[dist_meas_name]
            ...
            classifier = classification.SuppVecMachine(
                C=C, kernel_type=kernel_type, sample=sample
            )

        elif classifier_method_name == "TwoStep":  # - - - - - - - - - - - - - -
            s1_m_method = (
                float(self.classifier_method["s1_match_method"][0]),
                self.classifier_method["s1_match_method"][1],
            )
            if s1_m_method[-1] == "nearest":
                s1_m_method += (
                    int(self.classifier_method["s1_match_method"][2]),
            ...
            s1_nm_method = (
                float(self.classifier_method["s1_non_match_method"][0]),
                self.classifier_method["s1_non_match_method"][1],
            )
            if s1_nm_method[-1] == "nearest":
                s1_nm_method += (
                    int(self.classifier_method["s1_non_match_method"][2]),
            ...
            if self.classifier_method["s2_classifier"][0] == "kmeans":
                ...
                s2_classifier = (
                    self.classifier_method["s2_classifier"][0],
                    dist_meas,
                    max_iter_count,
                )
            else:  # SVM
            ...
            raise Exception("Illegal classifier method: %s" % (classifier_method_name))
        ...
        if self.re_run["w_vec_generate"] == True:  # Re-generate weight vectors
            ...
            if output_dict["w_vec_file"][0] == True:
                ...
                start_time = time.time()
                [field_names_list, w_vec_dict] = output.LoadWeightVectorFile(
                    w_vec_file_name
                )
                ...
                [field_names_list, w_vec_dict] = index.run(
                    length_filter_perc, cut_off_threshold
                )
            end_time_str = auxiliary.time_string(time.time() - start_time)
            self.writeStatusBar("Run record pair comparisons in %s." % (end_time_str))

            self.close_progress_bar()

            self.addToLog("")  # Add information on record pair comparison to log
            self.addToLog(
                "Compared %d record pairs in %s." % (len(w_vec_dict), end_time_str)
            )
        ...
        if classifier_method_name in ["OptimalThreshold", "SuppVecMachine"]:
            ...
            tm_set, tnm_set = classification.get_true_matches_nonmatches(
                w_vec_dict, match_status_funct
            )
            ...
            self.class_w_vec_dict = classification.extract_collapse_weight_vectors(
                manipulate_list, w_vec_dict
            )
        ...
        self.addToLog(
            "  Number of classified matches:          %d" % (len(self.result_sets[0]))
        )
        self.addToLog(
            "  Number of classified non-matches:      %d" % (len(self.result_sets[1]))
        )
        self.addToLog(
            "  Number of classified possible matches: %d" % (len(self.result_sets[2]))
        )
        self.addToLog("")
        ...
        if output_dict["histo_file"][0] == False:  # Don't save into file
            ...
            histo_list = output.GenerateHistogram(
                self.class_w_vec_dict, bin_width, histo_file_name
            )
        ...
        if output_dict["m_status_file"][0] == True:
            ...
            output.SaveMatchStatusFile(
                self.class_w_vec_dict, match_set, match_status_file_name
            )
            self.addToLog("Wrote match status into file: %s" % (match_status_file_name))
        ...
        self.writeStatusBar(
            "Classified weight vectors in %s, see Log page " % (end_time_str)
    ...
    def evaluateView(self):
        ...
        if len(self.class_w_vec_dict) == 0:
            self.messageDialog(
                "No weight vectors were generated,\nplease check "
        ...
        #
        if (evaluation_dict["available"] == False) or (sum(self.modified.values()) > 0):
            ...
            if self.match_status_funct == None:  # Cannot get true match status
                ...
                self.addToLog(
                    " True matches (TP):      %d" % (sum(w_tp_histo_dict.values()))
                )
                self.addToLog(
                    " False matches (FP):     %d" % (sum(w_fp_histo_dict.values()))
                )
                self.addToLog(
                    " True non-matches (TN):  %d" % (sum(w_tn_histo_dict.values()))
                )
                self.addToLog(
                    " False non-matches (FN): %d" % (sum(w_fn_histo_dict.values()))
                )
                self.addToLog("")
        ...
        if imp_matplot == True:
            ...
            if self.match_status_funct == None:
                ...
                #
                x_vals = list(evaluation_dict["class_m_weights"].keys())
                for x in list(evaluation_dict["class_nm_weights"].keys()) + list(
                ...
                p2 = histo_axis.bar(
                    x_vals, nm_y_vals, self.bar_width, color="b", bottom=m_y_vals
                )
                ...
                if len(evaluation_dict["class_pm_weights"]) > 0:
                    ...
                    histo_axis.legend(
                        (p1[0], p2[0]), ("Matches", "Non-matches"), shadow=False
                    )
                ...
                # Make a list of all x values over all sets
                #
                x_vals = list(evaluation_dict["class_tp_weights"].keys())
                for x in (
                    list(evaluation_dict["class_fn_weights"].keys())
                    + list(evaluation_dict["class_fp_weights"].keys())
                    + list(evaluation_dict["class_tn_weights"].keys())
                ):
                    if x not in x_vals:
                        x_vals.append(x)
                x_vals.sort()
                ...
                p2 = histo_axis.bar(
                    x_vals, fn_y_vals, self.bar_width, color="g", bottom=tp_y_vals
                )
                ...
                p3 = histo_axis.bar(
                    x_vals, fp_y_vals, self.bar_width, color="y", bottom=fp_bottom_vals
                )
                p4 = histo_axis.bar(
                    x_vals, tn_y_vals, self.bar_width, color="b", bottom=tn_bottom_vals
                )
            ...
            #
            histo_canvas = matplotlib.backends.backend_gtk.FigureCanvasGTK(histo_figure)
        ...
        #
        evaluation_dict["rr"] = measurements.reduction_ratio(class_w_vec_dict, ds1, ds2)

        if self.match_status_funct != None:
            w_vec_dict = self.w_vec_dict
            evaluation_dict["pq"] = measurements.pairs_quality(
                w_vec_dict, m_status_funct
            )
            ...
            acc, prec, reca, fmeas = measurements.quality_measures(
                w_vec_dict, m_set, nm_set, m_status_funct
            )
        ...
        for measure in ["acc", "prec", "reca", "fmeas", "rr", "pc", "pq"]:
            measure_label_widget = self.mainTree.get_widget("%s_val_label" % (measure))
        ...
        if imp_matplot == True:
            ...
            matplot_label = gtk.Label(
                "Matplotlib not available - sorry no "
    ...
    def logView(self):  # A switch to the Log page ------------------------------
        ...
        #
        mark = self.log_page_buffer.create_mark(
            "end", self.log_page_buffer.get_end_iter(), False
        )
