  ...
  """
    ...
    """
            ...
            if keyword.startswith("desc"):
                ...
                if value == None:
                    ...
                    if (value < 1) or (value > 50):
                        logging.exception(
                            "Illegal value for progress report, must be "
                            + 'False or between 1 and 50: "%s"' % (str(value))
                        )
                        raise Exception
                    self.progress_report = value
        ...
        if self.rec_comparator.dataset1 != self.dataset1:
            logging.exception(
                "Data set 1 in record comparator is different from"
        ...
        if self.rec_comparator.dataset2 != self.dataset2:
            logging.exception(
                "Data set 2 in record comparator is different from"
        ...
        for index_def_list in self.index_def:
            ...
            auxiliary.check_is_list(
                'Index definition list "%s"' % (str(index_def_list)), index_def_list
            )
            ...
            for index_def in index_def_list:
                ...
                auxiliary.check_is_list(
                    'Index definition "%s"' % (str(index_def)), index_def
                )
                if index_def == []:
                    logging.info(
                        "Empty index definition given: %s" % (str(self.index_def))
                    )
                ...
                if field_name1 not in dataset1_field_names:
                    logging.exception(
                        'Field "%s" is not in data set 1 field name ' % (field_name1)
                        + "list: %s" % (str(self.dataset1.field_list))
                    )
                    raise Exception
                ...
                if field_name2 not in dataset2_field_names:
                    logging.exception(
                        'Field "%s" is not in data set 2 field name ' % (field_name2)
                        + "list: %s" % (str(self.dataset2.field_list))
                    )
                    raise Exception
                ...
                if (index_def[5] != None) and (len(index_def[5]) > 0):
                    ...
                    auxiliary.check_is_function_or_method(
                        'Function "%s"' % (index_funct_def[0]), index_funct_def[0]
                    )
    ...
    """
        ...
        #
        build_list = [
            (self.index1, self.rec_cache1, self.dataset1, self.comp_field_used1, 0)
        ]  # For data set 1

        if self.do_deduplication == False:  # If linkage append data set 2
            build_list.append(
                (self.index2, self.rec_cache2, self.dataset2, self.comp_field_used2, 1)
            )
        ...
        for (index, rec_cache, dataset, comp_field_used_list, ds_index) in build_list:
            ...
            if self.progress_report != None:
                progress_report_cnt = max(
                    1, int(dataset.num_records / (100.0 / self.progress_report))
                )
            ...
            for (rec_ident, rec) in dataset.readall():  # Read all records in data set
                ...
                if (rec_read % progress_report_cnt) == 0:
                    self.__log_build_progress__(
                        rec_read, dataset.num_records, start_time
                    )

            used_sec_str = auxiliary.time_string(time.time() - start_time)
            rec_time_str = auxiliary.time_string(
                (time.time() - start_time) / dataset.num_records
            )
            logging.info(
                "Read and indexed %d records in %s (%s per record)"
    ...
    """
        ...
        for i in range(in_len - 1, min_len - 1, -1):
    ...
    def __compare_rec_pairs_from_dict__(
        self, length_filter_perc=None, cut_off_threshold=None
    ):
    ...
    """
        ...
        if self.weight_vec_file != None:
            try:
                ...
                logging.exception(
                    "Cannot write weight vector file: %s" % (self.weight_vec_file)
                )
                raise Exception
        ...
        if self.progress_report != None:
            progress_report_cnt = max(
                1, int(self.num_rec_pairs / (100.0 / self.progress_report))
            )
        else:  # So no progress report is being logged
        ...
        if length_filter_perc != None:
            auxiliary.check_is_percentage(
                "Length filter percentage", length_filter_perc
            )
        ...
        for rec_ident1 in rec_pair_dict:
            ...
            for rec_ident2 in rec_pair_dict[rec_ident1]:
                ...
                if length_filter_perc != None:
                    ...
                    perc_diff = float(abs(rec1_len - rec2_len)) / max(
                        rec1_len, rec2_len
                    )
        ...
        used_sec_str = auxiliary.time_string(time.time() - start_time)
        rec_time_str = auxiliary.time_string(
            (time.time() - start_time) / self.num_rec_pairs
        )
        logging.info(
            "Compared %d record pairs in %s (%s per pair)"
            % (self.num_rec_pairs, used_sec_str, rec_time_str)
        )
        if length_filter_perc != None:
            logging.info(
                "  Length filtering (set to %.1f%%) filtered %d record "
                % (length_filter_perc * 100, num_rec_pairs_filtered)
                + "pairs"
            )
        if cut_off_threshold != None:
            logging.info(
                "  %d record pairs had summed weights below threshold "
                % (num_rec_pairs_below_thres)
                + "%.2f" % (cut_off_threshold)
            )
                ...
                if field_val != "":  # Field value is not empty
                    ...
                    if funct_def != None:  # There is a function defined for this index
                        ...
                        if num_funct_arg == 1:  # No arguments
                            ...
                            funct_val = funct_call(
                                field_val, funct_def[1], funct_def[2]
                            )
                        elif num_funct_arg == 4:  # Three arguments
                            funct_val = funct_call(
                                field_val, funct_def[1], funct_def[2], funct_def[3]
                            )
                        else:
                            logging.exception(
    ...
    """
        ...
        log_str = "Read and indexed %d of %d records (%d%%) in %s (%s per" % (
            ...
            used_sec_str,
            rec_sec_str,
        ) + " record), estimated %s until finished." % (togo_sec_str)
    ...
    """
        ...
        log_str = "Compared %d of %d record pairs (%d%%) in %s (%s per " % (
            ...
            used_sec_str,
            rec_sec_str,
        ) + "pair), estimated %s until finished." % (togo_sec_str)
        logging.info(log_str)
    ...
    """
        ...
        logging.info("  Record comparator:      %s" % (self.rec_comparator.description))
        logging.info(
            "    Used fields indices from data set 1: %s" % (str(self.comp_field_used1))
        )
        logging.info(
            "    Used fields indices from data set 2: %s" % (str(self.comp_field_used2))
        )
        ...
        if self.weight_vec_file != None:
            logging.info(
                "  Weight vectors will be written into: %s" % (self.weight_vec_file)
            )
        ...
        if self.index_def != []:  # Index definitions are not empty - - - - - - -
            ...
            for i in range(len(self.index_def)):
                ...
                for j in range(len(self.index_def[i])):
                    ...
                    logging.info(
                        '      Data set 1 and 2 field names: "%s" (at column'
                        % (index_def[0])
                        + ' %d) and "%s" (at column %d)'
                        % (index_def_proc[0], index_def[1], index_def_proc[1])
                    )
                    logging.info(
                        "        Sort words flag set to %s" % (str(index_def[2]))
                    )
                    logging.info("        Reverse flag set to %s" % (str(index_def[3])))

                    if index_def_proc[4] == None:
                        logging.info(
                            "        No maximum length for index variable "
                            + "specified"
                        )
                    else:
                        logging.info(
                            "        Maximum length for index variable: %d"
                            % (index_def_proc[4])
                        )
...
class FullIndex(Indexing):
  ...
  """
    ...
    """
        ...
        for (rec_ident, rec_list) in self.small_dataset.readall():
            if rec_ident in small_data_set_dict:
                logging.exception(
                    'Duplicate record identifier "%s" in small data ' % (rec_ident)
                    + "set"
                )
        ...
        logging.info(
            "Built full index in %s" % (auxiliary.time_string(time.time() - start_time))
        )
    ...
    """
        ...
        logging.info("Started comparison of %d record pairs" % (self.num_rec_pairs))
        if self.log_funct != None:
            self.log_funct(
                "Started comparison of %d record pairs" % (self.num_rec_pairs)
            )
        ...
        if self.status != "compacted":  # Not necessary, but keep code consistent
            logging.exception(
                'Index "%s" has not been compacted, running ' % (self.description)
        ...
        if self.do_deduplication == True:  # A deduplication run - - - - - - - -
            ...
            #
            small_data_set_rec_id_list = list(small_data_set_dict.keys())
            ...
            for rec_ident1 in small_data_set_rec_id_list:
                ...
                for rec_ident2 in small_data_set_rec_id_list[rec_cnt:]:
                    ...
                    rec2 = small_data_set_dict[
                        rec_ident2
                    ]  # Get values of second record
  ...
  """
    ...
    """
        ...
        for i in range(num_indices):
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                logging.info(
                    "  Index %d for data set 1 contains %d blocks"
                    % (i, len(self.index1[i]))
                )
                ...
                logging.info(
                    "  Index %d for data set 1 contains %d blocks, and "
                    % (i, len(self.index1[i]))
                    + "for data set 2 contains %d" % (len(self.index2[i]))
                    + " blocks"
                )
                ...
                for block_val in self.index1[i]:  # Loop over all block values in index
                    ...
                    if (
                        block_val in self.index2[i]
                    ):  # Blocking values is both data sets'
                        # index
        ...
        logging.info(
            "Built blocking index in %s"
            % (auxiliary.time_string(time.time() - start_time))
        )

        logging.info("  Number of record pairs: %d" % (self.num_rec_pairs))
        logging.info(
            '  Largest block with value "%s" contains %d records'
            % (largest_block_val, largest_block_num_rec)
            + " (in index %d)" % (largest_block_index_num)
        )
    ...
    """
        ...
        if self.status != "built":
            logging.exception(
                'Index "%s" has not been built, compacting is not ' % (self.description)
                + "possible"
            )
            raise Exception
        ...
        for i in range(num_indices):
            ...
            logging.info(
                "  Compacted blocking index %d in %s"
    ...
    """
        ...
        # Compare the records
        #
        return self.__compare_rec_pairs_from_dict__(
    ...
    """
        ...
        for i in range(num_indices):
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                block_val_list = list(this_index.keys())  # Get all blocking values
                block_val_list.sort()

                num_block_vals = len(block_val_list)

                for j in range(num_block_vals):  # Loop over all blocks
                ...
                w_block_len1 = [
                    0
                ] * w  # Counts of number of records in blocks in window
                w_block_len2 = [0] * w

                curr_window_recs1 = []  # List of record identifiers in current window
                curr_window_recs2 = []

                block_val_list1 = list(this_index1.keys())  # Get all blocking values
                block_val_list1.sort()

                block_val_list2 = list(this_index2.keys())
                ...
                for block_val in comb_block_values:
                    ...
                    link_rec_pair_funct(
                        curr_window_recs1, curr_window_recs2, rec_pair_dict
                    )
            ...
            logging.info(
                "  Compacted sorting index %d in %s"
                % (i, auxiliary.time_string(time.time() - istart_time))
            )
    ...
    """
        ...
        for i in range(num_indices):
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                #
                block_val_set = set(list(this_index1.keys()) + list(this_index2.keys()))
                ...
                #
                assert len(rec_sorted_array) <= (
                    self.dataset1.num_records + self.dataset2.num_records
                ), (
                    len(rec_sorted_array),
                    (self.dataset1.num_records + self.dataset2.num_records),
                )
...
class AdaptSortingIndex(Indexing):
  ...
  """
    ...
    """
        ...
        logging.info("")
        logging.info('Build adaptive sorted index: "%s"' % (self.description))
        ...
        logging.info(
            "Built adaptive sorted index in %s"
            % (auxiliary.time_string(time.time() - start_time))
        )
    ...
    """
        ...
        logging.info("")
        logging.info('Compact adaptive sorted index: "%s"' % (self.description))
        ...
        for i in range(num_indices):
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                index_val_list = list(
                    this_index.keys()
                )  # Sort all unique index key values
                index_val_list.sort()
                ...
                while block_start_index < num_index_vals:
                    ...
                    w = (
                        1
                    )  # To make sure this works even with one single index key value
                    w_last = w_first + w
                    ...
                    if tmp_pos + 1 == num_index_vals:
                        ...
                        while (
                            str_cmp_funct(first_val, second_val) > str_cmp_thres
                        ) and ((tmp_pos + 1) < num_index_vals):
                            tmp_pos += 1
                    ...
                    for this_val in index_val_list[block_start_index:block_end_index]:
                        ...
                        curr_win_record_set = curr_win_record_set.union(
                            set(curr_rec_id_list)
                        )
                ...
                #
                index_val_set = set(list(this_index1.keys()) + list(this_index2.keys()))
                ...
                while block_start_index < num_index_vals:
                    ...
                    for this_val in index_val_list[block_start_index:block_end_index]:
                        ...
                        if this_val in this_index1:
                            ...
                            curr_win_record_set1 = curr_win_record_set1.union(
                                set(curr_rec_id_list1)
                            )

                        if this_val in this_index2:
                            curr_rec_id_list2 = this_index2[this_val]

                            curr_win_record_set2 = curr_win_record_set2.union(
                                set(curr_rec_id_list2)
                            )
                    ...
                    if (len(curr_win_record_set1) > 0) and (
                        ...
                        link_rec_pair_funct(
                            list(curr_win_record_set1),
                            list(curr_win_record_set2),
                            rec_pair_dict,
                        )
...
class QGramIndex(Indexing):
  ...
  """
    ...
    """
        ...
        self.log(
            [("Threshold", self.threshold), ("q", self.q), ("Padded flag", self.padded)]
        )  # Log a message
    ...
    """
        ...
        for i in range(num_indices):
            ...
            for (basic_index, qgram_index, ds_index) in index_list:
                ...
                for index_val in basic_index:  # Loop over all the values in this index
                    ...
                    #
                    qgram_list = [
                        qgram_str[j : j + q] for j in range(len(qgram_str) - (q - 1))
                    ]
                ...
                logging.info(
                    "  Built %d-gram index %d for data set %d in %s"
                    % (
                        q,
                        i,
                        ds_index + 1,
        ...
        logging.info(
            "Built %d-gram index in %s"
            % (q, auxiliary.time_string(time.time() - start_time))
        )

        logging.info(
            "  Number of basic index blocks (number of different "
            + "index variable values): %d" % (num_blocks)
        )
        logging.info(
            "  Number of %d-gram index blocks (number of different " % (q)
            + "%d-gram index values):  %d" % (q, num_qgram_blocks)
        )
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                for qgram_val in this_qgram_index:  # Loop over all q-gram values
                    ...
                    if (num_qgram_blocks_done % NUM_QGRAM_BLOCK_PROGRESS_REPORT) == 0:
                        logging.info(
                            "    Processed %d of %d %d-gram blocks"
                            % (num_qgram_blocks_done, len(this_qgram_index), self.q)
                        )
                        memory_usage_str = auxiliary.get_memory_usage()
                        if memory_usage_str != None:
                            logging.info("      " + memory_usage_str)
                ...
                for qgram_val in this_qgram_index1:  # Loop over all q-gram values
                    ...
                    if (num_qgram_blocks_done % NUM_QGRAM_BLOCK_PROGRESS_REPORT) == 0:
                        logging.info(
                            "    Processed %d of %d %d-gram blocks"
                            % (num_qgram_blocks_done, len(self.qgram_index1[i]), self.q)
                        )
                        memory_usage_str = auxiliary.get_memory_usage()
                        if memory_usage_str != None:
                            logging.info("      " + memory_usage_str)
  ...
  """
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("canopy_m"):
                ...
                if value == False:
                    logging.exception(
                        'Value of argument"skip_missing" cannot be set'
        ...
        if len(self.canopy_method) != 4:
            logging.exception(
                "Canopy method tuple needs four elements: %s"
        ...
        if self.canopy_method[0] not in ["tfidf", "jaccard"]:
            logging.exception(
                'Illegal canopy method given (must be "tfidf" or '
        ...
        if self.canopy_method[1] not in ["threshold", "nearest"]:
            logging.exception(
                "Illegal canopy retrieval method given (must be "
                + '"threshold" or "nearest"): %s' % (str(self.canopy_method))
            )
            raise Exception
        ...
        if self.canopy_method[1] == "threshold":
            ...
            if self.canopy_method[2] < self.canopy_method[3]:
                logging.exception(
                    "Tight threshold is smaller than loose threshold:"
                    + "%.3f / %.3f" % (self.canopy_method[2], self.canopy_method[3])
                )
                raise Exception
            ...
            if self.canopy_method[2] > self.canopy_method[3]:
                logging.exception(
                    "Remove nearest is larger than cluster nearest:"
                    + "%d / %d" % (self.canopy_method[2], self.canopy_method[3])
                )
                raise Exception
        ...
        self.log(
            [
                ...
                ("Delete percentage", self.delete_perc),
            ]
        )  # Log a message
    ...
    """
        ...
        if do_dedup == False:  # If linkage append data set 2
            build_list.append(
                (self.dataset2, self.rec_cache2, self.comp_field_used2, 1)
            )
        ...
        for (dataset, rec_cache, comp_field_used, ds_index) in build_list:
            ...
            for (rec_ident, rec) in dataset.readall():  # Read all records in data set
                ...
                if (rec_read % progress_report_cnt) == 0:
                    self.__log_build_progress__(
                        rec_read, dataset.num_records, rstart_time
                    )
            ...
            rec_time_str = auxiliary.time_string(
                (time.time() - rstart_time) / dataset.num_records
            )
            logging.info(
                "Read and indexed %d records in %s (%s per record)"
                % (dataset.num_records, used_sec_str, rec_time_str)
            ...
            for i in range(num_indices):
                if max_qgram_count[i] > -1:  # Some q-grams occured more than once
                    logging.info(
                        '    Index %d: %d-gram "%s" with count %d in value '
                        % (i, self.q, max_qgram_count_val[i][0], max_qgram_count[i])
                        + '"%s"' % (max_qgram_count_val[i][1])
                    )
                else:
                    logging.info(
                        "    Index %d: No %d-gram appeared more than once" % (i, self.q)
                    )
        ...
        if canopy_method_tfidf == True:  # Only needed for TF-IDF
            ...
            for i in range(num_indices):  # Put record identifier into all indices
                ...
                logging.info(
                    "  Calculate %d-gram IDF and Euclidean length (weight" % (self.q)
                    + " W_d) for index %d" % (i)
                )
                    ...
                    if index_val not in index_val_Wd_cache:
                        ...
                        for (qgram, qgram_count) in qgram_dict.items():
                            ...
                            if qgram not in qgram_inv_doc_freq_cache[i]:
                                ...
                                #
                                for (tmp_rec_id, tmp_rec_co) in this_index[
                                    qgram
                                ].items():
                ...
                logging.info(
                    "    Number of different index values: %d"
        ...
        for i in range(num_indices):
            logging.info(
                "  Index %d contains %d different %d-grams" % (i, len(index[i]), self.q)
            )
            max_len = -1
            max_val = None
            delete_qgram_list = []  # List of common q-grams to be deleted

            for (qgram, qgram_recs) in index[i].items():
            ...
            logging.info(
                '    Most common %d-gram: "%s" in %d of %d records'
            ...
            if delete_qgram_list != []:
                logging.info(
                    "    Deleted the following %d-grams (as " % (self.q)
    ...
    """
        ...
        qgram_list = [qgram_str[j : j + q] for j in range(len(qgram_str) - q1)]
    ...
    """
        ...
        for i in range(num_indices):
            ...
            logging.info(
                "  Compacting index %d containing %d records and %d "
                % (i, total_num_rec, len(self.index1[i]))
                + "%d-grams" % (self.q)
            )
            ...
            while len(this_index_val_cache) > 0:
                ...
                #
                if do_tfidf == True:
                    canopy_recs = tfidf_canopy_funct(
                        self.index1[i],
                        index_val,
                    ...
                    canopy_recs = jaccard_canopy_funct(
                        self.index1[i],
                        index_val,
                        this_index_val_cache,
                        self.index_val_num_qgram[i],
                ...
                if (num_canopies % NUM_CANOPY_PROGRESS_REPORT) == 0:
                    logging.info(
                        "    Created %d canopies; %d records and "
                        % (num_canopies, len(self.index_val_cache[i]))
                        + "%d %d-grams" % (len(self.index1[i]), self.q)
                        + " left"
                    )
                    memory_usage_str = auxiliary.get_memory_usage()
                    if memory_usage_str != None:
            ...
            logging.info(
                "  Compacted canopy index %d in %s"
                % (i, auxiliary.time_string(time.time() - istart_time))
            )
            logging.info("    Produced %d canopies" % (num_canopies))
            logging.info(
                "      Smallest canopy with %d records and center "
                % (smallest_canopy_size)
                + 'index value: "%s"' % (smallest_canopy_center)
            )
            logging.info(
                "      Largest canopy with %d records and center "
                % (largest_canopy_size)
                + 'index value: "%s"' % (largest_canopy_center)
            )
    ...
    def __tfidf_canopy__(
        self,
        index,
        index_val,
        index_val_cache,
        qgram_inv_doc_freq_cache,
        max_qgram_count,
    ):
        ...
        if self.canopy_method[1] == "threshold":
            ...
            for sim_val in sim_values:
                ...
                if (len(return_list) + len(sim_val_rec_list)) <= cluster_nearest:
                    ...
                    if (
                        (len(delete_list) + len(sim_val_rec_list)) <= remove_nearest
                    ) or (
                        delete_list == []
                    ):  # Make sure delete is not empty
    ...
    def __jaccard_canopy__(
        self, index, index_val, index_val_cache, index_val_num_qgram
    ):
        """Returns a list of record identifiers in the given inverted index
    ...
    """
        ...
        if self.canopy_method[1] == "threshold":
            ...
            phase_switch_threshold = max(
                1, 1 + int((1.0 - cluster_threshold) * num_qgrams)
            )
            ...
            if i < phase_switch_threshold:  # Consider new record identifers
                ...
                if do_threshold == False:
                    ...
                    if (i + 1) > len(qgram_len_list) / 2:
                        ...
                        #
                        if (
                            qgram_count_dict[i + 1] > cluster_nearest
                        ):  ### PC 23/01: i->i+1
        ...
        if do_threshold == False:  # Extract nearest from similariy dictionary -
            ...
            sim_values = list(jacc_sim_dict.keys())
            sim_values.sort(reverse=True)  # Largest values first
  ...
  """
    ...
    """
        ...
        if self.sub_dim > self.dim:
            logging.exception(
                'Argument "sub_dim" is larger than "dim": %d / %d'
        ...
        if self.grid_resolution not in [10, 100, 1000, 10000]:
            logging.exception(
                'Argument "grid_resolution" is not a power of 10 '
        ...
        if len(self.canopy_method) != 3:
            logging.exception(
                "Canopy method tuple needs three elements: %s"
                % (str(self.canopy_method))
            )
            raise Exception
        ...
        if self.canopy_method[0] == "threshold":
            ...
            if self.canopy_method[1] < self.canopy_method[2]:
                logging.exception(
                    "Tight threshold is smaller than loose threshold:"
                    + "%.3f / %.3f" % (self.canopy_method[1], self.canopy_method[2])
                )
                raise Exception
            ...
            if self.canopy_method[1] > self.canopy_method[2]:
                logging.exception(
                    "Remove nearest is larger than cluster nearest:"
                    + "%d / %d" % (self.canopy_method[1], self.canopy_method[2])
                )
                raise Exception
        ...
        self.log(
            [
                ...
                ("Canopy method", self.canopy_method),
                ("Similarity function", self.sim_funct),
            ]
        )  # Log a message
    ...
    """
        ...
        for i in range(self.m):
            ...
            for j in range(num_string):  # Calculate distance to all other strings
    ...
    """
        ...
        for i in range(num_indices):
            ...
            #
            string_list = list(self.index1[i].keys())  # From data set 1
            ...
            logging.info(
                "  Map index %d containing %d string values into a " % (i, num_string)
                + "%d-dimensional space" % (dim)
            )
            ...
            #
            for h in range(dim):  # Loop over dimensions
            ...
            for h in range(dim):  # One dictionary per dimension
                index_grid[h] = {}
            ...
            logging.info(
                "  Built string-map index %d with %d strings values " % (i, num_string)
                + "in %s" % (auxiliary.time_string(time.time() - istart_time))
            )
            logging.info(
                "    Number of distance calculations done: %d" % (self.num_dist_calc)
            )

            if self.cache_dist == True:
                logging.info(
                    "    Length of cache: %d (in average %.2f distance "
                    % (
    ...
    """
        ...
        for i in range(num_indices):
            ...
            str_val_list = list(str_coord_dict.keys())  # List of the string values only
            ...
            logging.info(
                "  Compacting index %d containing %d strings" % (i, total_num_str)
            )
            ...
            while len(str_val_list) > 0:
                ...
                while (got_enough_records_for_canopy == False) and (
                    ...
                    for h in range(dim):  # Loop over dimensions
                        ...
                        #
                        previous_cell_val = (
                            this_cell_val - neighbour_offset * interval_size
                        )
                        ...
                        if (h % sub_dim) == 0:  # Start new candidate set
                            ...
                            if ((h + 1) % sub_dim) == 0:
                                ...
                                #
                                final_candidate_set = final_candidate_set.union(
                                    candidate_set
                                )
                                del candidate_set
                                candidate_set = set()
                    ...
                    for str_candidate_val in final_candidate_set:
                        ...
                        if (neighbour_offset > 1) and (
                            ...
                            for h in range(dim):  # Loop over dimensions
                                dim_diff = abs(
                                    str_candidate_coord[h] - center_str_coord[h]
                                )
                    ...
                    if num_same_str_loc > 0:
                        logging.warning(
                            "%d string pairs that differ had Euclidean "
                            % (num_same_str_loc)
                            + "distance of 0.0 (set to"
                            + " a very small distance)"
                        )

                    dist_heap = list(dist_dict.keys())
                    ...
                    canopy_recs1 = (
                        []
                    )  # All record identifiers from data set 1 in canopy
                    canopy_recs2 = (
                        []
                    )  # Record identifiers from data set 2, linkage only
                    ...
                    while dist_heap != []:
                        ...
                        smallest_dist = heapq.heappop(
                            dist_heap
                        )  # Smallest distance value
                        smallest_str_list = dist_dict[
                            smallest_dist
                        ]  # Its string values

                        this_str_val_recs1 = (
                            []
                        )  # All record identifiers for these strings
                        this_str_val_recs2 = []

                        for (
                            str_val
                        ) in smallest_str_list:  # Get record ident. of strings
                        ...
                        if do_nearest == True:
                            ...
                            comb_list_len = (
                                len(canopy_recs1)
                                + len(canopy_recs2)
                                + len(this_str_val_recs1)
                                + len(this_str_val_recs2)
                            )
                    ...
                    if (len(dist_heap) == 0) and (
                        (
                            (do_nearest == True)
                            and (
                                len(canopy_recs1) + len(canopy_recs2) < cluster_nearest
                            )
                        )
                        or ((do_nearest == False) and (smallest_dist < loose_threshold))
                    ):
                ...
                if (num_canopies % NUM_CANOPY_PROGRESS_REPORT) == 0:
                    logging.info(
                        "    Created %d canopies; %d strings left"
            ...
            logging.info("    Produced %d canopies" % (num_canopies))
            logging.info(
                "      Smallest canopy with %d strings and center "
                % (smallest_canopy_size)
                + 'index value: "%s"' % (smallest_canopy_center)
            )
            logging.info(
                "      Largest canopy with %d strings and center "
                % (largest_canopy_size)
                + 'index value: "%s"' % (largest_canopy_center)
            )
  ...
  """
    ...
    """
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("block_m"):
                ...
                if value not in ["suffixonly", "allsubstr"]:
                    logging.exception(
                        'Illegal value for "suffix_method": %s ' % (value)
                        + ' (has to be either "suffixonly" or'
                        + '"allsubstr")'
                    )
                    raise Exception
        ...
        if len(self.block_method) != 2:
            logging.exception(
                "Blocking method tuple needs two elements: %s"
    ...
    """
        ...
        for (index, rec_cache, dataset, comp_field_used_list, ds_index) in build_list:
            ...
            for (rec_ident, rec) in dataset.readall():  # Read all records in data set
                ...
                for i in range(num_indices):  # Put record identifier into all indices
                    ...
                    if (index_val != "") or (skip_missing == False):
                        ...
                        this_suffix_str_set.add(
                            index_val
                        )  # Even if shorter than min_len
            ...
            rec_time_str = auxiliary.time_string(
                (time.time() - istart_time) / dataset.num_records
            )
            logging.info(
                "Read and indexed %d records in %s (%s per record)"
                % (dataset.num_records, used_sec_str, rec_time_str)
        ...
        for i in range(num_indices):
            ...
            this_suff_array_strings = list(this_index.keys())
            ...
            if self.do_deduplication == True:
                logging.info(
                    "  Removed %d (of %d) strings from suffix array 1 as "
                    % (num_removed_1, num_original)
                    + "they only had one record"
                )
            logging.info(
                "  Removed %d (of %d) strings from suffix array 1 as "
                % (num_removed_large, num_original)
                + "they had more "
            ...
            logging.info(
                "  Suffix array in index %d for data set 1 contains %d "
                % (i, len(this_suff_array_strings))
                + "strings"
            )
            ...
            if self.do_deduplication == False:
                ...
                logging.info(
                    "  Removed %d (of %d) strings from suffix array 2 as "
                    % (num_removed_large, num_original)
                    + "they had more "
                    + "than %d (maximum block size) records" % (max_block_size)
                )
                ...
                logging.info(
                    "  Suffix array in index %d for data set 2 contains %d "
                    % (i, len(this_suff_array_strings))
                    + "strings"
                )

            logging.info(
                "    Longest string in this suffix array is %d characters"
                % (max_suff_str_len[i])
                + " long"
            )

        logging.info(
            "Built suffix array index in %s"
            % (auxiliary.time_string(time.time() - start_time))
        )
    ...
    """
        ...
        for i in range(num_indices):
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                if self.progress_report != None:
                    progress_report_cnt = max(
                        1, int(this_str_list_len / (100.0 / self.progress_report))
                    )
                else:  # So no progress report is being logged
                ...
                for s in this_str_list:
                    ...
                    if (num_strings_done % progress_report_cnt) == 0:
                        perc_done = int(
                            round(100.0 * num_strings_done / this_str_list_len)
                        )
                        logging.info(
                            "    Processed %d of %d (%d%%) strings"
                ...
                for s in this_str_list1:
                    ...
                    curr_str_val_records1 = this_index1[
                        s
                    ]  # Suffix array from data set 1
                    ...
                    if s in this_str_list2:  # String is in both suffix arrays
                        ...
                        largest_block = max(
                            largest_block, curr_block_size1 + curr_block_size2
                        )

                        link_rec_pair_funct(
                            curr_str_val_records1, curr_str_val_records2, rec_pair_dict
                        )
            ...
            logging.info(
                "  Compacted suffix array index %d in %s"
                % (i, auxiliary.time_string(time.time() - istart_time))
            )
        ...
        logging.info(
            "Compacted suffix array index in %s"
            % (auxiliary.time_string(time.time() - start_time))
        )
        logging.info("  Number of record pairs: %d" % (self.num_rec_pairs))
...
class RobustSuffixArrayIndex(Indexing):
  ...
  """
    ...
    """
        ...
        logging.info("")
        logging.info('Compact robust suffix array index: "%s"' % (self.description))
        ...
        for i in range(num_indices):
            ...
            if self.do_deduplication == True:  # A deduplication - - - - - - - - -
                ...
                while j < (this_str_list_len - 1):
                    ...
                    if (j + 1) == k:  # Case 1: No merger, simply copy into merged index
                        ...
                        for l in range(j + 1, k):
                            ...
                            merged_rec_set = merged_rec_set.union(
                                set(this_index[other_str])
                            )
                ...
                if self.progress_report != None:
                    progress_report_cnt = max(
                        1, int(merged_str_list_len / (100.0 / self.progress_report))
                    )
                else:  # So no progress report is being logged
                ...
                for s in merged_str_list:
                    ...
                    curr_str_val_records = merged_index[
                        s
                    ]  # Get records for this string
                    curr_block_size = len(curr_str_val_records)
                    largest_block = max(largest_block, curr_block_size)
                    ...
                    if (num_strings_done % progress_report_cnt) == 0:
                        perc_done = int(
                            round(100.0 * num_strings_done / merged_str_list_len)
                        )
                        logging.info(
                            "    Processed %d of %d (%d%%) strings"
                ...
                while j < (comb_str_list_len - 1):
                    ...
                    if (j + 1) == k:  # Case 1: No merger, simply copy into merged index
                        ...
                        if (len(merged_rec_set1) > 0) and (len(merged_rec_set2) > 0):
                            merged_index[merged_str] = (
                                list(merged_rec_set1),
...
class BigMatchIndex(Indexing):
  ...
  """
    ...
    """
        ...
        if self.do_deduplication == True:
            logging.exception(
                "BigMatchIndex can only be used for linkages, but "
                + "not deduplications"
            )
            raise Exception
        ...
        if self.block_method[0] == "block":
            if len(self.block_method) != 1:
                logging.exception(
                    'Blocking method "block" does not need parameters:'
            ...
            if len(self.block_method) != 2:
                logging.exception(
                    'Blocking method "sort" needs one parameter: %s'
                    % (str(self.block_method))
                )
                raise Exception
            ...
            if len(self.block_method) != 4:
                logging.exception(
                    'Blocking method "qgram" needs three parameters: '
                    + "%s" % (str(self.block_method))
                )
                raise Exception
            ...
            logging.exception(
                "Illegal blocking method given: %s" % (str(self.block_method))
            )
            raise Exception
        ...
        self.log(
            [
                ("Small data set", self.small_dataset.description),
    ...
    """
        ...
        if self.progress_report != None:
            progress_report_cnt = max(
                1, int(self.small_dataset.num_records / (100.0 / self.progress_report))
            )
        else:  # So no progress report is being logged
        ...
        for (rec_ident, rec) in self.small_dataset.readall():
            ...
            if (rec_read % progress_report_cnt) == 0:
                self.__log_build_progress__(
                    rec_read, self.small_dataset.num_records, start_time
                )
        ...
        if self.block_method[0] == "sort":
            ...
            for i in range(num_indices):
                this_index_vals = list(this_index[i].keys())
        ...
        #
            ...
            for i in range(num_indices):
                ...
                for index_val in basic_index:  # Loop over all the values in this index
                    ...
                    #
                    qgram_list = [
                        qgram_str[j : j + q] for j in range(len(qgram_str) - q1)
                    ]
                ...
                logging.info(
                    "  Built %d-gram index %d with %d blocks in %s"
                    % (
                        q,
                        i,
                        len(qgram_index),
        ...
        used_sec_str = auxiliary.time_string(time.time() - start_time)
        rec_time_str = auxiliary.time_string(
            (time.time() - start_time) / self.small_dataset.num_records
        )
        logging.info(
            "Read and indexed %d records in %s (%s per record)"
        ...
        logging.info(
            "Built BigMatch index containing %d blocks in %s"
            % (total_num_blocks, auxiliary.time_string(time.time() - start_time))
        )
    ...
    """
        ...
        if self.log_funct != None:
            self.log_funct(
                "Read large data set and start comparison of record " + "pairs"
            )
        ...
        if self.progress_report != None:
            progress_report_cnt = max(
                1, int(self.large_dataset.num_records / (100.0 / self.progress_report))
            )
        else:  # So no progress report is being logged
        ...
        for (large_rec_ident, large_rec) in self.large_dataset.readall():
            ...
            for i in range(num_indices):  # Put record identifier into all indices
                ...
                if (index_val != "") or (skip_missing == False):
                    ...
                    if block_method == "block":  # Simply get record identifiers
                        ...
                        #
                        list_index = find_closest_funct(
                            sorted_index_val_list[i], index_val
                        )
                        ...
                        win_end = min(
                            num_sorted_index_val[i], list_index + win_size_after + 1
                        )
                        ...
                        for sort_index_val in win_index_val_list:
                            ...
                            for small_rec_ident in this_index[i].get(
                                sort_index_val, []
                            ):
                        ...
                        logging.exception(
                            "Illegal blocking method given: %s"
                            % (str(self.block_method))
                        )
                        raise Exception
                    ...
                    for small_rec_ident in small_rec_ident_list:
                        ...
                        if small_rec_ident not in small_block_rec_set:
                            ...
                            if length_filter_perc != None:
                                if (
                                    small_rec_ident in rec_length_cache
                                ):  # Length is cached
                                ...
                                perc_diff = float(
                                    abs(large_rec_len - small_rec_len)
                                ) / max(large_rec_len, small_rec_len)

                                if perc_diff > length_filter_perc:
                                    do_comp = (
                                        False
                                    )  # Difference too large, don't do comparison
                                    num_rec_pairs_filtered += 1
                            ...
                            if do_comp == True:
                                ...
                                if small_data_set_no == 0:
                                    ...
                                    if (cut_off_threshold == None) or (
                                        ...
                                        if self.weight_vec_file == None:
                                            weight_vec_dict[
                                                (small_rec_ident, large_rec_ident)
                                            ] = w_vec
                                        else:
                                            weight_vec_writer.writerow(
                                                [small_rec_ident, large_rec_ident]
                                                + w_vec
                                            )
                                    else:
                                    ...
                                    if (cut_off_threshold == None) or (
                                        ...
                                        if self.weight_vec_file == None:
                                            weight_vec_dict[
                                                (large_rec_ident, small_rec_ident)
                                            ] = w_vec
                                        else:
                                            weight_vec_writer.writerow(
                                                [large_rec_ident, small_rec_ident]
            ...
            if (rec_read % progress_report_cnt) == 0:
                self.__log_build_progress__(
                    rec_read, self.large_dataset.num_records, start_time
                )
                logging.info(
                    "    Number of comparisons done so far: %d (%.1f in "
                    % (comp_done, float(comp_done) / rec_read)
                    + "average per "
        ...
        used_sec_str = auxiliary.time_string(time.time() - start_time)
        rec_read_time_str = auxiliary.time_string(
            (time.time() - start_time) / self.large_dataset.num_records
        )
        if comp_done > 0:
            rec_comp_time_str = auxiliary.time_string(
                (time.time() - start_time) / comp_done
            )
        else:
            rec_comp_time_str = 0
        logging.info(
            "Read %d records in %s (%s per record)"
            % (self.large_dataset.num_records, used_sec_str, rec_read_time_str)
        )
        logging.info(
            "  Compared %d record pairs in %s (%s per pair)"
...
class DedupIndex(Indexing):
  ...
  """
    ...
    """
        ...
        if self.do_deduplication == False:
            logging.exception(
                "DedupIndex can only be used for deduplications, " + "but not linkages"
            )
            raise Exception
    ...
    """
        ...
        if self.progress_report != None:
            progress_report_cnt = max(
                1, int(self.dataset1.num_records / (100.0 / self.progress_report))
            )
        else:  # So no progress report is being logged
        ...
        for (rec_ident1, rec1) in self.dataset1.readall():
            ...
            if rec_ident1 in rec_cache:
                logging.warn(
                    'Record with identifier "%s" appears more than once ' % (rec_ident1)
                    + "in data set"
                )
            ...
            for rec_ident2 in this_rec_block_rec_list:
                ...
                if length_filter_perc != None:
                    ...
                    perc_diff = float(abs(rec_len1 - rec_len2)) / max(
                        rec_len1, rec_len2
                    )
            ...
            if (rec_read % progress_report_cnt) == 0:
                self.__log_build_progress__(
                    rec_read, self.dataset1.num_records, start_time
                )
                logging.info(
                    "    Number of comparisons done so far: %d (%.1f in "
                    % (comp_done, float(comp_done) / rec_read)
        ...
        used_sec_str = auxiliary.time_string(time.time() - start_time)
        rec_read_time_str = auxiliary.time_string(
            (time.time() - start_time) / self.dataset1.num_records
        )
        if comp_done > 0:
            rec_comp_time_str = auxiliary.time_string(
                (time.time() - start_time) / comp_done
