...
def parse_australia_phone_number(phonenum_str):
  ...
  """
    ...
    if not phonenum_str.isdigit():
        logging.exception(
            "Input phone number contains more than just digits: %s" % (phonenum_str)
        )
        raise Exception

    logging.debug("Attempting to parse Australian phone number: %s" % (phonenum_str))
    ...
    if len(phonenum_str) == 8:  # Standard Australian number without area code
        ...
        if area_code in australia_area_codes:
            logging.debug(
                "Valid Australian area code (%s) for: %s"
        ...
        logging.warn(
            "Illegal Australian phone number (not 8 or 10 digits): %s" % (phonenum_str)
        )
...
def parse_usa_canada_phone_number(phonenum_str):
  ...
  """
    ...
    logging.debug("Attempting to parse Canadian/US phone number: %s" % (phonenum_str))
    ...
    if len(phonenum_str) == 10:  # Parse as 3+3+4
        ...
        logging.warn(
            "Illegal Canadian/US phone number (not 7 or 10 digits): %s" % (phonenum_str)
        )
  ...
  """
    ...
    for idd_pattern in idd_patterns_list:
        ...
        if m:
            ...
            logging.debug(
                "Found matching IDD code (%s), new phone number string"
    ...
    if m:
        ...
        logging.debug(
            "Found an extension (%s), remaining number: %s" % (extension, buffer_str)
        )
    else:
        extension = ""
    ...
    #
    for name, code in list(phone_code_dict.items()):
        if len(code) == 4:
        ...
        if m:
            ...
            logging.debug(
                'Found country code (%s) of country "%s"' % (country_code, country_name)
            )
    ...
    if international == False:
        ...
        if buffer_str[0] == "1":
            ...
            logging.debug(
                'Removed leading "1", remaining phone number: %s' % (buffer_str)
            )
