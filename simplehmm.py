...
class hmm:
    ...
    def __init__(
        ...
        if trans_prob != None:  # Transition probabilities given as input
            ...
            for i in range(self.N):
                for j in range(self.N):
                    if (
                        (not isinstance(trans_prob[i][j], float))
                        or (trans_prob[i][j] < 0.0)
                        or (trans_prob[i][j] > 1.0)
                    ):
                        logging.exception(
                            'Argument "trans_prob" at index [%i,%i]' % (i, j)
                            + " is not a valid number between 0.0 "
                            + "and 1.0"
                        )
                        raise Exception
        ...
        if inita_prob != None:  # Initial state probabilities given as input
            ...
            for i in range(self.N):
                if (
                    (not isinstance(inita_prob[i], float))
                    or (inita_prob[i] < 0.0)
                    or (inita_prob[i] > 1.0)
                ):
                    logging.exception(
                        'Argument "inita_prob" at index [%i] is not ' % (i)
        ...
        if obser_prob != None:  # Observation probabilities given as input
            ...
            for i in range(self.N):
                for j in range(self.M):
                    if (
                        (not isinstance(obser_prob[i][j], float))
                        or (obser_prob[i][j] < 0.0)
                        or (obser_prob[i][j] > 1.0)
                    ):
                        logging.exception(
                            'Argument "obser_prob" at index [%i,%i]' % (i, j)
                            + " is not a valid number between 0.0 "
                            + "and 1.0"
                        )
                        raise Exception
    ...
    def set_trans_prob(self, from_state, to_state, trans_prob):
        ...
        if (
            (not isinstance(trans_prob, float))
            or (trans_prob < 0.0)
            or (trans_prob > 1.0)
        ):
            logging.exception(
                'Argument "trans_prob" is not a valid number ' + "between 0.0 and 1.0"
            )
            raise Exception
    ...
    def set_obser_prob(self, state, obser, obser_prob):
        ...
        if (
            (not isinstance(obser_prob, float))
            or (obser_prob < 0.0)
            or (obser_prob > 1.0)
        ):
            logging.exception(
                'Argument "obser_prob" is not a valid number ' + "between 0.0 and 1.0"
            )
            raise Exception
    ...
    def set_init_prob(self, state, init_prob):
        ...
        if (not isinstance(init_prob, float)) or (init_prob < 0.0) or (init_prob > 1.0):
            logging.exception(
                'Argument "init_prob" is not a valid number ' + "between 0.0 and 1.0"
            )
            raise Exception
    ...
    def check_prob(self):
        ...
        for i in range(self.N):
            ...
            if abs(sum - 1.0) > delta:
                logging.warn(
                    'HMM state "%s" has transition ' % (self.S[i])
        ...
        for i in range(self.N):
            ...
            if abs(sum - 1.0) > delta:
                logging.warn(
                    'HMM state "%s" has observation ' % (self.S[i])
                    + "probabilities sum not 1.0: "
                    + str(sum)
                )
                ret -= 1
    ...
    def train(self, train_data, smoothing=None):
        ...
        if smoothing not in [None, "laplace", "absdiscount"]:
            logging.exception(
                'Illegal value for "smoothing" argument: %s' % (smoothing)
                + ', possible are: None, "laplace" or '
                + '"absdiscount"'
            )
            raise Exception
    ...
    def save_hmm(self, file_name):
        ...
        # Write header  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        #
        f.write("# Hidden Markov Model written by 'simplehmm.py'" + os.linesep)
        f.write("#" + os.linesep)
    ...
    def load_hmm(self, file_name):
        ...
        for i in range(self.N):  # Read N lines with data
            ...
            if len(obser_probs) != self.M:
                logging.exception(
                    "Illegal number of observation probabilities in" + " file"
                )
                raise Exception
    ...
    def print_hmm(self):
        ...
        for i in range(self.N):
            ...
            for j in range(self.N):
                if self.A[i][j] > 0.0:
                    msg.append(
                        "      to state: " + self.S[j] + ": " + str(self.A[i][j])
                    )
