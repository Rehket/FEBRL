...
class RecordStandardiser:
    ...
    def __init__(self, **kwargs):
        ...
        auxiliary.check_is_not_none("Component standardiser list", self.comp_stand_list)

        if self.in_dataset.access_mode != "read":
            logging.exception(
                'Input data set "%s" must be in "read" access mode'
        ...
        if self.out_dataset.access_mode != "write":
            logging.exception(
                'Output data set "%s" must be in "write" access '
            ...
            for field in cs.out_fields:
                if field != None:  # Only check fields that are not set to None
                    if field in output_fields_set:
                        logging.exception(
                            "Output fields definition conflict with "
                            + 'field "%s"' % (str(field))
                        )
                        raise Exception
        ...
        for field_tuple in self.pass_field_list:
            ...
            if len(field_tuple) != 2:
                logging.exception(
                    'Field tuple in "pass_field_list" does not '
            ...
            if in_field not in in_dataset_fields:
                logging.exception(
                    'Input field in "pass_field_list" is not in the '
            ...
            if out_field not in out_dataset_fields:
                logging.exception(
                    'Output field in "pass_field_list" is not in the'
            ...
            if out_field in pass_out_field_set:
                logging.exception(
                    'Output field in "pass_field_list" appears twice'
            ...
            if out_field in output_fields_set:
                logging.exception(
                    'Output field in "pass_field_list" is also an '
        ...
        for i in range(len(self.comp_stand_list)):
            ...
            for in_f in cs.in_fields:
                if in_f != None:
                    ...
                    if in_f not in in_dataset_fields:
                        logging.exception(
                            'Component standardiser "%s" contains a ' % (cs.description)
            ...
            if in_not_none == 0:
                logging.exception(
                    "No input field, or only None input fields "
                    + 'defined for component standardiser "%s"' % (cs.description)
                )
                raise Exception
            ...
            if out_not_none == 0:
                logging.exception(
                    "No output field, or only None output fields"
                    + 'defined for component standardiser "%s"' % (cs.description)
                )
                raise Exception
    ...
    def standardise(self):
        ...
        #
        if self.progress_report != None:
            progress_report_cnt = int(
                self.in_dataset.num_records / (100.0 / self.progress_report)
            )
        ...
        rec_time_str = auxiliary.time_string(
            (time.time() - start_time) / self.in_dataset.num_records
        )
        logging.info(
            "Read and standardised %d records in %s (%s per record)"
...
class ComponentStandardiser:
    ...
    def __tag_component__(self, in_str):
        ...
        while (
            (len(tag_list) > 2)
            and ((tag_list[0] == "VB") and (tag_list[-1] == "VB"))
            or ((tag_list[0] == "OB") and (tag_list[-1] == "CB"))
        ):
    ...
    def __write_hmm_train_record__(
        self, in_str, tok_list, tag_perm_list, state_list, hmm_prob
    ):
    ...
    def __write_hmm_seq_prob_file__(self, hmm_name_str):
        ...
        try:
            ...
            logging.exception(
                "Cannot write to HMM sequence probability file: "
                + '"%s"' % (self.hmm_seq_prob_file)
            )

        self.hmm_seq_prob_fp.write("#" + "#" * 70 + os.linesep)  # Write a header
        self.hmm_seq_prob_fp.write(
            '# HMM sequence probabilities for: "%s"' % (hmm_name_str) + os.linesep
        )
        self.hmm_seq_prob_fp.write("#" + os.linesep)
        self.hmm_seq_prob_fp.write("# Created " + time.ctime(time.time()) + os.linesep)
        self.hmm_seq_prob_fp.write("#" + os.linesep)
        ...
        for (freq, hmm_prob, tag_state_str) in sorted_seq_key_list:
            self.hmm_seq_prob_fp.write("# Pattern: %s" % (tag_state_str) + os.linesep)
            cum_freq += freq
            freq_perc = 100.0 * freq / float(num_rec)
            cum_freq_perc = 100.0 * cum_freq / float(num_rec)

            self.hmm_seq_prob_fp.write(
                "#   Frequency: %d (%.2f%%, cumulative:" % (freq, freq_perc)
                + " %.2f%%)" % (cum_freq_perc)
                + os.linesep
            )
            examples = self.hmm_seq_prob_dict[tag_state_str]
            self.hmm_seq_prob_fp.write(
                "#   Normalised HMM Viterbi probability: "
                + "%.20f" % (hmm_prob)
                + os.linesep
            )
...
class DateStandardiser(ComponentStandardiser):
    ...
    def __init__(self, **kwargs):
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("parse_for"):
                ...
                if (value < 0) or (value > 99):
                    logging.exception(
                        'Argument "pivot_year" is not an integer '
        ...
        if len(self.out_fields) != 3:
            logging.exception(
                'Attribute "output_fields" is not a list with '
        ...
        for format_str in self.parse_formats:
            auxiliary.check_is_string(
                'Format string "%s"' % (str(format_str)), format_str
            )
            if len(format_str) != 8:
                logging.exception(
                    "Format string has wrong length (should be 8 "
            ...
            while tmp_str != "":  # Check if all directives are valid
                if tmp_str[:2] not in ["%b", "%B", "%d", "%m", "%y", "%Y"]:
                    logging.exception(
                        'Illegal directive in format string "%s"' % (format_str)
                    )
                    raise Exception
        ...
        if self.pivot_year == None:
            ...
            if (next_year < 0) or (next_year > 99):
                logging.exception(
                    'Illegal value for "next_year" (not between 0 '
                    + "and 99): "
                    + str(next_year)
                )
                raise Exception
        ...
        self.log(
            [("Pivot year", self.pivot_year), ("Parse formats", self.parse_formats)]
        )  # Log a message
...
class PhoneNumStandardiser(ComponentStandardiser):
    ...
    def __init__(self, **kwargs):
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("default_c"):
                ...
                if value.lower() not in ["australia", "canada/usa"]:
                    logging.exception(
                        'Illegal value for argument "default_country"'
        ...
        if len(self.out_fields) != 5:
            logging.exception(
                'Attribute "output_fields" is not a list with five '
    ...
    def standardise(self, in_str, clean_in_str):
        ...
        parsed_phone_number = phonenum.str_to_phonenum(
            clean_in_str, default_country=self.default_country
        )
...
class NameStandardiser(ComponentStandardiser):
    ...
    def __init__(self, **kwargs):
        ...
        for (keyword, value) in list(kwargs.items()):
            ...
            if keyword.startswith("male_t"):
                ...
                if value not in ["gname", "sname"]:
                    logging.exception(
                        'Argument "first_name_comp" must be either '
                        + '"gname" or "sname"'
                    )
                    raise Exception
        ...
        if len(self.out_fields) != 6:
            logging.exception(
                'Attribute "output_fields" is not a list with six '
                + "elements: %s" % (str(self.out_fields))
            )
            raise Exception
        ...
        if set(self.female_titles).intersection(set(self.male_titles)) != set():
            logging.exception(
                "At least one title word is in both the male and " + "female title list"
            )
            raise Exception
        ...
        if (self.name_hmm == None) and (self.hmm_train_file == None):
            logging.exception(
                "Both name HMM and HMM training file set to None."
        ...
        if self.hmm_train_file != None:  # Open HMM training file for writing
            try:
                ...
                logging.exception(
                    'Cannot write to HMM training file: "%s"' % (self.hmm_train_file)
                )

            self.hmm_train_fp.write("#" + "#" * 70 + os.linesep)  # Write a header
            self.hmm_train_fp.write("# Tagged name data for HMM training." + os.linesep)
            self.hmm_train_fp.write("#" + os.linesep)
            self.hmm_train_fp.write("# Created " + time.ctime(time.time()) + os.linesep)
            self.hmm_train_fp.write("#" + os.linesep)
            self.hmm_train_fp.write("# Possible HMM state for names are:" + os.linesep)
            self.hmm_train_fp.write(
                "# baby, knwn, andor, gname1, gname2, ghyph,"
                + " gopbr, gclbr, agname1,"
                + os.linesep
            )
            self.hmm_train_fp.write(
                "# agname2, coma, sname1, sname2, shyph, "
                + "sopbr, sclbr, asname1, asname2,"
                + os.linesep
            )
        ...
        if (self.hmm_seq_prob_file != None) and (self.name_hmm == None):
            logging.exception(
                "HMM sequence probability file given but no HMM" + " provided."
            )
            raise Exception
        ...
        self.log(
            [
                ("Female title list", self.female_titles),
    ...
    def standardise(self, in_str, clean_in_str):
        ...
        while i < len(tag_list):
            ...
            if tag_list[i] == "NE":
                ...
                #
                if (
                    ((i == 0) or (i == len(tag_list) - 1))
                    or (tag_list[i - 1] in ["PR", "BO"])
                    or ((tag_list[i - 1] == "ST") and ("_" in tok_list[i - 1]))
                ):
        ...
        if self.name_hmm != None:  # Parse using the HMM
            (name_list, state_seq, hmm_prob) = self.__get_name_hmm__(
                tok_list, tag_perm_list
            )
        ...
        if self.hmm_train_file != None:
            self.__write_hmm_train_record__(
                in_str, tok_list, tag_perm_list, state_seq, hmm_prob
            )
    ...
    def __get_name_hmm__(self, tok_list, tag_perm_list):
        ...
        for tag_list in tag_perm_list:
            ...
            logging.info(
                "  Sequence: %s has Viterbi probability: %f" % (str(tag_list), prob)
            )

        logging.info(
            "Best state sequence: %s with tag sequence %s"
            % (str(best_state_seq), str(best_tag_list))
        )

        if max_prob == 0.0:
            logging.warn(
                "Probability is 0.0 for best state sequence: %s" % (str(best_state_seq))
            )
        ...
        if len(tag_list) != len(tok_list):
            logging.exception(
                "Length of token list and best tag list differ: "
        ...
        while i < tag_list_len:  # Loop over tags, tokens and HMM states - - - -
            ...
            if token in ["|", "(", ")"]:  # Do not output vertical bars and brackets
                ...
                if i < tag_list_len - 1:
                    if best_state_seq[i + 1] in [
                        "gname1",
                        "gname2",
                    ]:  # Add to givenname
                        name_list[1].append(token)

                    elif best_state_seq[i + 1] in [
                        "agname1",
                        "agname2",
                    ]:  # Add to agname
                        ...
                        logging.warn(
                            'Strange situation with "baby of": %s' % (str(tok_list))
                        )
                else:
                    logging.warn(
                        'Strange situation with "baby of": %s' % (str(tok_list))
                ...
                if (i > 0) and (
                    ...
                    if name_list[1] != []:  # Only append hyphen after given name
                        ...
                        logging.warn(
                            "Strange hyphen situation in given name: %s"
                            % (str(tok_list))
                        )
                    ...
                    if name_list[1] != []:  # Only append hyphen after alt. given name
                        name_list[1].append(
                            token
                        )  # Append to alternative given name list
                    else:
                        logging.warn(
                            "Strange hyphen situation in alternative given "
                            + "name: %s" % (str(tok_list))
                        )
                else:
                    logging.warn(
                ...
                if (i > 0) and (
                    ...
                    if name_list[2] != []:  # Only append hyphen after a surname
                        ...
                        logging.warn(
                            "Strange hyphen situation in surname: %s" % (str(tok_list))
                        )
                    ...
                    if name_list[3] != []:  # Only append hyphen after a alt. surname
                        ...
                        logging.warn(
                            "Strange hyphen situation in alternative "
                ...
                if (i < tag_list_len - 1) and (
                    best_state_seq[i + 1] in ["pref1", "pref2"]
                ):
                ...
                if (i < tag_list_len - 1) and (
                    ...
                    if name_list[0] != []:  # There is already a given name
                        name_list[0].append(token)  # Append to given name list
                        logging.warn(
                            "Strange name prefix situation in given name: %s"
                            % (str(tok_list))
                        )
                    else:
                        name_list[0].append(token)  # Append to given name list
                    ...
                    if name_list[1] != []:  # There is already an alternative given name
                        ...
                        logging.warn(
                            "Strange name prefix situation in alternative "
                    ...
                    if name_list[2] != []:  # There is already a surname
                        name_list[2].append(token)  # Append to surname list
                        logging.warn(
                            "Strange name prefix situation in surname: %s"
                            % (str(tok_list))
                        )
                    else:
                        name_list[2].append(token)  # Append to surname list
                ...
                logging.warn(
                    "This should never happen! State: %s, tag: %s, token" % (state, tag)
                    + ": %s, token list: %s" % (token, tok_list)
                )
            i += 1
        ...
        for n_list in name_list:
            if len(n_list) > 3:
                logging.warn(
                    "A name output field contains more than three "
        ...
        if (name_list[0] == []) and (name_list[1] != []):
            logging.warn(
                "No given name but an alternative given name: %s" % (str(name_list[1]))
                + " -> Corrected"
            )
        ...
        if (name_list[2] == []) and (name_list[3] != []):
            logging.warn(
                "No surname but an alternative surname: %s" % (str(name_list[3]))
                + " -> Corrected"
            )
        ...
        if (name_list[0] != []) and (name_list[2] == []):
            ...
            if name_list[1] != []:  # An alternative given name is availabe
                logging.warn(
                    "No surname but an alternative given name: %s" % (str(name_list))
                    + " -> Corrected"
                )
                ...
                logging.debug(
                    "No surname but more than one given name: %s" % (str(name_list))
                    + " -> Corrected"
                )
    ...
    def finalise(self):
        ...
        std_type_dict = {
            ...
            "G": "Given name only",
            "S": "Surname only",
            "GS": "Given name / surname",
            "SG": "Surname / given name",
            "HMM": "HMM standardisation",
        }
...
class AddressStandardiser(ComponentStandardiser):
    ...
    def __init__(self, **kwargs):
        ...
        if len(self.out_fields) != 27:
            logging.exception(
                'Attribute "output_fields" is not a list with 27 '
                + " elements: %s" % (str(self.out_fields))
            )
            raise Exception
        ...
        if (self.address_hmm == None) and (self.hmm_train_file == None):
            logging.exception(
                "Both address HMM and HMM training file set to "
        ...
        if self.hmm_train_file != None:  # Open HMM training file for writing
            ...
            self.hmm_train_fp.write("#" + "#" * 70 + os.linesep)  # Write a header
            self.hmm_train_fp.write(
                "# Tagged address data for HMM training." + os.linesep
            )
            self.hmm_train_fp.write("#" + os.linesep)
            self.hmm_train_fp.write("# Created " + time.ctime(time.time()) + os.linesep)
            self.hmm_train_fp.write("#" + os.linesep)
            self.hmm_train_fp.write(
                "# Possible HMM state for addresses are:" + os.linesep
            )
            for hmm_state_line in [
                ...
                "locality_name3, postcode, state_abbrev",
                "slash",
                "hyph",
                "rubb",
            ]:
    ...
    def standardise(self, in_str, clean_in_str):
        ...
        if self.address_hmm != None:  # Parse using the HMM
            (address_list, state_seq, hmm_prob) = self.__get_address_hmm__(
                tok_list, tag_perm_list
            )
        else:
            address_list = 27 * [""]
    ...
    def __get_address_hmm__(self, tok_list, tag_perm_list):
        ...
        for i in range(tag_list_len):  # Loop over tokens and states
            ...
            if token in [",", "(", ")", "|", "-", "/"]:
                logging.info(
                    'Discard character "%s" from input (with tag: %s)' % (token, state)
                )
            else:
        ...
        for name_state in ["building_name", "street_name", "locality_name"]:
            if (
                (name_state + "1" in address_dict)
                or (name_state + "2" in address_dict)
        ...
        for name_state in ["locality_name", "state_abbrev", "country"]:
            if name_state in address_dict:
                ...
                if len(name_state_list) > 1:  # Contains more than one word
                    ...
                    if name_state_tuple in self.tag_table:
        ...
        #
        if ("street_type" not in address_dict) and ("locality_name" in address_dict):
            ...
            if (first_loc_name.startswith("saint_")) or (
                ...
                if loc_name_tuple in self.tag_table:
                    lookup_val = self.tag_table[
                        loc_name_tuple
                    ...
                    logging.info(
                        'Corrected locality name "%s" to "%s" and moved '
                        % (" ".join(org_loc_name_list), " ".join(loc_name_list))
                        + '"%s" to street type as "%s"'
                        % (first_loc_name[:5], " ".join(address_dict["street_type"]))
                    )

        # Finally do some tests on the output fields  - - - - - - - - - - - - - - -
        #
        for (out_field, val_list) in list(address_dict.items()):

            # Check if an output field value list has more than three elements
            #
            if len(val_list) > 3:
                logging.warn(
                    'Output field "%s" contains more than ' % (out_field)
            ...
            if out_field.endswith("number"):  # Numbers only, not pre- or suffix
                if len(val_list) > 1:
                    logging.warn(
                        'More than one element in output field "%s": %s'
                        % (out_field, str(val_list))
                    )
                for num in val_list:
                    if not num.isdigit():  # Element contains not only digits
                        logging.warn(
                            'Output field "%s" contains no number: %s'
                            % (out_field, str(val_list))
                        )
            ...
            if out_field.endswith("type"):  # street_type, flat_type, level_type
                if len(val_list) > 1:
                    logging.warn(
                        'More than element in output field "%s": %s'
                        % (out_field, str(val_list))
                    )
                for word in val_list:
                    word_tuple = (word,)  # Make it a tuple
                    if word_tuple in self.tag_table:
                        word_tag = self.tag_table[word_tuple][1]  # Get it's tag

                        if (
                            (out_field.startswith("street") and ("WT" not in word_tag))
                            or (out_field.startswith("flat") and ("FT" not in word_tag))
                            or (
                                out_field.startswith("level") and ("LT" not in word_tag)
                            )
                        ):
                            logging.warn(
                                'Type word "%s" in output field "%s" does not '
                                % (word, out_field)
                                + 'have a correct "type" '
                                + "tag: %s" % (word_tag)
                            )
        ...
        #
        if "wayfare_qualifier" in address_dict:  # Check wayfare qualifier
            wf_quali = address_dict["wayfare_qualifier"]
            for wq in wf_quali:
                if ((wq,) not in self.tag_table) or (
                    (
                        (wq,) in self.tag_table
                        and (self.tag_table[(wq,)][1].find("LQ") < 0)
                    )
                ):
                    logging.warn("Wayfare qualifier word is not known: %s" % (str(wq)))
                    break  # Exit for loop

        if "locality_qualifier" in address_dict:  # Check locality qualifier
            loc_quali = address_dict["locality_qualifier"]
            for lq in loc_quali:
                if ((lq,) not in self.tag_table) or (
                    (
                        (lq,) in self.tag_table
                        and (self.tag_table[(lq,)][1].find("LQ") < 0)
                    )
                ):
        ...
        for check_state in [
            ...
            if check_state in address_dict:
