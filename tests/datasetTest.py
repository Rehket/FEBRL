...
class TestCase(unittest.TestCase):
    ...
    def testCSV(self):  # - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for test_file in ["./test-data.csv", "./test-data.csv.gz"]:
            ...
            test_ds = dataset.DataSetCSV(
                description="A test CSV data set",
                access_mode="read",
                field_list=[
                    ("rec-id", 0),
                    ("gname", 1),
        ...
        assert isinstance(
            test_ds.field_list, list
        ), "CSV data set field list is not of a list: %s" % (str(test_ds.field_list))
    ...
    def testCOL(self):  # - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for test_file in ["./test-data.col", "./test-data.col.gz"]:
            ...
            #
            test_ds = dataset.DataSetCOL(
                description="A test COL data set",
                access_mode="read",
                field_list=[
                    ("rec-id", 6),
                    ("gname", 10),
        ...
        assert isinstance(
            test_ds.field_list, list
        ), "COL data set field list is not of a list: %s" % (str(test_ds.field_list))
        assert test_ds.access_mode == "write", (
    ...
    def testMemory(self):  # ---------------------------------------------------
        ...
        #
        test_ds = dataset.DataSetMemory(
            description="A test Memory data set",
        ...
        assert isinstance(
            test_ds.field_list, list
        ), "Memory data set field list is not of a list: %s" % (str(test_ds.field_list))
    ...
    def testShelve(self):  # ---------------------------------------------------
        ...
        #
        test_ds = dataset.DataSetShelve(
            description="A test Shelve data set",
        ...
        assert isinstance(
            test_ds.field_list, list
        ), "Shelve data set field list is not of a list: %s" % (str(test_ds.field_list))
        assert test_ds.access_mode == "readwrite", (
    ...
    def testCSVdelimiter(self):  # - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for (test_file, delim, num_col) in [
            ("./test-comma.txt", ",", 6),
            ("./test-comma.txt", "x", 1),
            ("./test-period.txt", ":", 6),
            ("./test-period.txt", ",", 1),
            ...
            for (rec_id, rec_data) in test_ds.readall():
                ...
                if num_col == 6:
                    ...
                    for col_val in rec_data:
                        ...
                        assert col_val == check_col_val, (
                            "Different column value than "
    ...
    def testMissValStripFields(self):  # - - - - - - - - - - - - - - - - - - - -
        ...
        for (miss_val, strip_fields_flag) in [
            ("15", False),
            ("15", True),
            (["3"], False),
            (["3"], True),
            ...
            for (rec_id, rec_data) in test_ds.readall():
                for col_val in rec_data:
                    if (strip_fields_flag == True) and (len(col_val) > 0):
                        assert (
                            col_val[0] not in string.whitespace
                        ), 'Field value not stripped of whitespaces: "%s"' % (col_val)
                        assert (
                            col_val[-1] not in string.whitespace
                        ), 'Field value not stripped of whitespaces: "%s"' % (col_val)
