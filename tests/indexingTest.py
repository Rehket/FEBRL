...
# TODO: Fix All the Unreachable Code
...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dataset1 = dataset.DataSetCSV(
            description="First test CSV data set",
            access_mode="read",
            rec_ident="rec_id",
            header_line=True,
        ...
        self.dataset2 = dataset.DataSetCSV(
            description="Second test CSV data set",
            access_mode="read",
            rec_ident="rec_id",
            header_line=True,
            file_name="./test-data.csv",
        ...
        gn_jfc = comparison.FieldComparatorJaro(threshold=0.75, desc="Givenname Jaro")
        sn_wfcc = comparison.FieldComparatorWinkler(
            threshold=0.5, desc="Surname Winkler", do_cache=True
        )
        sub_pqfcc = comparison.FieldComparatorPosQGram(
            threshold=0.6,
            q=2,
            max_dist=3,
            common_div="average",
        ...
        pc_kfc = comparison.FieldComparatorKeyDiff(
            max_key_di=2, desc="Postcode KeyDiff"
        )

        field_comp_list = [
            (gn_jfc, "given_name", "given_name"),
            (sn_wfcc, "surname", "surname"),
            (sub_pqfcc, "suburb", "suburb"),
            (pc_kfc, "postcode", "postcode"),
        ]

        self.rec_comp_link = comparison.RecordComparator(
            self.dataset1,
            self.dataset2,
            field_comp_list,
        ...
        self.rec_comp_dedupl = comparison.RecordComparator(
            self.dataset1,
            self.dataset1,
            field_comp_list,
    ...
    def testFullIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - - - -
        ...
        full_index = indexing.FullIndex(
            description="Test full index",
        ...
        for i in range(10):
            ...
            assert full_index.num_rec_pairs == len(self.rec_ident1) * len(
                self.rec_ident2
            )
            ...
            for rec_ident1 in self.rec_ident1:
                for rec_ident2 in self.rec_ident2:
                    ...
                    assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                        full_index.rec_comparator.field_comparison_list
                    )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = full_index.run(length_filter_perc=lf)
        ...
        for cot in [0.05, 0.1, 0.2, 0.5, 0.7, 0.8, 0.9]:
            ...
            [field_names_list, this_w_vec_dict] = full_index.run(cut_off_threshold=cot)
    ...
    def testBlockingIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        block_index = indexing.BlockingIndex(
            description="Test blocking index",
            dataset1=self.dataset1,
            dataset2=self.dataset2,
            rec_comparator=self.rec_comp_link,
            progress=2,
        ...
        assert block_index.num_rec_pairs <= len(self.rec_ident1) * len(self.rec_ident2)
        assert isinstance(block_index.index1, dict)
        assert isinstance(block_index.index2, dict)
        assert isinstance(block_index.description, str)
        assert isinstance(block_index.rec_cache1, dict)
        ...
        for (rec_ident1, rec_ident2) in weight_vec_dict:
            ...
            assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                block_index.rec_comparator.field_comparison_list
            )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = block_index.run(length_filter_perc=lf)
        ...
        for cot in [0.05, 0.1, 0.2, 0.5, 0.7, 0.8, 0.9]:
            ...
            [field_names_list, this_w_vec_dict] = block_index.run(cut_off_threshold=cot)
    ...
    def testSortingIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - - -
        ...
        for w in [1, 2, 3, 4, 5, 6, 7, 9, 11, 12, 15, 20]:
            ...
            sort_index = indexing.SortingIndex(
                description="Test Sorting index",
                dataset1=self.dataset1,
                dataset2=self.dataset2,
                rec_comparator=self.rec_comp_link,
                progress=2,
            ...
            assert sort_index.num_rec_pairs <= len(self.rec_ident1) * len(
                self.rec_ident2
            )
            assert isinstance(sort_index.index1, dict)
            assert isinstance(sort_index.index2, dict)
            ...
            for (rec_ident1, rec_ident2) in weight_vec_dict:
                ...
                assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                    sort_index.rec_comparator.field_comparison_list
                )

            # All record pairs from the blocking index should be in the sorting index
            # as well
            #
            assert len(block_index_weight_vec_dict) <= len(
                weight_vec_dict
            ...
            #
            for ((rec_ident1, rec_ident2), weight_list) in list(
                weight_vec_dict.items()
            ):
                if rec_ident1 > rec_ident2:
        ...
        for (rec_ident1, rec_ident2) in list(weight_vec_dict.keys()):
            assert (
                rec_ident1,
                rec_ident2,
            ) in block_index_weight_vec_dict, "Not in blocking weight vectors: %s" % (
                str((rec_ident1, rec_ident2))
            )
        ...
        for (rec_ident1, rec_ident2) in list(block_index_weight_vec_dict.keys()):
            assert (
                rec_ident1,
                rec_ident2,
            ) in weight_vec_dict, "Not in sorting weight vectors: %s" % (
                str((rec_ident1, rec_ident2))
            )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = sort_index.run(length_filter_perc=lf)
        ...
        for cot in [0.05, 0.1, 0.2, 0.5, 0.7, 0.8, 0.9]:
            ...
            [field_names_list, this_w_vec_dict] = sort_index.run(cut_off_threshold=cot)
    ...
    def testAdaptSortIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        for (str_cmp_funct, str_cmp_thres) in [
            ...
            (stringcmp.jaro, 0.8),
            (stringcmp.jaro, 0.7),
            (stringcmp.bigram, 0.7),
            (stringcmp.bigram, 0.8),
            ...
            adsort_index = indexing.AdaptSortingIndex(
                description="Test Adapt Sorting index",
                dataset1=self.dataset1,
                dataset2=self.dataset2,
                rec_comparator=self.rec_comp_link,
                progress=2,
            ...
            assert adsort_index.num_rec_pairs <= len(self.rec_ident1) * len(
                self.rec_ident2
            )
            assert isinstance(adsort_index.index1, dict)
            assert isinstance(adsort_index.index2, dict)
            ...
            for (rec_ident1, rec_ident2) in weight_vec_dict:
                ...
                assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                    adsort_index.rec_comparator.field_comparison_list
                )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = adsort_index.run(
                length_filter_perc=lf
            )
    ...
    def testSortArrayIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        for w in [2, 3, 4, 5, 6, 7, 9, 11, 12, 15, 20]:
            ...
            sort_index = indexing.SortingArrayIndex(
                description="Test SortingArray index",
                dataset1=self.dataset1,
                dataset2=self.dataset2,
                rec_comparator=self.rec_comp_link,
                progress=2,
    ...
    def testQGramIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - - -
        ...
        for t in [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.3, 0.1]:
            ...
            qgram_index = indexing.QGramIndex(
            ...
            assert qgram_index.num_rec_pairs <= len(self.rec_ident1) * len(
                self.rec_ident2
            )
            assert isinstance(qgram_index.index1, dict)
            assert isinstance(qgram_index.index2, dict)
            ...
            for (rec_ident1, rec_ident2) in weight_vec_dict:
                ...
                assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                    qgram_index.rec_comparator.field_comparison_list
                )
        ...
        for (rec_ident1, rec_ident2) in list(block_index_weight_vec_dict.keys()):
            assert (
                rec_ident1,
                rec_ident2,
            ) in weight_vec_dict, "Not in q-gram weight vectors: %s" % (
                str((rec_ident1, rec_ident2))
            )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = qgram_index.run(length_filter_perc=lf)
        ...
        for cot in [0.05, 0.1, 0.2, 0.5, 0.7, 0.8, 0.9]:
            ...
            [field_names_list, this_w_vec_dict] = qgram_index.run(cut_off_threshold=cot)
    ...
    def testCanopyIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - - -
        ...
        index_def1 = [["surname", "surname", False, False, None, []]]
        index_def2 = [
            ["given_name", "given_name", True, False, 4, []],
        ...
        for canopy_method_list in [
            [
                ("tfidf", "threshold", 0.999999, 0.999999),
            ...
            [("tfidf", "nearest", 4, 4), ("tfidf", "nearest", 4, 6)],
            [
            ...
            [("jaccard", "nearest", 4, 4), ("jaccard", "nearest", 4, 6)],
        ]:
            ...
            for canopy_method in canopy_method_list:
                ...
                canopy_index = indexing.CanopyIndex(
                    description="Test canopy index",
                    dataset1=self.dataset1,
                    dataset2=self.dataset2,
                ...
                assert canopy_index.num_rec_pairs <= len(self.rec_ident1) * len(
                    self.rec_ident2
                )
                assert isinstance(canopy_index.index1, dict)
                assert isinstance(canopy_index.index2, dict)
                ...
                for (rec_ident1, rec_ident2) in weight_vec_dict:
                    ...
                    assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                        canopy_index.rec_comparator.field_comparison_list
                    )
                ...
                for rec_ident_pair in prev_rec_dict.keys():
                    ...
                    assert (
                        rec_ident_pair in weight_vec_dict
        ...
        for canopy_method in [
            ...
            for (rec_ident1, rec_ident2) in list(weight_vec_dict.keys()):
                assert (
                    rec_ident1,
                    rec_ident2,
                ) in block_index_weight_vec_dict, (
                    "Not in blocking weight vectors: %s"
                    % (str((rec_ident1, rec_ident2)))
                )
            ...
            for (rec_ident1, rec_ident2) in list(block_index_weight_vec_dict.keys()):
                assert (
                    rec_ident1,
                    rec_ident2,
                ) in weight_vec_dict, "Not in canopy weight vectors: %s" % (
                    str((rec_ident1, rec_ident2))
                )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = canopy_index.run(
                length_filter_perc=lf
            )
    ...
    def testStringMapIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        for canopy_method_list in [
            ...
            [("nearest", 2, 2), ("nearest", 2, 3), ("nearest", 2, 4)],
            [("nearest", 3, 3), ("nearest", 4, 6)],
            [("threshold", 1.0, 1.0), ("threshold", 1.0, 0.9), ("threshold", 1.0, 0.7)],
            [("threshold", 0.9, 0.9), ("threshold", 0.9, 0.7), ("threshold", 0.9, 0.5)],
            [("threshold", 0.7, 0.7), ("threshold", 0.7, 0.2)],
        ]:
            ...
            for (dim, sub_dim) in [
                ...
                for canopy_method in canopy_method_list:
                    ...
                    strmap_index = indexing.StringMapIndex(
                        descri="Test str-map index",
                        dataset1=self.dataset1,
                        dataset2=self.dataset2,
                    ...
                    assert strmap_index.num_rec_pairs <= len(self.rec_ident1) * len(
                        self.rec_ident2
                    )
                    assert isinstance(strmap_index.index1, dict)
                    assert isinstance(strmap_index.index2, dict)
                    ...
                    for (rec_ident1, rec_ident2) in weight_vec_dict:
                        ...
                        assert rec_ident2 in self.rec_ident2
                        assert isinstance(
                            weight_vec_dict[(rec_ident1, rec_ident2)], list
                        )

                        assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                            strmap_index.rec_comparator.field_comparison_list
                        )
        ...
        #
        for canopy_method in [("threshold", 1.0, 1.0), ("nearest", 1, 1)]:
            ...
            for (dim, sub_dim) in [
                ...
                for (rec_ident1, rec_ident2) in list(
                    block_index_weight_vec_dict.keys()
                ):
                    assert (rec_ident1, rec_ident2) in weight_vec_dict, (
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = strmap_index.run(
                length_filter_perc=lf
            )
    ...
    def testStringMapIndexDedupl(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        for canopy_method_list in [
            ...
            [("threshold", 1.0, 1.0), ("threshold", 1.0, 0.9), ("threshold", 1.0, 0.7)],
            [("threshold", 0.9, 0.9), ("threshold", 0.9, 0.7), ("threshold", 0.9, 0.5)],
            [("threshold", 0.7, 0.7), ("threshold", 0.7, 0.3)],
        ]:
    ...
    def testBigMatchIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        for block_method_list in [
            ...
            for block_method in block_method_list:
                ...
                bigmatch_index = indexing.BigMatchIndex(
                    descrip="Test BigMatch index",
                    dataset1=self.dataset1,
                    dataset2=self.dataset2,
                ...
                assert len(weight_vec_dict) == bigmatch_index.num_rec_pairs, (
                    len(weight_vec_dict),
                    bigmatch_index.num_rec_pairs,
                )
                assert bigmatch_index.num_rec_pairs > 0
                assert bigmatch_index.num_rec_pairs <= len(self.rec_ident1) * len(
                    self.rec_ident2
                )
                ...
                for (rec_ident1, rec_ident2) in weight_vec_dict:
                    ...
                    assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                        bigmatch_index.rec_comparator.field_comparison_list
                    )
                ...
                for (rec_ident1, rec_ident2) in block_index_weight_vec_dict:
                    assert (
                        rec_ident1,
                        rec_ident2,
                    ) in weight_vec_dict, "Record pair %s from blocking index not in BigMatch index" % (
                        str((rec_ident1, rec_ident2))
                    ) + " with block method: %s" % (
                        block_method
                    )
        ...
        for block_method in [
            ...
            for (rec_ident1, rec_ident2) in list(block_index_weight_vec_dict.keys()):
                assert (
                    rec_ident1,
                    rec_ident2,
                ) in weight_vec_dict, "Not in BigMatch weight vectors: %s" % (
                    str((rec_ident1, rec_ident2))
                )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = bigmatch_index.run(
                length_filter_perc=lf
            )
    ...
    def testDedupIndexLinkage(self):  # - - - - - - - - - - - - - - - - - - - - -
        ...
        for block_method_list in [
            ...
            for block_method in block_method_list:
                ...
                dedup_index = indexing.DedupIndex(
                    descrip="Test Dedup index",
                    dataset1=self.dataset1,
                    dataset2=self.dataset1,
                    block_method=block_method,
                    rec_comp=self.rec_comp_dedupl,
                ...
                for (rec_ident1, rec_ident2) in weight_vec_dict:
                    ...
                    assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                        dedup_index.rec_comparator.field_comparison_list
                    )
                ...
                for (rec_ident1, rec_ident2) in block_index_weight_vec_dict:
                    assert (
                        rec_ident1,
                        rec_ident2,
                    ) in weight_vec_dict, "Record pair %s from blocking index not in Dedup index" % (
                        str((rec_ident1, rec_ident2))
                    ) + " with block method: %s" % (
                        block_method
                    )
        ...
        for block_method in [
            ...
            for (rec_ident1, rec_ident2) in list(block_index_weight_vec_dict.keys()):
                assert (
                    rec_ident1,
                    rec_ident2,
                ) in weight_vec_dict, "Not in Dedup weight vectors: %s" % (
                    str((rec_ident1, rec_ident2))
                )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = dedup_index.run(length_filter_perc=lf)
        ...
        for cot in [0.05, 0.1, 0.2, 0.5, 0.7, 0.8, 0.9]:
            ...
            [field_names_list, this_w_vec_dict] = dedup_index.run(cut_off_threshold=cot)
    ...
    def testSuffixArrayIndexLinkage(self):  # - - - - - - - - - - - - - - - - - -
        ...
        block_method_lists = [
            [(5, 5, False), (4, 5, False), (3, 5, False), (2, 5, False), (1, 5, False)],
            [(5, 5, True), (4, 5, True), (3, 5, True), (2, 5, True), (1, 5, True)],
            [
                (5, 10, False),
                (4, 10, False),
                (3, 10, False),
            ...
            [(5, 10, True), (4, 10, True), (3, 10, True), (2, 10, True), (1, 10, True)],
        ]
        ...
        for block_method_list in block_method_lists:
            for (min_q_gram_len, max_block_size, padded) in block_method_list:
                ...
                for suff_method in ["suffixonly", "allsubstr"]:
                    ...
                    sarray_index = indexing.SuffixArrayIndex(
                        desc="Test suff-arr index",
                        dataset1=self.dataset1,
                        dataset2=self.dataset2,
                        rec_compar=self.rec_comp_link,
                        progress=2,
                        ...
                        padd=padded,
                        block_method=(min_q_gram_len, max_block_size),
                        index_def=[index_def1, index_def2],
                    )
                    ...
                    assert sarray_index.num_rec_pairs <= len(self.rec_ident1) * len(
                        self.rec_ident2
                    )
                    assert isinstance(sarray_index.index1, dict)
                    assert isinstance(sarray_index.index2, dict)
                    ...
                    for (rec_ident1, rec_ident2) in weight_vec_dict:
                        ...
                        assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                            sarray_index.rec_comparator.field_comparison_list
                        )
        ...
        for (rec_ident1, rec_ident2) in list(block_index_weight_vec_dict.keys()):
            assert (
                rec_ident1,
                rec_ident2,
            ) in weight_vec_dict, "Not in suffix array weight vectors: %s" % (
                str((rec_ident1, rec_ident2))
            )
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = sarray_index.run(
                length_filter_perc=lf
            )
    ...
    def testRobustSuffixArrayIndexLinkage(self):  # - - - - - - - - - - - - - - -
        ...
        for block_method_list in block_method_lists:
            for (min_q_gram_len, max_block_size, padded) in block_method_list:
                ...
                for (str_cmp_funct, str_cmp_thres) in [
                    ...
                    rsarray_index = indexing.RobustSuffixArrayIndex(
                        desc="Test robust suff-arr index",
                        dataset1=self.dataset1,
                        dataset2=self.dataset2,
                        rec_compar=self.rec_comp_link,
                        progress=2,
                    ...
                    assert rsarray_index.block_method == (
                        min_q_gram_len,
                        max_block_size,
                    )
                    ...
                    assert rsarray_index.num_rec_pairs <= len(self.rec_ident1) * len(
                        self.rec_ident2
                    )
                    assert isinstance(rsarray_index.index1, dict)
                    assert isinstance(rsarray_index.index2, dict)
                    ...
                    for (rec_ident1, rec_ident2) in weight_vec_dict:
                        ...
                        assert len(weight_vec_dict[(rec_ident1, rec_ident2)]) == len(
                            rsarray_index.rec_comparator.field_comparison_list
                        )
                    ...
                    if min_q_gram_len == 1:
                        ...
                        for rec_ident_pair in prev_rec_dict.keys():
                            assert rec_ident_pair in weight_vec_dict, (
                                ...
                                % (
                                    str(rec_ident_pair),
                                    (str(rsarray_index.block_method)),
                                )
                            )
        ...
        #
        rsarray_index = indexing.RobustSuffixArrayIndex(
            desc="Test robust suffix array index",
            dataset1=self.dataset1,
            dataset2=self.dataset2,
            rec_compar=self.rec_comp_link,
            progress=2,
        ...
        for lf in [99, 50, 30, 20, 15, 10, 5, 1, 0.5]:
            ...
            [field_names_list, this_w_vec_dict] = rsarray_index.run(
                length_filter_perc=lf
            )
