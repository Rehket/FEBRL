...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tag_lookup_files = [
            src_data_dir + "post_address.tbl",
        ...
        self.frequency_lookup_files = [
            src_data_dir + "givenname_f_freq.csv",
        ...
        self.correction_list_files = [
            src_data_dir + "address_corr.lst",
    ...
    def testTagLookupTables(self):  # - - - - - - - - - - - - - - - - - - - - - -
        ...
        #
        lookup_table = lookup.TagLookupTable(descr=self.tag_lookup_files[0], default="")
        ...
        assert len(lookup_table) > 0, "Combined look-up table is empty after load"

        assert lookup_table["xyz1234zyx"] == lookup_table.default, (
            "Combined look-up table returns wrond default: " + lookup_table.default
        )
    ...
    def testFrequencyLookupTables(self):  # - - - - - - - - - - - - - - - - - - -
        ...
        for f in self.frequency_lookup_files:
            ...
            assert lookup_table.sum > 0, 'Look-up table "' + f + '" has zero sum'

            assert lookup_table.sum > lookup_table.length, (
                'Look-up table "'
                + f
                + '" has sum smaller than number of entries: '
                + str(self.length)
            )
        ...
        lookup_table = lookup.FrequencyLookupTable(
            descr=self.frequency_lookup_files[0], default=""
        )
        ...
        assert lookup_table.sum == None, "Combined look-up table has non-None sum"

        lookup_table.load(self.frequency_lookup_files)  # Load the table

        assert len(lookup_table) > 0, "Combined look-up table is empty after load"

        assert lookup_table.sum > 0, "Combined look-up table has zero sum"
    ...
    def testGeocodeLookupTables(self):  # - - - - - - - - - - - - - - - - - - - -
        ...
        for f in self.geocode_lookup_files:
            ...
            for (key, value) in list(lookup_table.items()):
                ...
                assert (value[0] >= -180.0) and (value[0] <= 180.0), (
                    'Location in look-up table "'
                    + f
                    + '" has illegal longitude: '
                    + str(value[0])
                )
                assert (value[1] >= -90.0) and (value[1] <= 90.0), (
                    'Location in look-up table "'
                    + f
                    + '" has illegal latitude: '
                    + str(value[1])
                )
        ...
        lookup_table = lookup.GeocodeLookupTable(
            descr=self.geocode_lookup_files[0], default=""
        )
    ...
    def testCorrectionLists(self):  # - - - - - - - - - - - - - - - - - - - - - -
        ...
        for f in self.correction_list_files:
            ...
            assert corr_list.file_name == "", (
                'Correction list "'
                + f
                + '" has non-empty file name: '
                + str(corr_list.file_name)
            )
            ...
            assert len(corr_list) > 0, 'Correction list "' + f + '" is empty after load'
