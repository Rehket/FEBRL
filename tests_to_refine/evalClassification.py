...
#
result_file_name = (
    "./results/evalClassification-" + time.strftime("%Y%m%d-%H%M") + ".res"
)
...
#
census_ds_A = dataset.DataSetCSV(
    description="Census data set A",
    access_mode="read",
    delimiter="\t",
    rec_ident="rec_id",
    header_line=False,
...
census_ds_B = dataset.DataSetCSV(
    description="Census data set B",
    access_mode="read",
    delimiter="\t",
    rec_ident="rec_id",
    header_line=False,
...
census_surname_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="surname_qgram"
)
census_given_name_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="given_name_qgram"
)
census_suburb_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="suburb_qgram"
)

census_surname_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="surname_bagdist"
)
census_given_name_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="given_name_bagdist"
)
census_suburb_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="suburb_bagdist"
)

census_surname_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="surname_lcs"
)
census_given_name_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="given_name_lcs"
)
census_suburb_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="suburb_lcs"
)

census_middle_inital_exact = comparison.FieldComparatorExactString(
    desc="middle_inital_exact"
)
census_zipcode_exact = comparison.FieldComparatorExactString(desc="zipcode_exact")

census_zipcode_keydiff = comparison.FieldComparatorKeyDiff(
    max_key_diff=2, desc="zipcode_keydiff"
)
...
census_rec_comp = comparison.RecordComparator(
    census_ds_A, census_ds_B, census_fc_list, "Census record comparator"
)
...
cens_bigmatch_index = indexing.BigMatchIndex(
    descrip="Census BigMatch index",
    dataset1=census_ds_A,
    dataset2=census_ds_B,
    weight_vec_file=weight_vect_dir + "census-bigmatch-index-weight-vectors.csv",
...
cens_full_index = indexing.FullIndex(
    description="Census Full index",
    dataset1=census_ds_A,
    dataset2=census_ds_B,
    weight_vec_file=weight_vect_dir + "census-full-index-weight-vectors.csv",
    rec_comparator=census_rec_comp,
    progress=progress_precentage,
    index_def=[],
)  # Not needed for full ind.
...
# - No record identifer is available.
#
rest_ds = dataset.DataSetCSV(
    description="Restaurant data set",
    access_mode="read",
    rec_ident="rec_id",
    header_line=True,
...
rest_index_list = [
    ...
    [["city", "city", False, False, None, [encode.dmetaphone, 4]]],
]
...
rest_name_winkler = comparison.FieldComparatorWinkler(thres=0, desc="name_winkler")
rest_addr_winkler = comparison.FieldComparatorWinkler(thres=0, desc="addr_winkler")
rest_city_winkler = comparison.FieldComparatorWinkler(thres=0, desc="city_winkler")

rest_name_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="name_qgram"
)
rest_addr_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="addr_qgram"
)
rest_city_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="city_qgram"
)

rest_name_bagdist = comparison.FieldComparatorBagDist(thres=0, desc="name_bagdist")
rest_addr_bagdist = comparison.FieldComparatorBagDist(thres=0, desc="addr_bagdist")
rest_city_bagdist = comparison.FieldComparatorBagDist(thres=0, desc="city_bagdist")

rest_name_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="name_lcs"
)
rest_addr_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="addr_lcs"
)
rest_city_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="city_lcs"
)

rest_fc_list = [
    (rest_class_exact, "class", "class"),
...
rest_rec_comp = comparison.RecordComparator(
    rest_ds, rest_ds, rest_fc_list, "Restaurant record comparator"
)
...
rest_dedup_index = indexing.DedupIndex(
    description="Restaurant Dedup index",
    dataset1=rest_ds,
    dataset2=rest_ds,
    weight_vec_file=weight_vect_dir + "restaurant-dedup-index-weight-vectors.csv",
...
rest_full_index = indexing.FullIndex(
    description="Restaurant Full index",
    dataset1=rest_ds,
    dataset2=rest_ds,
    weight_vec_file=weight_vect_dir + "restaurant-full-index-weight-vectors.csv",
    rec_comparator=rest_rec_comp,
    progress=progress_precentage,
    index_def=[],
)  # Not needed for full ind.
...
cora_ds = dataset.DataSetCSV(
    description="Cora data set",
    access_mode="read",
    rec_ident="rec_id",
    delimiter="\t",
    header_line=False,
...
cora_index_list = [
    ...
    [["year", "year", False, False, None, [encode.dmetaphone, 3]]],
]

# Exact comparison of 'paper_id': If the same it is a match, otherwise not
#
cora_paper_id_exact = comparison.FieldComparatorExactString(desc="paper_id_exact")

cora_author_list_winkler = comparison.FieldComparatorWinkler(
    thres=0, desc="author_list_winkler"
)
cora_title_winkler = comparison.FieldComparatorWinkler(thres=0, desc="title_winkler")
cora_conf_journal_winkler = comparison.FieldComparatorWinkler(
    thres=0, desc="conf_journal_winkler"
)

cora_author_list_qgram = comparison.FieldComparatorQGram(
    thres=0, common_divisor="average", desc="author_list_qgram"
)
cora_title_qgram = comparison.FieldComparatorQGram(
    thres=0, common_divisor="average", desc="title_qgram"
)
cora_conf_journal_qgram = comparison.FieldComparatorQGram(
    thres=0, common_divisor="average", desc="conf_journal_qgram"
)

cora_author_list_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="author_list_bagdist"
)
cora_title_bagdist = comparison.FieldComparatorBagDist(thres=0, desc="title_bagdist")
cora_conf_journal_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="conf_journal_bagdist"
)

cora_author_list_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="author_list_qgram"
)
cora_title_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="title_qgram"
)
cora_conf_journal_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="conf_journal_qgram"
)

cora_year_exact = comparison.FieldComparatorExactString(desc="year_exact")
cora_year_keydiff = comparison.FieldComparatorKeyDiff(
    max_key_diff=1, desc="year_keydiff"
)

cora_pages_exact = comparison.FieldComparatorExactString(desc="pages_exact")
cora_pages_keydiff = comparison.FieldComparatorKeyDiff(
    max_key_diff=2, desc="pages_keydiff"
)

cora_fc_list = [
    (cora_paper_id_exact, "paper_id", "paper_id"),
...
cora_rec_comp = comparison.RecordComparator(
    cora_ds, cora_ds, cora_fc_list, "Cora record comparator"
)
...
cora_dedup_index = indexing.DedupIndex(
    description="Cora Dedup index",
    dataset1=cora_ds,
    dataset2=cora_ds,
    weight_vec_file=weight_vect_dir + "cora-dedup-index-weight-vectors.csv",
...
synth_index_list = [
    [
        ["surname", "surname", False, False, None, [encode.soundex]],
        ["postcode", "postcode", False, False, None, [encode.get_substring, 0, 2]],
    ],
    [
        ["given_name", "given_name", False, False, None, [encode.soundex]],
        ["postcode", "postcode", False, False, None, [encode.get_substring, 1, 3]],
    ],
    [
        ["suburb", "suburb", False, False, None, [encode.soundex]],
        ["postcode", "postcode", False, False, None, [encode.get_substring, 2, 4]],
    ],
    [
        ["suburb", "suburb", False, False, None, [encode.soundex]],
    ...
    [
        ["surname", "surname", False, False, None, [encode.get_substring, 0, 2]],
        ["given_name", "given_name", False, False, None, [encode.get_substring, 0, 2]],
    ],
]

synth_surname_winkler = comparison.FieldComparatorWinkler(
    thres=0, desc="surname_winkler"
)
synth_given_name_winkler = comparison.FieldComparatorWinkler(
    thres=0, desc="given_name_winkler"
)
synth_address_1_winkler = comparison.FieldComparatorWinkler(
    thres=0, desc="address_1_winkler"
)
synth_suburb_winkler = comparison.FieldComparatorWinkler(thres=0, desc="suburb_winkler")

synth_surname_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="surname_qgram"
)
synth_given_name_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="given_name_qgram"
)
synth_address_1_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="address_1_qgram"
)
synth_suburb_qgram = comparison.FieldComparatorQGram(
    thres=0, q=2, common_divisor="average", desc="suburb_qgram"
)

synth_surname_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="surname_bagdist"
)
synth_given_name_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="given_name_bagdist"
)
synth_address_1_bagdist = comparison.FieldComparatorBagDist(
    thres=0, desc="address_1_bagdist"
)
synth_suburb_bagdist = comparison.FieldComparatorBagDist(thres=0, desc="suburb_bagdist")

synth_surname_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="surname_lcs"
)
synth_given_name_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="given_name_lcs"
)
synth_address_1_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="address_1_lcs"
)
synth_suburb_lcs = comparison.FieldComparatorLCS(
    thres=0, min_common_len=2, common_divisor="average", desc="suburb_lcs"
)

synth_street_num_exact = comparison.FieldComparatorExactString(desc="street_num_exact")
synth_street_num_keydiff = comparison.FieldComparatorKeyDiff(
    max_key_diff=1, desc="street_num_keydiff"
)

synth_postcode_exact = comparison.FieldComparatorExactString(desc="postcode_exact")
synth_postcode_keydiff = comparison.FieldComparatorKeyDiff(
    max_key_diff=1, desc="postcode_keydiff"
)

synth_state_exact = comparison.FieldComparatorExactString(desc="state_exact")
synth_state_keydiff = comparison.FieldComparatorKeyDiff(
    max_key_diff=1, desc="state_keydiff"
)
...
for size in [
    ...
    synth_ds = dataset.DataSetCSV(
        description="Febrl synthetic data set %s" % (size),
        access_mode="read",
        rec_ident="rec_id",
        header_line=True,
        file_name="./data/dedup-dsgen/%s" % (ds_file_name),
    )

    synth_rec_comp = comparison.RecordComparator(
        synth_ds, synth_ds, synth_fc_list, "Febrl synthetic data record" + " comparator"
    )

    synth_dedup_index = indexing.DedupIndex(
        description="Febrl synthetic " + " Dedup index",
        dataset1=synth_ds,
        dataset2=synth_ds,
...
for experiment in experiment_list:
    ...
    if data_set_a == data_set_b:
        ...
        res_file.write(os.linesep)
        res_file.write(
            "  Number of records in data set: %d" % (data_set_a.num_records)
            + os.linesep
        )
        ...
        res_file.write(os.linesep)
        res_file.write(
            "  Number of records in data sets: %d / %d "
    ...
    if weight_vec_file != None:
        ...
        #
        file_avail = (
            os.access(weight_vec_file, os.R_OK)
            or os.access(weight_vec_file + ".gz", os.R_OK)
            or os.access(weight_vec_file + ".GZ", os.R_OK)
        )  # Can read file
    else:
        file_avail = False
    ...
    if file_avail == True:  # File is available, so load it - - - - - - - - - -
        ...
        [field_names_list, w_vec_dict] = output.LoadWeightVectorFile(weight_vec_file)
        ...
        res_file.write(
            "  Build and compact index, then run comparison step" + os.linesep
        )
    ...
    res_file.write(os.linesep)
    res_file.write(
        "  Returned dictionary with %d weight vectors" % (num_w_vec) + os.linesep
    )
    res_file.flush()

    # Get the true matches and true non-matches in the weight vector dictionary
    #
    true_m_set, true_nm_set = classification.get_true_matches_nonmatches(
        w_vec_dict, match_check_funct
    )
    ...
    res_file.write(
        "  Number of true matches and non-matches: %d / %d" % (num_true_m, num_true_nm)
        + os.linesep
    )
    res_file.write(os.linesep)
    res_file.flush()
    ...
    acc, prec, reca, fmeas = measurements.quality_measures(
        w_vec_dict, true_m_set, true_nm_set, match_check_funct
    )
    ...
    if do_complexity == True:
        ...
        pc = measurements.pairs_completeness(
            w_vec_dict, data_set_a, data_set_b, get_id_funct, match_check_funct
        )
    ...
    for (sel_name, sel_list) in field_comp_sel:
        ...
        res_file.write("  Field comparison selection: %s" % (sel_name) + os.linesep)
        res_file.write(
            "  ----------------------------"
            + "-" * len(sel_name)
            + os.linesep
        ...
        res_file.write(
            "    Number of fields comparisons: %d" % (num_weights) + os.linesep
        )
        ...
        sel_w_vec_dict = classification.extract_collapse_weight_vectors(
            sel_list, w_vec_dict
        )
        ...
        res_file.write(
            "    Classifier experiment                |  Acc  |  "
            + "Prec |  Reca | F-Meas|  Time (s)"
            + os.linesep
        )
        res_file.write("    " + "=" * 80 + os.linesep)
        ...
        one_dim_w_vec_dict = classification.extract_collapse_weight_vectors(
            one_dim_sel_list, sel_w_vec_dict
        )
        assert len(one_dim_w_vec_dict) == num_w_vec

        # Three variations: minimise pos-neg, pos only, neg only - - - - - - - - -
        #
        opt_thres_param_list = [
            ("Minimise false pos-neg ", "pos-neg"),
        ...
        for (class_name, min_method_name) in opt_thres_param_list:
            ...
            opt_thres_classifier = classification.OptimalThreshold(
                bin_width=0.01, min_method=min_method_name
            )
            ...
            test_results = opt_thres_classifier.cross_validate(
                one_dim_w_vec_dict, true_m_set, true_nm_set, n
            )

            opt_acc, opt_prec, opt_reca, opt_fmeas = get_measures(test_results)

            opt_thres_res_list.append(
                (class_name, opt_acc, opt_prec, opt_reca, opt_fmeas, opt_time)
            )
            ...
            res_file.write(
                "    %37s| %5.3f | %5.3f | %5.3f | %5.3f | %9.3f"
                % (class_name, acc, prec, reca, fmeas, rtime)
                + os.linesep
            )
            res_file.flush()
        ...
        #
        res_file.write(
            "    " + "Support vector machine classifier".center(79) + os.linesep
        )
        res_file.write("    " + "-" * 80 + os.linesep)

        svm_param_list = [
            ("Linear kernel, C=10 ", "LINEAR", 10),
        ...
        for (class_name, kernel_name, C_val) in svm_param_list:
            ...
            svm_classifier = classification.SuppVecMachine(
                kernel_type=kernel_name, C=C_val
            )
            start_time = time.time()
            ...
            test_results = svm_classifier.cross_validate(
                sel_w_vec_dict, true_m_set, true_nm_set, n
            )

            svm_acc, svm_prec, svm_reca, svm_fmeas = get_measures(test_results)

            svm_res_list.append(
                (class_name, svm_acc, svm_prec, svm_reca, svm_fmeas, svm_time)
            )
        ...
        kmeans_param_list = [
            ("Manhatten dist, min/max init", mymath.distL1, "min/max"),
            ("Euclidean dist, min/max init", mymath.distL2, "min/max"),
            ...
            #                          'min/max'),
            ("Manhatten dist, random init", mymath.distL1, "random"),
            ("Euclidean dist, random init", mymath.distL2, "random"),
        ...
        for (class_name, dist_meas, centr_init) in kmeans_param_list:
            ...
            kmeans_classifier = classification.KMeans(
                dist_measure=dist_meas, max_iter_count=10000, centroid_init=centr_init
            )
            start_time = time.time()
            ...
            test_results = kmeans_classifier.cross_validate(
                sel_w_vec_dict, true_m_set, true_nm_set, n
            )

            kmeans_acc, kmeans_prec, kmeans_reca, kmeans_fmeas = get_measures(
                test_results
            )

            kmeans_res_list.append(
                (
                    class_name,
                    kmeans_acc,
                    kmeans_prec,
        ...
        #
        res_file.write("    " + "Farthest-first clustering".center(79) + os.linesep)
        res_file.write("    " + "-" * 80 + os.linesep)

        ffirst_param_list = [
            ("Manhatten dist, traditional init", mymath.distL1, "traditional"),
            ("Euclidean dist, traditional init", mymath.distL2, "traditional"),
            ("Cosine dist, traditional init", mymath.distCosine, "traditional"),
            ...
            ("Manhatten dist, min/max init", mymath.distL1, "min/max"),
            ("Euclidean dist, min/max init", mymath.distL2, "min/max"),
            ("Cosine dist, min/max init", mymath.distCosine, "min/max"),
            #                         ('L-Infinity dist, min/max init', mymath.distLInf,
            #                          'min/max'),
            #                         ('Canberra dist, min/max init', mymath.distCanberra,
            #                          'min/max'),
            ("Manhatten dist, mode/max init", mymath.distL1, "mode/max"),
            ("Euclidean dist, mode/max init", mymath.distL2, "mode/max"),
            ("Cosine dist, mode/max init", mymath.distCosine, "mode/max")
        ...
        for (class_name, dist_meas, centr_init) in ffirst_param_list:
            ...
            ffirst_classifier = classification.FarthestFirst(
                dist_measu=dist_meas, centroid_i=centr_init
            )
            start_time = time.time()
            ...
            test_results = ffirst_classifier.cross_validate(
                sel_w_vec_dict, true_m_set, true_nm_set, n
            )

            ffirst_acc, ffirst_prec, ffirst_reca, ffirst_fmeas = get_measures(
                test_results
            )

            ffirst_res_list.append(
                (
                    class_name,
                    ffirst_acc,
                    ffirst_prec,
        ...
        for (class_name, dist_meas, kernel_name, C_val) in tailor_param_list:
            ...
            tailor_classifier = classification.TAILOR(
            ...
            test_results = tailor_classifier.cross_validate(
                sel_w_vec_dict, true_m_set, true_nm_set, n
            )

            tailor_acc, tailor_prec, tailor_reca, tailor_fmeas = get_measures(
                test_results
            )

            tailor_res_list.append(
                (
                    class_name,
                    tailor_acc,
                    tailor_prec,
        ...
        #
        res_file.write(
            "    " + "Two-step classifier (threshold based)".center(79) + os.linesep
        )
        res_file.write("    " + "-" * 80 + os.linesep)
        ...
        for (class_name, s1_m, s1_nm, s2_class) in two_step_param_list:
            ...
            for rand_sel in [
                ...
                for iter in range(num_iter):  # Loop over number of iterations - - - -
                    two_step_classifier = classification.TwoStep(
                        s1_match_method=s1_m,
                    ...
                    test_results = two_step_classifier.cross_validate(
                        sel_w_vec_dict, true_m_set, true_nm_set, n
                    )

                    two_step_acc, two_step_prec, two_step_reca, two_step_fmeas = get_measures(
                        test_results
                    )
        ...
        #
        res_file.write(
            "    " + "Two-step classifier (nearest based)".center(79) + os.linesep
        )
        res_file.write("    " + "-" * 80 + os.linesep)
        ...
        for (class_name, s1_m, s1_nm, s2_class) in two_step_param_list:
            ...
            for rand_sel in [
                ...
                rec_w_vec_ratio = float(max_num_matches) / float(
                    num_w_vec - max_num_matches
                )
        ...
        if do_timing == True:
            ...
            res_file.write(
                ...
                + "min/max initialisation):         %7.2f" % (kmeans_time)
                + os.linesep
            )
            res_file.flush()

            ffirst_classifier = classification.FarthestFirst(
                dist_me=mymath.distL2, centroid_i="mode/max"
            )
            start_time = time.time()
            ffirst_classifier.train(sel_w_vec_dict, true_m_set, true_nm_set)
            res_sets = ffirst_classifier.classify(sel_w_vec_dict)
            ...
            res_file.write(
                "    Farthest first clustering (Euclidean distance, "
                + "mode/max initialisation): %7.2f" % (ffirst_time)
                + os.linesep
            )
            res_file.flush()

            two_s_classifier = classification.TwoStep(
                s1_match=(1.0, "threshold", 0.5),
            ...
            res_file.write(
                "    Two step classifier (thresholds 0.5/0.5, rand "
            ...
            res_file.write(
                "    Two step classifier (nearest 5%, B, U, "
                + "rand expo 5%%/5%%, SVM RBF C=1):    %7.2f" % (two_s_time)
                + os.linesep
            )
            res_file.flush()
            ...
            res_file.write(
                "    Two step classifier (nearest 5%, IB, U,"
                + " rand expo 5%%/5%%, SVM RBF C=1):   %7.2f" % (two_s_time)
                + os.linesep
            )
            res_file.flush()
...
for x in []:
    for y in []:
        for z in []:
            ...
            if (
                (do_random == True)
                and (num_m_train_data > 0)
                and (num_nm_train_data > 0)
            ):
                ...
                #
                num_m_random = max(1, int(num_m_train_data * 0.10))
                num_nm_random = max(1, int(num_nm_train_data * 0.10))
                ...
                for r_iter in range(num_random_select_iterations):
                    ...
                    #
                    start_time = time.time()
                    m_setru, nm_setru = classification.get_random_examples(
                        sel_w_vec_dict,
                        m_set_tr,
                        nm_set_tr,
                        "uniform",
                    ...
                    conf_matrix = classification.svm_classifier(
                        sel_w_vec_dict, r_m_set_tr, r_nm_set_tr, match_check_funct
                    )
                    rand_u_time.append(time.time() - start_time)

                    acc = float(conf_matrix[0] + conf_matrix[3]) / sum(conf_matrix)
                    prec = float(conf_matrix[0]) / float(
                        conf_matrix[0] + conf_matrix[2]
                    )

                    if float(conf_matrix[0] + conf_matrix[1]) > 0:
                        reca = float(conf_matrix[0]) / float(
                    ...
                    #
                    start_time = time.time()
                    m_setrl, nm_setrl = classification.get_random_examples(
                        sel_w_vec_dict,
                        m_set_tr,
                        nm_set_tr,
                        "linear",
                ...
                res_file.write(
                    "    %19s |%9f | %9f |%9f | %9f |%9f"
                    % ("(+ rand uni)".rjust(19), acc_u, prec_u, reca_u, fmeas_u, time_u)
                    + os.linesep
                )

                res_file.write(
                    "    %19s |%9f | %9f |%9f | %9f |%9f"
                    % ("(+ rand lin)".rjust(19), acc_l, prec_l, reca_l, fmeas_l, time_l)
                    + os.linesep
                )

                res_file.write(
                    "    %19s |%9f | %9f |%9f | %9f |%9f"
                    % ("(+ rand exp)".rjust(19), acc_e, prec_e, reca_e, fmeas_e, time_e)
                    + os.linesep
                )
                res_file.flush()
...
for data_set_decr_tuple in res_dict:  # Loop over all data set results
    ...
    tech_name_str_list = list(avrg_acc_dict.keys())
    ...
    for tech_name_str in tech_name_str_list:
        ...
        res_file.write(
            "  %38s | %9f | %9f | %9f | %9f"
            % (tech_name_str.rjust(37), avrg_acc, avrg_prec, avrg_reca, avrg_fmeas)
            + os.linesep
        )
    res_file.write(os.linesep)
...
m_sete, nm_sete = classification.get_examples(
    wv_dict,
...
m_sete, nm_sete = classification.get_examples(
    wv_dict, match_method=(1.0, "nearest", 100), non_match_method=(0.0, "nearest", 100)
)
