...
res_file.write(
    os.linesep
    + os.linesep
    + "Experiment started %s:" % (time.strftime("%Y%m%d-%H%M"))
    + os.linesep
)
...
#
cddb_ds = dataset.DataSetCSV(
    description="CDDB data set",
    access_mode="read",
    rec_ident="rec_id",
    header_line=True,
...
cddb_artist_winkler = comparison.FieldComparatorWinkler(thres=0, desc="artist_winkler")
cddb_title_winkler = comparison.FieldComparatorWinkler(thres=0, desc="title_winkler")

cddb_fc_list = [
    (cddb_class_exact, "identifier", "identifier"),
    (cddb_artist_winkler, "artist", "artist"),
    (cddb_title_winkler, "title", "title"),
]

cddb_rec_comp = comparison.RecordComparator(
    cddb_ds, cddb_ds, cddb_fc_list, "CDDB record comparator"
)
...
synth_fc_list = [
    (synth_rec_id_exact, "rec_id", "rec_id"),
...
for size in [
    ...
    if size[0] in ["A", "B", "C"]:  # A deduplication
        ...
        synth_ds1 = dataset.DataSetCSV(
            description="Febrl synthetic data set %s" % (size) + " (org)",
            access_mode="read",
            rec_ident="rec_id",
            header_line=True,
            file_name="./data/link-dsgen/%s" % (ds_file_name1),
        )

        synth_ds2 = dataset.DataSetCSV(
            description="Febrl synthetic data set %s" % (size) + " (dup)",
            access_mode="read",
            rec_ident="rec_id",
            header_line=True,
            file_name="./data/link-dsgen/%s" % (ds_file_name2),
        )
    ...
    synth_rec_comp = comparison.RecordComparator(
        ...
        synth_ds2,
        synth_fc_list,
        "Febrl synthetic data record" + " comparator",
    )
...
for k in list(experiment_dict.keys()):
...
for ds_name in experiment_name_list:
    ...
    for w in [2, 3, 5, 7, 10]:
        ...
        for this_index_def in index_def_list:
            ...
            sorted_index = indexing.SortingIndex(
                description="Sorting index: " + "w=%d" % (w),
                dataset1=data_set1,
                dataset2=data_set2,
                rec_comparator=rec_cmp,
                progress=progress_precentage,
    ...
    for w in [2, 3, 5, 7, 10]:
        ...
        for this_index_def in index_def_list:
            ...
            sorted_index = indexing.SortingArrayIndex(
                description="Sorting " + "array index: w=%d" % (w),
                dataset1=data_set1,
                dataset2=data_set2,
                rec_comparator=rec_cmp,
                progress=progress_precentage,
    ...
    #
    for str_cmp_funct in [
        ("Jaro", stringcmp.jaro),
        ("Bigram", stringcmp.bigram),
        ("ED", stringcmp.editdist),
        ("LCS", stringcmp.lcs),
    ]:
        for str_cmp_thres in [0.8, 0.9]:
            ...
            for this_index_def in index_def_list:
                ...
                sorted_index = indexing.AdaptSortingIndex(
                    description="Adaptive "
                    + "sorting index: str_cmp=%s" % (str_cmp_funct[0])
                    + ", thres=%.1f" % (str_cmp_thres),
                    dataset1=data_set1,
                    dataset2=data_set2,
                    rec_comparator=rec_cmp,
                    progress=progress_precentage,
        ...
        for max_block in [5, 10, 20]:
            ...
            for this_index_def in index_def_list:
                ...
                sarray_index = indexing.SuffixArrayIndex(
                    desc="Suffix array index:"
                    + " min_len=%d, max_block=%d" % (min_len, max_block),
                    dataset1=data_set1,
                    dataset2=data_set2,
                    rec_comparator=rec_cmp,
                    progress=progress_precentage,
                ...
                for str_cmp_thres in [0.8, 0.9]:
                    ...
                    for this_index_def in index_def_list:
                        ...
                        rsarray_index = indexing.RobustSuffixArrayIndex(
                            desc="Robust suffix array index:"
                            + " min_len=%d, max_block=%d," % (min_len, max_block)
                            + "str_cmp=%s, thres=%.1f"
        ...
        for thres in [0.8, 0.9]:
            ...
            for this_index_def in index_def_list:
                ...
                qgram_index = indexing.QGramIndex(
                    desc="Q-gram index: " + "q=%d, thres=%.1f" % (q, thres),
                    dataset1=data_set1,
                    dataset2=data_set2,
                    rec_comparator=rec_cmp,
                    progress=progress_precentage,
        ...
        for canopy_method in [
            ...
            for this_index_def in index_def_list:
                ...
                canopy_index = indexing.CanopyIndex(
                    desc="Canopy TH index: q=%d, %s" % (q, str(canopy_method)),
                    dataset1=data_set1,
                    dataset2=data_set2,
                    rec_comparator=rec_cmp,
                    progress=progress_precentage,
    ...
    for q in [2, 3]:
        for canopy_method in [
            ...
            for this_index_def in index_def_list:
                ...
                canopy_index = indexing.CanopyIndex(
                    desc="Canopy NN index: q=%d, %s" % (q, str(canopy_method)),
                    dataset1=data_set1,
                    dataset2=data_set2,
                    rec_comparator=rec_cmp,
                    progress=progress_precentage,
                ...
                for canopy_method in [
                    ...
                    for this_index_def in index_def_list:
                        ...
                        strmap_index = indexing.StringMapIndex(
                            desc="StringMap TH index:"
                            + " index: dim=%d, sub-dim=%d, grid=" % (dim, subdim)
                            + "%d, str_cmp=%s, %s"
                            % (grid, str_cmp_funct[0], str(canopy_method)),
                            dataset1=data_set1,
                            dataset2=data_set2,
                            rec_comparator=rec_cmp,
                            progress=progress_precentage,
    ...
    for (dim, subdim) in [(15, 3), (20, 5)]:
        for grid in [100, 1000]:
            for str_cmp_funct in [
                ...
                for canopy_method in [("nearest", 5, 10), ("nearest", 10, 20)]:

                    for this_index_def in index_def_list:

                        strmap_index = indexing.StringMapIndex(
                            desc="StringMap NN index:"
                            + " index: dim=%d, sub-dim=%d, grid=" % (dim, subdim)
                            + "%d, str_cmp=%s, %s"
                            % (grid, str_cmp_funct[0], str(canopy_method)),
                            dataset1=data_set1,
    ...
    for (index_name, index_method) in ds_index_list:
        ...
        #
        if (arg_index_method_name != "all") and (index_name != arg_index_method_name):
            print()
        ...
        print(
            "    Time used (in sec): build: %.4f,  compact: %.4f"
        ...
        if task == "dedup":
            ...
            if len(ent_id_dict1) < len(ent_id_dict2):
                for (ent_id, ent_count) in ent_id_dict1.items():
                    if ent_id in ent_id_dict2:
                        tm += ent_count * ent_id_dict2[ent_id]

            else:
                for (ent_id, ent_count) in ent_id_dict2.items():
